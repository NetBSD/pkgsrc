$NetBSD: patch-src_3rdparty_chromium_sandbox_policy_netbsd_sandbox__netbsd.cc,v 1.1 2022/04/18 11:18:19 adam Exp $

--- src/3rdparty/chromium/sandbox/policy/netbsd/sandbox_netbsd.cc.orig	2022-04-06 06:26:57.469415579 +0000
...
+    } else if (sandbox_type == SandboxType::kAudio) {
...
+  } else if (process_type == switches::kUtilityProcess) {
+    if (sandbox_type == SandboxType::kAudio)
+      SetPledge(NULL, "/etc/chromium/pledge.utility_audio");
+    else if (sandbox_type == SandboxType::kNetwork)
...
+  base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
+  if (SandboxTypeFromCommandLine(*command_line) == SandboxType::kNoSandbox) {
+    return false;
+  }
+
...
+
+  *error = ResourceLimits::Lower(
+      RLIMIT_DATA, static_cast<rlim_t>(kDataSizeLimit));
+
...
+  DCHECK(thread);
+  // XXXX currently just here to stop an "undefined reference" error.
+}
+
+}  // namespace policy
+}  // namespace sandbox
