# $NetBSD: cargo-depends.mk,v 1.5 2022/05/30 21:27:09 pin Exp $

CARGO_CRATE_DEPENDS+=	adler-1.0.2
CARGO_CRATE_DEPENDS+=	aho-corasick-0.7.18
CARGO_CRATE_DEPENDS+=	anyhow-1.0.57
CARGO_CRATE_DEPENDS+=	approx-0.4.0
CARGO_CRATE_DEPENDS+=	as-slice-0.1.5
CARGO_CRATE_DEPENDS+=	atomic-polyfill-0.1.7
CARGO_CRATE_DEPENDS+=	atty-0.2.14
CARGO_CRATE_DEPENDS+=	autocfg-1.1.0
CARGO_CRATE_DEPENDS+=	bare-metal-0.2.5
CARGO_CRATE_DEPENDS+=	bare-metal-1.0.0
CARGO_CRATE_DEPENDS+=	bit_field-0.10.1
CARGO_CRATE_DEPENDS+=	bitfield-0.13.2
CARGO_CRATE_DEPENDS+=	bitflags-1.3.2
CARGO_CRATE_DEPENDS+=	bstr-0.2.17
CARGO_CRATE_DEPENDS+=	byteorder-1.4.3
CARGO_CRATE_DEPENDS+=	bzip2-0.4.3
CARGO_CRATE_DEPENDS+=	bzip2-sys-0.1.11+1.0.8
CARGO_CRATE_DEPENDS+=	cc-1.0.72
CARGO_CRATE_DEPENDS+=	cfg-if-1.0.0
CARGO_CRATE_DEPENDS+=	chrono-0.4.19
CARGO_CRATE_DEPENDS+=	clap-3.1.18
CARGO_CRATE_DEPENDS+=	clap_lex-0.2.0
CARGO_CRATE_DEPENDS+=	console-0.15.0
CARGO_CRATE_DEPENDS+=	cortex-m-0.7.4
CARGO_CRATE_DEPENDS+=	crc32fast-1.3.0
CARGO_CRATE_DEPENDS+=	critical-section-0.2.6
CARGO_CRATE_DEPENDS+=	csv-1.1.6
CARGO_CRATE_DEPENDS+=	csv-core-0.1.10
CARGO_CRATE_DEPENDS+=	dbase-0.2.3
CARGO_CRATE_DEPENDS+=	embedded-hal-0.2.7
CARGO_CRATE_DEPENDS+=	encode_unicode-0.3.6
CARGO_CRATE_DEPENDS+=	flate2-1.0.22
CARGO_CRATE_DEPENDS+=	float-cmp-0.9.0
CARGO_CRATE_DEPENDS+=	generic-array-0.12.4
CARGO_CRATE_DEPENDS+=	generic-array-0.13.3
CARGO_CRATE_DEPENDS+=	generic-array-0.14.5
CARGO_CRATE_DEPENDS+=	geo-0.20.1
CARGO_CRATE_DEPENDS+=	geo-types-0.7.4
CARGO_CRATE_DEPENDS+=	geographiclib-rs-0.2.0
CARGO_CRATE_DEPENDS+=	geojson-0.22.4
CARGO_CRATE_DEPENDS+=	hash32-0.1.1
CARGO_CRATE_DEPENDS+=	hash32-0.2.1
CARGO_CRATE_DEPENDS+=	hashbrown-0.11.2
CARGO_CRATE_DEPENDS+=	heapless-0.6.1
CARGO_CRATE_DEPENDS+=	heapless-0.7.10
CARGO_CRATE_DEPENDS+=	hermit-abi-0.1.19
CARGO_CRATE_DEPENDS+=	indexmap-1.8.0
CARGO_CRATE_DEPENDS+=	indicatif-0.16.2
CARGO_CRATE_DEPENDS+=	itoa-0.4.8
CARGO_CRATE_DEPENDS+=	itoa-1.0.1
CARGO_CRATE_DEPENDS+=	kml-0.4.3
CARGO_CRATE_DEPENDS+=	lazy_static-1.4.0
CARGO_CRATE_DEPENDS+=	libc-0.2.112
CARGO_CRATE_DEPENDS+=	libm-0.2.2
CARGO_CRATE_DEPENDS+=	lock_api-0.4.7
CARGO_CRATE_DEPENDS+=	log-0.4.14
CARGO_CRATE_DEPENDS+=	memchr-2.4.1
CARGO_CRATE_DEPENDS+=	miniz_oxide-0.4.4
CARGO_CRATE_DEPENDS+=	nb-0.1.3
CARGO_CRATE_DEPENDS+=	nb-1.0.0
CARGO_CRATE_DEPENDS+=	num-integer-0.1.44
CARGO_CRATE_DEPENDS+=	num-traits-0.2.15
CARGO_CRATE_DEPENDS+=	number_prefix-0.4.0
CARGO_CRATE_DEPENDS+=	once_cell-1.9.0
CARGO_CRATE_DEPENDS+=	os_str_bytes-6.0.0
CARGO_CRATE_DEPENDS+=	pdqselect-0.1.0
CARGO_CRATE_DEPENDS+=	pkg-config-0.3.24
CARGO_CRATE_DEPENDS+=	polyline-0.9.0
CARGO_CRATE_DEPENDS+=	proc-macro2-1.0.36
CARGO_CRATE_DEPENDS+=	quick-xml-0.22.0
CARGO_CRATE_DEPENDS+=	quote-1.0.14
CARGO_CRATE_DEPENDS+=	regex-1.5.4
CARGO_CRATE_DEPENDS+=	regex-automata-0.1.10
CARGO_CRATE_DEPENDS+=	regex-syntax-0.6.25
CARGO_CRATE_DEPENDS+=	riscv-0.7.0
CARGO_CRATE_DEPENDS+=	riscv-target-0.1.2
CARGO_CRATE_DEPENDS+=	robust-0.2.3
CARGO_CRATE_DEPENDS+=	rstar-0.8.4
CARGO_CRATE_DEPENDS+=	rstar-0.9.3
CARGO_CRATE_DEPENDS+=	rustc_version-0.2.3
CARGO_CRATE_DEPENDS+=	ryu-1.0.9
CARGO_CRATE_DEPENDS+=	scopeguard-1.1.0
CARGO_CRATE_DEPENDS+=	semver-0.9.0
CARGO_CRATE_DEPENDS+=	semver-parser-0.7.0
CARGO_CRATE_DEPENDS+=	serde-1.0.133
CARGO_CRATE_DEPENDS+=	serde_derive-1.0.136
CARGO_CRATE_DEPENDS+=	serde_json-1.0.75
CARGO_CRATE_DEPENDS+=	shapefile-0.3.0
CARGO_CRATE_DEPENDS+=	smallvec-1.8.0
CARGO_CRATE_DEPENDS+=	spin-0.9.2
CARGO_CRATE_DEPENDS+=	stable_deref_trait-1.2.0
CARGO_CRATE_DEPENDS+=	strsim-0.10.0
CARGO_CRATE_DEPENDS+=	syn-1.0.86
CARGO_CRATE_DEPENDS+=	termcolor-1.1.2
CARGO_CRATE_DEPENDS+=	terminal_size-0.1.17
CARGO_CRATE_DEPENDS+=	textwrap-0.15.0
CARGO_CRATE_DEPENDS+=	thiserror-1.0.30
CARGO_CRATE_DEPENDS+=	thiserror-impl-1.0.30
CARGO_CRATE_DEPENDS+=	time-0.1.44
CARGO_CRATE_DEPENDS+=	topojson-0.5.0
CARGO_CRATE_DEPENDS+=	typenum-1.15.0
CARGO_CRATE_DEPENDS+=	unicode-width-0.1.9
CARGO_CRATE_DEPENDS+=	unicode-xid-0.2.2
CARGO_CRATE_DEPENDS+=	vcell-0.1.3
CARGO_CRATE_DEPENDS+=	version_check-0.9.4
CARGO_CRATE_DEPENDS+=	void-1.0.2
CARGO_CRATE_DEPENDS+=	volatile-register-0.2.1
CARGO_CRATE_DEPENDS+=	wasi-0.10.0+wasi-snapshot-preview1
CARGO_CRATE_DEPENDS+=	winapi-0.3.9
CARGO_CRATE_DEPENDS+=	winapi-i686-pc-windows-gnu-0.4.0
CARGO_CRATE_DEPENDS+=	winapi-util-0.1.5
CARGO_CRATE_DEPENDS+=	winapi-x86_64-pc-windows-gnu-0.4.0
CARGO_CRATE_DEPENDS+=	wkt-0.10.1
CARGO_CRATE_DEPENDS+=	zip-0.5.13
