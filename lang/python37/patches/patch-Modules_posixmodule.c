...
@@ -3435,15 +3608,43 @@ os_link_impl(PyObject *module, path_t *s
 #ifdef HAVE_LINKAT
...
+
 ...
 #endif /* MS_WINDOWS */
@@ -3566,6 +3767,7 @@ _posix_listdir(path_t *path, PyObject *l
     errno = 0;
 #ifdef HAVE_FDOPENDIR
     if (path->fd != -1) {
+      if (HAVE_FDOPENDIR_RUNTIME) {
         ...
         fd = _Py_dup(path->fd);
         if (fd == -1)
@@ -3576,6 +3778,11 @@ _posix_listdir(path_t *path, PyObject *l
         Py_BEGIN_ALLOW_THREADS
         dirp = fdopendir(fd);
         Py_END_ALLOW_THREADS
+      } else {
