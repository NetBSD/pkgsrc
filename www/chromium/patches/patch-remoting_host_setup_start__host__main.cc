$NetBSD: patch-remoting_host_setup_start__host__main.cc,v 1.25 2026/09/02 13:13:34 kikadf Exp $

* Part of patchset to build chromium on NetBSD
* Based on OpenBSD's chromium patches, and
  pkgsrc's qt5-qtwebengine patches

--- remoting/host/setup/start_host_main.cc.orig	2026-08-31 22:47:51.000000000 +0000
...
@@ -92,7 +92,7 @@ constexpr char kDisableCrashReportingSwi
...
+#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_BSD)
 ...
 #endif
@@ -119,7 +119,7 @@ void PrintDefaultHelpMessage(const char*
...
@@ -135,7 +135,7 @@ void PrintCorpUserHelpMessage(const char
...
+#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_BSD)
   std::cerr << base::StringPrintf(" [--%s=<host type>]", kHostTypeSwitchName)
             << "\n\n";
   HostType::PrintHostTypeHelp();
@@ -159,7 +159,7 @@ void PrintCloudUserHelpMessage(const cha
...
+#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_BSD)
   std::cerr << base::StringPrintf(" [--%s=<host type>]", kHostTypeSwitchName)
             << "\n\n";
   HostType::PrintHostTypeHelp();
@@ -339,7 +339,7 @@ bool InitializeCorpMachineParams(HostSta
...
@@ -388,7 +388,7 @@ bool InitializeCloudMachineParams(HostSt
     cloud_arg_count++;
   }
