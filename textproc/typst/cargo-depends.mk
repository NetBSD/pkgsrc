# $NetBSD: cargo-depends.mk,v 1.20 2026/06/22 07:56:09 pin Exp $

CARGO_CRATE_DEPENDS+=	adler2-2.0.0
CARGO_CRATE_DEPENDS+=	aho-corasick-1.1.3
CARGO_CRATE_DEPENDS+=	android-tzdata-0.1.1
CARGO_CRATE_DEPENDS+=	android_system_properties-0.1.5
CARGO_CRATE_DEPENDS+=	anstream-0.6.18
CARGO_CRATE_DEPENDS+=	anstyle-1.0.10
CARGO_CRATE_DEPENDS+=	anstyle-parse-0.2.6
CARGO_CRATE_DEPENDS+=	anstyle-query-1.1.2
CARGO_CRATE_DEPENDS+=	anstyle-wincon-3.0.7
CARGO_CRATE_DEPENDS+=	approx-0.5.1
CARGO_CRATE_DEPENDS+=	arbitrary-1.4.1
CARGO_CRATE_DEPENDS+=	arrayref-0.3.9
CARGO_CRATE_DEPENDS+=	arrayvec-0.7.6
CARGO_CRATE_DEPENDS+=	ascii-1.1.0
CARGO_CRATE_DEPENDS+=	autocfg-1.4.0
CARGO_CRATE_DEPENDS+=	az-1.2.1
CARGO_CRATE_DEPENDS+=	base64-0.22.1
CARGO_CRATE_DEPENDS+=	biblatex-0.12.0
CARGO_CRATE_DEPENDS+=	bincode-1.3.3
CARGO_CRATE_DEPENDS+=	bit-set-0.8.0
CARGO_CRATE_DEPENDS+=	bit-vec-0.8.0
CARGO_CRATE_DEPENDS+=	bitflags-1.3.2
CARGO_CRATE_DEPENDS+=	bitflags-2.11.1
CARGO_CRATE_DEPENDS+=	bitvec-1.0.1
CARGO_CRATE_DEPENDS+=	bstr-1.12.1
CARGO_CRATE_DEPENDS+=	bumpalo-3.20.2
CARGO_CRATE_DEPENDS+=	by_address-1.2.1
CARGO_CRATE_DEPENDS+=	bytemuck-1.25.0
CARGO_CRATE_DEPENDS+=	bytemuck_derive-1.10.2
CARGO_CRATE_DEPENDS+=	byteorder-1.5.0
CARGO_CRATE_DEPENDS+=	byteorder-lite-0.1.0
CARGO_CRATE_DEPENDS+=	cc-1.2.11
CARGO_CRATE_DEPENDS+=	cfg-if-1.0.0
CARGO_CRATE_DEPENDS+=	chinese-number-0.7.8
CARGO_CRATE_DEPENDS+=	chinese-variant-1.1.5
CARGO_CRATE_DEPENDS+=	chrono-0.4.39
CARGO_CRATE_DEPENDS+=	chunked_transfer-1.5.0
CARGO_CRATE_DEPENDS+=	ciborium-0.2.2
CARGO_CRATE_DEPENDS+=	ciborium-io-0.2.2
CARGO_CRATE_DEPENDS+=	ciborium-ll-0.2.2
CARGO_CRATE_DEPENDS+=	citationberg-0.7.0
CARGO_CRATE_DEPENDS+=	clap-4.5.28
CARGO_CRATE_DEPENDS+=	clap_builder-4.5.27
CARGO_CRATE_DEPENDS+=	clap_complete-4.5.44
CARGO_CRATE_DEPENDS+=	clap_derive-4.5.28
CARGO_CRATE_DEPENDS+=	clap_lex-0.7.4
CARGO_CRATE_DEPENDS+=	clap_mangen-0.3.0
CARGO_CRATE_DEPENDS+=	cobs-0.3.0
CARGO_CRATE_DEPENDS+=	codespan-reporting-0.11.1
CARGO_CRATE_DEPENDS+=	codex-0.3.0
CARGO_CRATE_DEPENDS+=	color-0.3.3
CARGO_CRATE_DEPENDS+=	color-print-0.3.7
CARGO_CRATE_DEPENDS+=	color-print-proc-macro-0.3.7
CARGO_CRATE_DEPENDS+=	color_quant-1.1.0
CARGO_CRATE_DEPENDS+=	colorchoice-1.0.3
CARGO_CRATE_DEPENDS+=	comemo-0.5.1
CARGO_CRATE_DEPENDS+=	comemo-macros-0.5.1
CARGO_CRATE_DEPENDS+=	core-foundation-0.9.4
CARGO_CRATE_DEPENDS+=	core-foundation-sys-0.8.7
CARGO_CRATE_DEPENDS+=	core_maths-0.1.1
CARGO_CRATE_DEPENDS+=	crc32fast-1.5.0
CARGO_CRATE_DEPENDS+=	crossbeam-channel-0.5.15
CARGO_CRATE_DEPENDS+=	crossbeam-deque-0.8.6
CARGO_CRATE_DEPENDS+=	crossbeam-epoch-0.9.18
CARGO_CRATE_DEPENDS+=	crossbeam-utils-0.8.21
CARGO_CRATE_DEPENDS+=	crunchy-0.2.3
CARGO_CRATE_DEPENDS+=	csv-1.3.1
CARGO_CRATE_DEPENDS+=	csv-core-0.1.11
CARGO_CRATE_DEPENDS+=	data-url-0.3.1
CARGO_CRATE_DEPENDS+=	deranged-0.3.11
CARGO_CRATE_DEPENDS+=	dirs-6.0.0
CARGO_CRATE_DEPENDS+=	dirs-sys-0.5.0
CARGO_CRATE_DEPENDS+=	displaydoc-0.2.5
CARGO_CRATE_DEPENDS+=	ecow-0.2.6
CARGO_CRATE_DEPENDS+=	either-1.13.0
CARGO_CRATE_DEPENDS+=	enum-ordinalize-4.3.2
CARGO_CRATE_DEPENDS+=	enum-ordinalize-derive-4.3.2
CARGO_CRATE_DEPENDS+=	env_proxy-0.4.1
CARGO_CRATE_DEPENDS+=	equivalent-1.0.1
CARGO_CRATE_DEPENDS+=	errno-0.3.10
CARGO_CRATE_DEPENDS+=	euclid-0.22.14
CARGO_CRATE_DEPENDS+=	fancy-regex-0.16.2
CARGO_CRATE_DEPENDS+=	fast-srgb8-1.0.0
CARGO_CRATE_DEPENDS+=	fastrand-2.3.0
CARGO_CRATE_DEPENDS+=	fdeflate-0.3.7
CARGO_CRATE_DEPENDS+=	fearless_simd-0.4.0
CARGO_CRATE_DEPENDS+=	filetime-0.2.28
CARGO_CRATE_DEPENDS+=	flate2-1.1.2
CARGO_CRATE_DEPENDS+=	float-cmp-0.9.0
CARGO_CRATE_DEPENDS+=	fnv-1.0.7
CARGO_CRATE_DEPENDS+=	foldhash-0.2.0
CARGO_CRATE_DEPENDS+=	font-types-0.11.3
CARGO_CRATE_DEPENDS+=	fontconfig-parser-0.5.7
CARGO_CRATE_DEPENDS+=	fontdb-0.23.0
CARGO_CRATE_DEPENDS+=	foreign-types-0.3.2
CARGO_CRATE_DEPENDS+=	foreign-types-shared-0.1.1
CARGO_CRATE_DEPENDS+=	form_urlencoded-1.2.1
CARGO_CRATE_DEPENDS+=	fs_extra-1.3.0
CARGO_CRATE_DEPENDS+=	fsevent-sys-4.1.0
CARGO_CRATE_DEPENDS+=	funty-2.0.0
CARGO_CRATE_DEPENDS+=	getrandom-0.2.15
CARGO_CRATE_DEPENDS+=	getrandom-0.3.1
CARGO_CRATE_DEPENDS+=	gif-0.14.2
CARGO_CRATE_DEPENDS+=	glidesort-0.1.2
CARGO_CRATE_DEPENDS+=	glifo-0.1.0
CARGO_CRATE_DEPENDS+=	guillotiere-0.7.0
CARGO_CRATE_DEPENDS+=	half-2.4.1
CARGO_CRATE_DEPENDS+=	hashbrown-0.16.1
CARGO_CRATE_DEPENDS+=	hashbrown-0.17.1
CARGO_CRATE_DEPENDS+=	hayagriva-0.10.1
CARGO_CRATE_DEPENDS+=	hayro-0.7.1
CARGO_CRATE_DEPENDS+=	hayro-ccitt-0.3.0
CARGO_CRATE_DEPENDS+=	hayro-cmap-0.1.0
CARGO_CRATE_DEPENDS+=	hayro-interpret-0.7.0
CARGO_CRATE_DEPENDS+=	hayro-jbig2-0.3.0
CARGO_CRATE_DEPENDS+=	hayro-jpeg2000-0.3.5
CARGO_CRATE_DEPENDS+=	hayro-postscript-0.1.0
CARGO_CRATE_DEPENDS+=	hayro-svg-0.7.0
CARGO_CRATE_DEPENDS+=	hayro-syntax-0.7.2
CARGO_CRATE_DEPENDS+=	hayro-write-0.7.0
CARGO_CRATE_DEPENDS+=	heck-0.5.0
CARGO_CRATE_DEPENDS+=	httpdate-1.0.3
CARGO_CRATE_DEPENDS+=	hypher-0.1.5
CARGO_CRATE_DEPENDS+=	iana-time-zone-0.1.61
CARGO_CRATE_DEPENDS+=	iana-time-zone-haiku-0.1.2
CARGO_CRATE_DEPENDS+=	icu_collator-2.2.0
CARGO_CRATE_DEPENDS+=	icu_collator_data-2.2.0
CARGO_CRATE_DEPENDS+=	icu_collections-2.2.0
CARGO_CRATE_DEPENDS+=	icu_locale-2.2.0
CARGO_CRATE_DEPENDS+=	icu_locale_core-2.2.0
CARGO_CRATE_DEPENDS+=	icu_locale_data-2.2.0
CARGO_CRATE_DEPENDS+=	icu_normalizer-2.2.0
CARGO_CRATE_DEPENDS+=	icu_normalizer_data-2.2.0
CARGO_CRATE_DEPENDS+=	icu_properties-2.2.0
CARGO_CRATE_DEPENDS+=	icu_properties_data-2.2.0
CARGO_CRATE_DEPENDS+=	icu_provider-2.2.0
CARGO_CRATE_DEPENDS+=	icu_provider_blob-2.2.0
CARGO_CRATE_DEPENDS+=	icu_segmenter-2.2.0
CARGO_CRATE_DEPENDS+=	icu_segmenter_data-2.2.0
CARGO_CRATE_DEPENDS+=	idna-1.0.3
CARGO_CRATE_DEPENDS+=	idna_adapter-1.2.2
CARGO_CRATE_DEPENDS+=	image-0.25.10
CARGO_CRATE_DEPENDS+=	image-webp-0.2.4
CARGO_CRATE_DEPENDS+=	imagesize-0.14.0
CARGO_CRATE_DEPENDS+=	indexmap-2.12.1
CARGO_CRATE_DEPENDS+=	infer-0.19.0
CARGO_CRATE_DEPENDS+=	inotify-0.11.0
CARGO_CRATE_DEPENDS+=	inotify-sys-0.1.5
CARGO_CRATE_DEPENDS+=	is-docker-0.2.0
CARGO_CRATE_DEPENDS+=	is-wsl-0.4.0
CARGO_CRATE_DEPENDS+=	is_terminal_polyfill-1.70.1
CARGO_CRATE_DEPENDS+=	itoa-1.0.14
CARGO_CRATE_DEPENDS+=	jobserver-0.1.32
CARGO_CRATE_DEPENDS+=	js-sys-0.3.77
CARGO_CRATE_DEPENDS+=	kamadak-exif-0.6.1
CARGO_CRATE_DEPENDS+=	kqueue-1.0.8
CARGO_CRATE_DEPENDS+=	kqueue-sys-1.0.4
CARGO_CRATE_DEPENDS+=	krilla-0.8.2
CARGO_CRATE_DEPENDS+=	krilla-svg-0.8.1
CARGO_CRATE_DEPENDS+=	kurbo-0.13.1
CARGO_CRATE_DEPENDS+=	libc-0.2.169
CARGO_CRATE_DEPENDS+=	libdeflate-sys-1.25.2
CARGO_CRATE_DEPENDS+=	libdeflater-1.25.2
CARGO_CRATE_DEPENDS+=	libfuzzer-sys-0.4.9
CARGO_CRATE_DEPENDS+=	libm-0.2.11
CARGO_CRATE_DEPENDS+=	libredox-0.1.3
CARGO_CRATE_DEPENDS+=	libz-rs-sys-0.5.1
CARGO_CRATE_DEPENDS+=	linebender_resource_handle-0.1.1
CARGO_CRATE_DEPENDS+=	linked-hash-map-0.5.6
CARGO_CRATE_DEPENDS+=	linux-raw-sys-0.4.15
CARGO_CRATE_DEPENDS+=	lipsum-0.9.1
CARGO_CRATE_DEPENDS+=	litemap-0.8.2
CARGO_CRATE_DEPENDS+=	lock_api-0.4.12
CARGO_CRATE_DEPENDS+=	log-0.4.29
CARGO_CRATE_DEPENDS+=	lzma-sys-0.1.20
CARGO_CRATE_DEPENDS+=	memchr-2.8.0
CARGO_CRATE_DEPENDS+=	memmap2-0.9.5
CARGO_CRATE_DEPENDS+=	minimal-lexical-0.2.1
CARGO_CRATE_DEPENDS+=	miniz_oxide-0.8.5
CARGO_CRATE_DEPENDS+=	mio-1.0.4
CARGO_CRATE_DEPENDS+=	moxcms-0.8.1
CARGO_CRATE_DEPENDS+=	mutate_once-0.1.1
CARGO_CRATE_DEPENDS+=	native-tls-0.2.13
CARGO_CRATE_DEPENDS+=	nom-7.1.3
CARGO_CRATE_DEPENDS+=	notify-8.0.0
CARGO_CRATE_DEPENDS+=	notify-types-2.0.0
CARGO_CRATE_DEPENDS+=	num-bigint-0.4.6
CARGO_CRATE_DEPENDS+=	num-conv-0.1.0
CARGO_CRATE_DEPENDS+=	num-integer-0.1.46
CARGO_CRATE_DEPENDS+=	num-traits-0.2.19
CARGO_CRATE_DEPENDS+=	once_cell-1.20.2
CARGO_CRATE_DEPENDS+=	open-5.3.2
CARGO_CRATE_DEPENDS+=	openssl-0.10.80
CARGO_CRATE_DEPENDS+=	openssl-macros-0.1.1
CARGO_CRATE_DEPENDS+=	openssl-probe-0.1.6
CARGO_CRATE_DEPENDS+=	openssl-src-300.4.1+3.4.0
CARGO_CRATE_DEPENDS+=	openssl-sys-0.9.116
CARGO_CRATE_DEPENDS+=	option-ext-0.2.0
CARGO_CRATE_DEPENDS+=	oxipng-10.0.0
CARGO_CRATE_DEPENDS+=	palette-0.7.6
CARGO_CRATE_DEPENDS+=	palette_derive-0.7.6
CARGO_CRATE_DEPENDS+=	parking_lot-0.12.3
CARGO_CRATE_DEPENDS+=	parking_lot_core-0.9.10
CARGO_CRATE_DEPENDS+=	paste-1.0.15
CARGO_CRATE_DEPENDS+=	pathdiff-0.2.3
CARGO_CRATE_DEPENDS+=	pdf-writer-0.15.0
CARGO_CRATE_DEPENDS+=	peniko-0.6.1
CARGO_CRATE_DEPENDS+=	percent-encoding-2.3.2
CARGO_CRATE_DEPENDS+=	phf-0.11.3
CARGO_CRATE_DEPENDS+=	phf-0.13.1
CARGO_CRATE_DEPENDS+=	phf_codegen-0.11.3
CARGO_CRATE_DEPENDS+=	phf_generator-0.11.3
CARGO_CRATE_DEPENDS+=	phf_generator-0.13.1
CARGO_CRATE_DEPENDS+=	phf_macros-0.13.1
CARGO_CRATE_DEPENDS+=	phf_shared-0.11.3
CARGO_CRATE_DEPENDS+=	phf_shared-0.13.1
CARGO_CRATE_DEPENDS+=	pic-scale-0.7.9
CARGO_CRATE_DEPENDS+=	pico-args-0.5.0
CARGO_CRATE_DEPENDS+=	pixglyph-0.6.1
CARGO_CRATE_DEPENDS+=	pkg-config-0.3.31
CARGO_CRATE_DEPENDS+=	plist-1.8.0
CARGO_CRATE_DEPENDS+=	png-0.18.1
CARGO_CRATE_DEPENDS+=	polycool-0.4.0
CARGO_CRATE_DEPENDS+=	portable-atomic-1.10.0
CARGO_CRATE_DEPENDS+=	postcard-1.1.3
CARGO_CRATE_DEPENDS+=	potential_utf-0.1.5
CARGO_CRATE_DEPENDS+=	powerfmt-0.2.0
CARGO_CRATE_DEPENDS+=	ppv-lite86-0.2.20
CARGO_CRATE_DEPENDS+=	proc-macro-hack-0.5.20+deprecated
CARGO_CRATE_DEPENDS+=	proc-macro2-1.0.101
CARGO_CRATE_DEPENDS+=	psm-0.1.24
CARGO_CRATE_DEPENDS+=	pxfm-0.1.24
CARGO_CRATE_DEPENDS+=	quick-error-2.0.1
CARGO_CRATE_DEPENDS+=	quick-xml-0.38.3
CARGO_CRATE_DEPENDS+=	quote-1.0.45
CARGO_CRATE_DEPENDS+=	radium-0.7.0
CARGO_CRATE_DEPENDS+=	rand-0.8.6
CARGO_CRATE_DEPENDS+=	rand_chacha-0.3.1
CARGO_CRATE_DEPENDS+=	rand_core-0.6.4
CARGO_CRATE_DEPENDS+=	rayon-1.12.0
CARGO_CRATE_DEPENDS+=	rayon-core-1.13.0
CARGO_CRATE_DEPENDS+=	read-fonts-0.39.2
CARGO_CRATE_DEPENDS+=	redox_syscall-0.5.8
CARGO_CRATE_DEPENDS+=	redox_users-0.5.0
CARGO_CRATE_DEPENDS+=	regex-1.11.1
CARGO_CRATE_DEPENDS+=	regex-automata-0.4.9
CARGO_CRATE_DEPENDS+=	regex-syntax-0.8.5
CARGO_CRATE_DEPENDS+=	resvg-0.47.0
CARGO_CRATE_DEPENDS+=	rgb-0.8.53
CARGO_CRATE_DEPENDS+=	roff-1.1.1
CARGO_CRATE_DEPENDS+=	roman-numerals-rs-3.1.0
CARGO_CRATE_DEPENDS+=	roxmltree-0.20.0
CARGO_CRATE_DEPENDS+=	roxmltree-0.21.1
CARGO_CRATE_DEPENDS+=	rust_decimal-1.36.0
CARGO_CRATE_DEPENDS+=	rustc-hash-2.1.1
CARGO_CRATE_DEPENDS+=	rustix-0.38.44
CARGO_CRATE_DEPENDS+=	rustversion-1.0.19
CARGO_CRATE_DEPENDS+=	rustybuzz-0.20.1
CARGO_CRATE_DEPENDS+=	ryu-1.0.19
CARGO_CRATE_DEPENDS+=	same-file-1.0.6
CARGO_CRATE_DEPENDS+=	schannel-0.1.27
CARGO_CRATE_DEPENDS+=	scopeguard-1.2.0
CARGO_CRATE_DEPENDS+=	security-framework-2.11.1
CARGO_CRATE_DEPENDS+=	security-framework-sys-2.14.0
CARGO_CRATE_DEPENDS+=	self-replace-1.5.0
CARGO_CRATE_DEPENDS+=	semver-1.0.25
CARGO_CRATE_DEPENDS+=	serde-1.0.228
CARGO_CRATE_DEPENDS+=	serde_core-1.0.228
CARGO_CRATE_DEPENDS+=	serde_derive-1.0.228
CARGO_CRATE_DEPENDS+=	serde_json-1.0.138
CARGO_CRATE_DEPENDS+=	serde_path_to_error-0.1.20
CARGO_CRATE_DEPENDS+=	serde_spanned-0.6.8
CARGO_CRATE_DEPENDS+=	serde_yaml-0.9.34+deprecated
CARGO_CRATE_DEPENDS+=	shell-escape-0.1.5
CARGO_CRATE_DEPENDS+=	shlex-1.3.0
CARGO_CRATE_DEPENDS+=	sigpipe-0.1.3
CARGO_CRATE_DEPENDS+=	simd-adler32-0.3.7
CARGO_CRATE_DEPENDS+=	similar-3.1.0
CARGO_CRATE_DEPENDS+=	simplecss-0.2.2
CARGO_CRATE_DEPENDS+=	siphasher-1.0.1
CARGO_CRATE_DEPENDS+=	skrifa-0.42.1
CARGO_CRATE_DEPENDS+=	slab-0.4.11
CARGO_CRATE_DEPENDS+=	slotmap-1.0.7
CARGO_CRATE_DEPENDS+=	smallvec-1.15.1
CARGO_CRATE_DEPENDS+=	spin-0.9.8
CARGO_CRATE_DEPENDS+=	stable_deref_trait-1.2.0
CARGO_CRATE_DEPENDS+=	stacker-0.1.21
CARGO_CRATE_DEPENDS+=	strict-num-0.1.1
CARGO_CRATE_DEPENDS+=	strsim-0.11.1
CARGO_CRATE_DEPENDS+=	strum-0.27.2
CARGO_CRATE_DEPENDS+=	strum_macros-0.27.2
CARGO_CRATE_DEPENDS+=	subsetter-0.2.6
CARGO_CRATE_DEPENDS+=	svgtypes-0.16.1
CARGO_CRATE_DEPENDS+=	syn-2.0.98
CARGO_CRATE_DEPENDS+=	synstructure-0.13.1
CARGO_CRATE_DEPENDS+=	syntect-5.3.0
CARGO_CRATE_DEPENDS+=	tap-1.0.1
CARGO_CRATE_DEPENDS+=	tar-0.4.46
CARGO_CRATE_DEPENDS+=	tempfile-3.16.0
CARGO_CRATE_DEPENDS+=	termcolor-1.4.1
CARGO_CRATE_DEPENDS+=	terminal_size-0.4.1
CARGO_CRATE_DEPENDS+=	thin-vec-0.2.18
CARGO_CRATE_DEPENDS+=	thiserror-2.0.18
CARGO_CRATE_DEPENDS+=	thiserror-impl-2.0.18
CARGO_CRATE_DEPENDS+=	time-0.3.37
CARGO_CRATE_DEPENDS+=	time-core-0.1.2
CARGO_CRATE_DEPENDS+=	time-macros-0.2.19
CARGO_CRATE_DEPENDS+=	tiny-skia-0.12.0
CARGO_CRATE_DEPENDS+=	tiny-skia-path-0.12.0
CARGO_CRATE_DEPENDS+=	tiny_http-0.12.0
CARGO_CRATE_DEPENDS+=	tinystr-0.8.3
CARGO_CRATE_DEPENDS+=	tinyvec-1.8.1
CARGO_CRATE_DEPENDS+=	tinyvec_macros-0.1.1
CARGO_CRATE_DEPENDS+=	toml-0.8.19
CARGO_CRATE_DEPENDS+=	toml_datetime-0.6.8
CARGO_CRATE_DEPENDS+=	toml_edit-0.22.23
CARGO_CRATE_DEPENDS+=	ttf-parser-0.25.1
CARGO_CRATE_DEPENDS+=	two-face-0.4.3
CARGO_CRATE_DEPENDS+=	typed-arena-2.0.2
CARGO_CRATE_DEPENDS+=	typed-path-0.12.3
CARGO_CRATE_DEPENDS+=	typst-assets-0.15.0
#CARGO_CRATE_DEPENDS+=	typst-dev-assets-0.15.0
CARGO_CRATE_DEPENDS+=	unic-langid-0.9.6
CARGO_CRATE_DEPENDS+=	unic-langid-impl-0.9.6
CARGO_CRATE_DEPENDS+=	unic-langid-macros-0.9.6
CARGO_CRATE_DEPENDS+=	unic-langid-macros-impl-0.9.6
CARGO_CRATE_DEPENDS+=	unicode-bidi-0.3.18
CARGO_CRATE_DEPENDS+=	unicode-bidi-mirroring-0.4.0
CARGO_CRATE_DEPENDS+=	unicode-ccc-0.4.0
CARGO_CRATE_DEPENDS+=	unicode-ident-1.0.16
CARGO_CRATE_DEPENDS+=	unicode-math-class-0.1.0
CARGO_CRATE_DEPENDS+=	unicode-normalization-0.1.24
CARGO_CRATE_DEPENDS+=	unicode-properties-0.1.3
CARGO_CRATE_DEPENDS+=	unicode-script-0.5.7
CARGO_CRATE_DEPENDS+=	unicode-segmentation-1.12.0
CARGO_CRATE_DEPENDS+=	unicode-vo-0.1.0
CARGO_CRATE_DEPENDS+=	unicode-width-0.1.14
CARGO_CRATE_DEPENDS+=	unicode_names2-2.0.0
CARGO_CRATE_DEPENDS+=	unicode_names2_generator-2.0.0
CARGO_CRATE_DEPENDS+=	unsafe-libyaml-0.2.11
CARGO_CRATE_DEPENDS+=	unscanny-0.1.0
CARGO_CRATE_DEPENDS+=	ureq-2.12.1
CARGO_CRATE_DEPENDS+=	url-2.5.4
CARGO_CRATE_DEPENDS+=	usvg-0.47.0
CARGO_CRATE_DEPENDS+=	utf16_iter-1.0.5
CARGO_CRATE_DEPENDS+=	utf8_iter-1.0.4
CARGO_CRATE_DEPENDS+=	utf8parse-0.2.2
CARGO_CRATE_DEPENDS+=	vcpkg-0.2.15
CARGO_CRATE_DEPENDS+=	vello_common-0.0.8
CARGO_CRATE_DEPENDS+=	vello_cpu-0.0.8
CARGO_CRATE_DEPENDS+=	version_check-0.9.5
CARGO_CRATE_DEPENDS+=	walkdir-2.5.0
CARGO_CRATE_DEPENDS+=	wasi-0.11.0+wasi-snapshot-preview1
CARGO_CRATE_DEPENDS+=	wasi-0.13.3+wasi-0.2.2
CARGO_CRATE_DEPENDS+=	wasm-bindgen-0.2.100
CARGO_CRATE_DEPENDS+=	wasm-bindgen-backend-0.2.100
CARGO_CRATE_DEPENDS+=	wasm-bindgen-macro-0.2.100
CARGO_CRATE_DEPENDS+=	wasm-bindgen-macro-support-0.2.100
CARGO_CRATE_DEPENDS+=	wasm-bindgen-shared-0.2.100
CARGO_CRATE_DEPENDS+=	wasmi-1.0.9
CARGO_CRATE_DEPENDS+=	wasmi_collections-1.0.9
CARGO_CRATE_DEPENDS+=	wasmi_core-1.0.9
CARGO_CRATE_DEPENDS+=	wasmi_ir-1.0.9
CARGO_CRATE_DEPENDS+=	wasmparser-0.228.0
CARGO_CRATE_DEPENDS+=	web-sys-0.3.77
CARGO_CRATE_DEPENDS+=	weezl-0.1.12
CARGO_CRATE_DEPENDS+=	winapi-util-0.1.9
CARGO_CRATE_DEPENDS+=	windows-core-0.52.0
CARGO_CRATE_DEPENDS+=	windows-sys-0.52.0
CARGO_CRATE_DEPENDS+=	windows-sys-0.59.0
CARGO_CRATE_DEPENDS+=	windows-targets-0.52.6
CARGO_CRATE_DEPENDS+=	windows_aarch64_gnullvm-0.52.6
CARGO_CRATE_DEPENDS+=	windows_aarch64_msvc-0.52.6
CARGO_CRATE_DEPENDS+=	windows_i686_gnu-0.52.6
CARGO_CRATE_DEPENDS+=	windows_i686_gnullvm-0.52.6
CARGO_CRATE_DEPENDS+=	windows_i686_msvc-0.52.6
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnu-0.52.6
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnullvm-0.52.6
CARGO_CRATE_DEPENDS+=	windows_x86_64_msvc-0.52.6
CARGO_CRATE_DEPENDS+=	winnow-0.7.1
CARGO_CRATE_DEPENDS+=	wit-bindgen-rt-0.33.0
CARGO_CRATE_DEPENDS+=	write-fonts-0.48.1
CARGO_CRATE_DEPENDS+=	write16-1.0.0
CARGO_CRATE_DEPENDS+=	writeable-0.6.3
CARGO_CRATE_DEPENDS+=	wyz-0.5.1
CARGO_CRATE_DEPENDS+=	xattr-1.4.0
CARGO_CRATE_DEPENDS+=	xmlwriter-0.1.0
CARGO_CRATE_DEPENDS+=	xmp-writer-0.3.3
CARGO_CRATE_DEPENDS+=	xz2-0.1.7
CARGO_CRATE_DEPENDS+=	yaml-rust-0.4.5
CARGO_CRATE_DEPENDS+=	yoke-0.8.2
CARGO_CRATE_DEPENDS+=	yoke-derive-0.8.2
CARGO_CRATE_DEPENDS+=	zerocopy-0.7.35
CARGO_CRATE_DEPENDS+=	zerocopy-derive-0.7.35
CARGO_CRATE_DEPENDS+=	zerofrom-0.1.8
CARGO_CRATE_DEPENDS+=	zerofrom-derive-0.1.7
CARGO_CRATE_DEPENDS+=	zerotrie-0.2.4
CARGO_CRATE_DEPENDS+=	zerovec-0.11.6
CARGO_CRATE_DEPENDS+=	zerovec-derive-0.11.3
CARGO_CRATE_DEPENDS+=	zip-8.0.0
CARGO_CRATE_DEPENDS+=	zlib-rs-0.5.1
CARGO_CRATE_DEPENDS+=	zopfli-0.8.3
CARGO_CRATE_DEPENDS+=	zune-core-0.5.1
CARGO_CRATE_DEPENDS+=	zune-jpeg-0.5.15
