# $NetBSD: cargo-depends.mk,v 1.13 2025/04/01 10:43:27 adam Exp $

CARGO_CRATE_DEPENDS+=	archery-1.2.1
CARGO_CRATE_DEPENDS+=	autocfg-1.3.0
CARGO_CRATE_DEPENDS+=	cc-1.0.90
CARGO_CRATE_DEPENDS+=	cfg-if-1.0.0
CARGO_CRATE_DEPENDS+=	heck-0.5.0
CARGO_CRATE_DEPENDS+=	indoc-2.0.5
CARGO_CRATE_DEPENDS+=	libc-0.2.155
CARGO_CRATE_DEPENDS+=	memoffset-0.9.1
CARGO_CRATE_DEPENDS+=	once_cell-1.19.0
CARGO_CRATE_DEPENDS+=	portable-atomic-1.6.0
CARGO_CRATE_DEPENDS+=	proc-macro2-1.0.86
CARGO_CRATE_DEPENDS+=	pyo3-0.24.0
CARGO_CRATE_DEPENDS+=	pyo3-build-config-0.24.0
CARGO_CRATE_DEPENDS+=	pyo3-ffi-0.24.0
CARGO_CRATE_DEPENDS+=	pyo3-macros-0.24.0
CARGO_CRATE_DEPENDS+=	pyo3-macros-backend-0.24.0
CARGO_CRATE_DEPENDS+=	python3-dll-a-0.2.12
CARGO_CRATE_DEPENDS+=	quote-1.0.36
CARGO_CRATE_DEPENDS+=	rpds-1.1.0
CARGO_CRATE_DEPENDS+=	syn-2.0.69
CARGO_CRATE_DEPENDS+=	target-lexicon-0.13.2
CARGO_CRATE_DEPENDS+=	triomphe-0.1.13
CARGO_CRATE_DEPENDS+=	unicode-ident-1.0.12
CARGO_CRATE_DEPENDS+=	unindent-0.2.3
