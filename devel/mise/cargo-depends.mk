# $NetBSD: cargo-depends.mk,v 1.117 2026/09/25 08:55:34 pin Exp $

CARGO_CRATE_DEPENDS+=	addr2line-0.25.1
CARGO_CRATE_DEPENDS+=	adler2-2.0.1
CARGO_CRATE_DEPENDS+=	aead-0.5.2
CARGO_CRATE_DEPENDS+=	aead-0.6.1
CARGO_CRATE_DEPENDS+=	aes-0.8.4
CARGO_CRATE_DEPENDS+=	aes-0.9.3
CARGO_CRATE_DEPENDS+=	aes-gcm-0.10.3
CARGO_CRATE_DEPENDS+=	aes-kw-0.2.1
CARGO_CRATE_DEPENDS+=	age-0.11.5
CARGO_CRATE_DEPENDS+=	age-0.12.1
CARGO_CRATE_DEPENDS+=	age-core-0.11.0
CARGO_CRATE_DEPENDS+=	age-core-0.12.0
CARGO_CRATE_DEPENDS+=	ahash-0.8.12
CARGO_CRATE_DEPENDS+=	aho-corasick-1.1.5
CARGO_CRATE_DEPENDS+=	alloc-no-stdlib-2.0.4
CARGO_CRATE_DEPENDS+=	alloc-stdlib-0.2.4
CARGO_CRATE_DEPENDS+=	allocator-api2-0.2.21
CARGO_CRATE_DEPENDS+=	ambient-id-0.0.11
CARGO_CRATE_DEPENDS+=	android_system_properties-0.1.6
CARGO_CRATE_DEPENDS+=	ansi-str-0.9.0
CARGO_CRATE_DEPENDS+=	ansitok-0.3.0
CARGO_CRATE_DEPENDS+=	anstream-1.0.0
CARGO_CRATE_DEPENDS+=	anstyle-1.0.14
CARGO_CRATE_DEPENDS+=	anstyle-parse-1.0.0
CARGO_CRATE_DEPENDS+=	anstyle-query-1.1.5
CARGO_CRATE_DEPENDS+=	anstyle-wincon-3.0.11
CARGO_CRATE_DEPENDS+=	anyhow-1.0.104
CARGO_CRATE_DEPENDS+=	arc-swap-1.9.2
CARGO_CRATE_DEPENDS+=	archspec-0.2.0
CARGO_CRATE_DEPENDS+=	argon2-0.5.3
CARGO_CRATE_DEPENDS+=	arrayvec-0.7.8
CARGO_CRATE_DEPENDS+=	assert-json-diff-2.0.2
CARGO_CRATE_DEPENDS+=	astral-reqwest-middleware-0.5.1
CARGO_CRATE_DEPENDS+=	astral-tokio-tar-0.6.4
CARGO_CRATE_DEPENDS+=	astral_async_http_range_reader-0.11.1
CARGO_CRATE_DEPENDS+=	astral_async_zip-0.0.21
CARGO_CRATE_DEPENDS+=	async-backtrace-0.2.7
CARGO_CRATE_DEPENDS+=	async-backtrace-attributes-0.2.7
CARGO_CRATE_DEPENDS+=	async-compression-0.4.48
CARGO_CRATE_DEPENDS+=	async-fd-lock-0.2.0
CARGO_CRATE_DEPENDS+=	async-once-cell-0.5.4
CARGO_CRATE_DEPENDS+=	async-trait-0.1.92
CARGO_CRATE_DEPENDS+=	atomic-waker-1.1.2
CARGO_CRATE_DEPENDS+=	aube-2.3.0
CARGO_CRATE_DEPENDS+=	aube-codes-2.3.0
CARGO_CRATE_DEPENDS+=	aube-linker-2.3.0
CARGO_CRATE_DEPENDS+=	aube-lockfile-2.3.0
CARGO_CRATE_DEPENDS+=	aube-manifest-2.3.0
CARGO_CRATE_DEPENDS+=	aube-registry-2.3.0
CARGO_CRATE_DEPENDS+=	aube-resolver-2.3.0
CARGO_CRATE_DEPENDS+=	aube-runtime-2.3.0
CARGO_CRATE_DEPENDS+=	aube-scripts-2.3.0
CARGO_CRATE_DEPENDS+=	aube-settings-2.3.0
CARGO_CRATE_DEPENDS+=	aube-store-2.3.0
CARGO_CRATE_DEPENDS+=	aube-util-2.3.0
CARGO_CRATE_DEPENDS+=	aube-workspace-2.3.0
CARGO_CRATE_DEPENDS+=	autocfg-1.5.1
CARGO_CRATE_DEPENDS+=	aws-config-1.12.0
CARGO_CRATE_DEPENDS+=	aws-credential-types-1.3.0
CARGO_CRATE_DEPENDS+=	aws-lc-fips-sys-0.14.2
CARGO_CRATE_DEPENDS+=	aws-lc-rs-1.18.1
CARGO_CRATE_DEPENDS+=	aws-lc-sys-0.45.0
CARGO_CRATE_DEPENDS+=	aws-runtime-1.9.2
CARGO_CRATE_DEPENDS+=	aws-sdk-s3-1.146.1
CARGO_CRATE_DEPENDS+=	aws-sdk-sso-1.109.0
CARGO_CRATE_DEPENDS+=	aws-sdk-ssooidc-1.111.0
CARGO_CRATE_DEPENDS+=	aws-sdk-sts-1.114.0
CARGO_CRATE_DEPENDS+=	aws-sigv4-1.5.1
CARGO_CRATE_DEPENDS+=	aws-smithy-async-1.3.0
CARGO_CRATE_DEPENDS+=	aws-smithy-checksums-0.65.0
CARGO_CRATE_DEPENDS+=	aws-smithy-eventstream-0.61.2
CARGO_CRATE_DEPENDS+=	aws-smithy-http-0.64.0
CARGO_CRATE_DEPENDS+=	aws-smithy-http-client-1.4.0
CARGO_CRATE_DEPENDS+=	aws-smithy-json-0.63.0
CARGO_CRATE_DEPENDS+=	aws-smithy-observability-0.3.0
CARGO_CRATE_DEPENDS+=	aws-smithy-query-0.62.0
CARGO_CRATE_DEPENDS+=	aws-smithy-runtime-1.14.0
CARGO_CRATE_DEPENDS+=	aws-smithy-runtime-api-1.16.0
CARGO_CRATE_DEPENDS+=	aws-smithy-runtime-api-macros-1.1.0
CARGO_CRATE_DEPENDS+=	aws-smithy-schema-0.2.0
CARGO_CRATE_DEPENDS+=	aws-smithy-types-1.6.3
CARGO_CRATE_DEPENDS+=	aws-smithy-xml-0.62.0
CARGO_CRATE_DEPENDS+=	aws-types-1.6.0
CARGO_CRATE_DEPENDS+=	axum-0.8.9
CARGO_CRATE_DEPENDS+=	axum-core-0.5.6
CARGO_CRATE_DEPENDS+=	backtrace-0.3.76
CARGO_CRATE_DEPENDS+=	backtrace-ext-0.2.1
CARGO_CRATE_DEPENDS+=	base16ct-0.2.0
CARGO_CRATE_DEPENDS+=	base64-0.21.7
CARGO_CRATE_DEPENDS+=	base64-0.22.1
CARGO_CRATE_DEPENDS+=	base64-0.23.1
CARGO_CRATE_DEPENDS+=	base64-simd-0.8.0
CARGO_CRATE_DEPENDS+=	base64ct-1.8.3
CARGO_CRATE_DEPENDS+=	basic-toml-0.1.10
CARGO_CRATE_DEPENDS+=	bcrypt-pbkdf-0.10.0
CARGO_CRATE_DEPENDS+=	bech32-0.9.1
CARGO_CRATE_DEPENDS+=	bech32-0.11.1
CARGO_CRATE_DEPENDS+=	beef-0.5.2
CARGO_CRATE_DEPENDS+=	bindgen-0.72.1
CARGO_CRATE_DEPENDS+=	binstall-tar-0.4.42
CARGO_CRATE_DEPENDS+=	bit-set-0.8.0
CARGO_CRATE_DEPENDS+=	bit-vec-0.8.0
CARGO_CRATE_DEPENDS+=	bitfields-1.0.3
CARGO_CRATE_DEPENDS+=	bitfields-impl-1.0.3
CARGO_CRATE_DEPENDS+=	bitflags-1.3.2
CARGO_CRATE_DEPENDS+=	bitflags-2.13.2
CARGO_CRATE_DEPENDS+=	bitvec-1.1.1
CARGO_CRATE_DEPENDS+=	blake2-0.10.6
CARGO_CRATE_DEPENDS+=	blake2-0.11.0
CARGO_CRATE_DEPENDS+=	blake3-1.8.7
CARGO_CRATE_DEPENDS+=	block-buffer-0.10.4
CARGO_CRATE_DEPENDS+=	block-buffer-0.12.1
CARGO_CRATE_DEPENDS+=	block-padding-0.3.3
CARGO_CRATE_DEPENDS+=	block-padding-0.4.2
CARGO_CRATE_DEPENDS+=	blowfish-0.9.1
CARGO_CRATE_DEPENDS+=	brotli-8.0.4
CARGO_CRATE_DEPENDS+=	brotli-decompressor-5.0.3
CARGO_CRATE_DEPENDS+=	bs58-0.5.1
CARGO_CRATE_DEPENDS+=	bstr-1.13.1
CARGO_CRATE_DEPENDS+=	buffer-redux-1.1.0
CARGO_CRATE_DEPENDS+=	built-0.8.1
CARGO_CRATE_DEPENDS+=	bumpalo-3.20.3
CARGO_CRATE_DEPENDS+=	bytecheck-0.8.3
CARGO_CRATE_DEPENDS+=	bytecheck_derive-0.8.3
CARGO_CRATE_DEPENDS+=	bytecount-0.6.9
CARGO_CRATE_DEPENDS+=	byteorder-1.5.0
CARGO_CRATE_DEPENDS+=	bytes-1.12.1
CARGO_CRATE_DEPENDS+=	bytes-utils-0.1.4
CARGO_CRATE_DEPENDS+=	bytesize-2.7.0
CARGO_CRATE_DEPENDS+=	bzip2-0.6.1
CARGO_CRATE_DEPENDS+=	cache_control-0.2.0
CARGO_CRATE_DEPENDS+=	calendrical_calculations-0.2.4
CARGO_CRATE_DEPENDS+=	calm_io-0.1.1
CARGO_CRATE_DEPENDS+=	calmio_filters-0.1.0
CARGO_CRATE_DEPENDS+=	camellia-0.1.0
CARGO_CRATE_DEPENDS+=	cast5-0.11.1
CARGO_CRATE_DEPENDS+=	cbc-0.1.2
CARGO_CRATE_DEPENDS+=	cbc-0.2.1
CARGO_CRATE_DEPENDS+=	cc-1.4.7
CARGO_CRATE_DEPENDS+=	cexpr-0.6.0
CARGO_CRATE_DEPENDS+=	cfb-mode-0.8.2
CARGO_CRATE_DEPENDS+=	cfg-if-1.0.5
CARGO_CRATE_DEPENDS+=	cfg_aliases-0.2.2
CARGO_CRATE_DEPENDS+=	chacha20-0.9.1
CARGO_CRATE_DEPENDS+=	chacha20-0.10.2
CARGO_CRATE_DEPENDS+=	chacha20poly1305-0.10.1
CARGO_CRATE_DEPENDS+=	chacha20poly1305-0.11.0
CARGO_CRATE_DEPENDS+=	chrono-0.4.45
CARGO_CRATE_DEPENDS+=	chrono-tz-0.9.0
CARGO_CRATE_DEPENDS+=	chrono-tz-0.10.4
CARGO_CRATE_DEPENDS+=	chrono-tz-build-0.3.0
CARGO_CRATE_DEPENDS+=	ci_info-0.14.15
CARGO_CRATE_DEPENDS+=	cipher-0.4.4
CARGO_CRATE_DEPENDS+=	cipher-0.5.2
CARGO_CRATE_DEPENDS+=	clang-sys-1.9.1
CARGO_CRATE_DEPENDS+=	clru-0.6.3
CARGO_CRATE_DEPENDS+=	clx-3.0.2
CARGO_CRATE_DEPENDS+=	cmac-0.7.2
CARGO_CRATE_DEPENDS+=	cmake-0.1.58
CARGO_CRATE_DEPENDS+=	cmov-0.5.4
CARGO_CRATE_DEPENDS+=	cmpv2-0.2.0
CARGO_CRATE_DEPENDS+=	cms-0.2.3
CARGO_CRATE_DEPENDS+=	coalesced_map-0.1.4
CARGO_CRATE_DEPENDS+=	color-eyre-0.6.5
CARGO_CRATE_DEPENDS+=	color-print-0.3.7
CARGO_CRATE_DEPENDS+=	color-print-proc-macro-0.3.7
CARGO_CRATE_DEPENDS+=	colorchoice-1.0.5
CARGO_CRATE_DEPENDS+=	colored-3.1.1
CARGO_CRATE_DEPENDS+=	combine-4.6.8
CARGO_CRATE_DEPENDS+=	comfy-table-8.0.0
CARGO_CRATE_DEPENDS+=	compression-codecs-0.4.43
CARGO_CRATE_DEPENDS+=	compression-core-0.4.33
CARGO_CRATE_DEPENDS+=	configparser-3.3.0
CARGO_CRATE_DEPENDS+=	confique-0.4.0
CARGO_CRATE_DEPENDS+=	confique-macro-0.0.13
CARGO_CRATE_DEPENDS+=	console-0.16.6
CARGO_CRATE_DEPENDS+=	const-hex-1.19.3
CARGO_CRATE_DEPENDS+=	const-oid-0.9.6
CARGO_CRATE_DEPENDS+=	const-oid-0.10.2
CARGO_CRATE_DEPENDS+=	constant_time_eq-0.4.2
CARGO_CRATE_DEPENDS+=	contracts-0.6.10
CARGO_CRATE_DEPENDS+=	convert_case-0.10.0
CARGO_CRATE_DEPENDS+=	cookie-0.18.2
CARGO_CRATE_DEPENDS+=	cookie-factory-0.3.3
CARGO_CRATE_DEPENDS+=	cookie_store-0.22.1
CARGO_CRATE_DEPENDS+=	core-foundation-0.9.4
CARGO_CRATE_DEPENDS+=	core-foundation-0.10.1
CARGO_CRATE_DEPENDS+=	core-foundation-sys-0.8.7
CARGO_CRATE_DEPENDS+=	core_maths-0.1.1
CARGO_CRATE_DEPENDS+=	countme-3.0.1
CARGO_CRATE_DEPENDS+=	cpubits-0.1.1
CARGO_CRATE_DEPENDS+=	cpufeatures-0.2.17
CARGO_CRATE_DEPENDS+=	cpufeatures-0.3.1
CARGO_CRATE_DEPENDS+=	crc-fast-1.10.0
CARGO_CRATE_DEPENDS+=	crc24-0.1.6
CARGO_CRATE_DEPENDS+=	crc32fast-1.5.2
CARGO_CRATE_DEPENDS+=	crmf-0.2.0
CARGO_CRATE_DEPENDS+=	crossbeam-channel-0.5.17
CARGO_CRATE_DEPENDS+=	crossbeam-deque-0.8.8
CARGO_CRATE_DEPENDS+=	crossbeam-epoch-0.9.21
CARGO_CRATE_DEPENDS+=	crossbeam-utils-0.8.23
CARGO_CRATE_DEPENDS+=	crossterm-0.29.0
CARGO_CRATE_DEPENDS+=	crossterm_winapi-0.9.1
CARGO_CRATE_DEPENDS+=	crypto-bigint-0.5.5
CARGO_CRATE_DEPENDS+=	crypto-common-0.1.7
CARGO_CRATE_DEPENDS+=	crypto-common-0.2.2
CARGO_CRATE_DEPENDS+=	ctor-1.0.13
CARGO_CRATE_DEPENDS+=	ctr-0.9.2
CARGO_CRATE_DEPENDS+=	ctutils-0.4.2
CARGO_CRATE_DEPENDS+=	curve25519-dalek-4.1.3
CARGO_CRATE_DEPENDS+=	curve25519-dalek-5.0.0
CARGO_CRATE_DEPENDS+=	curve25519-dalek-derive-0.1.1
CARGO_CRATE_DEPENDS+=	cx448-0.1.1
CARGO_CRATE_DEPENDS+=	darling-0.20.11
CARGO_CRATE_DEPENDS+=	darling-0.23.0
CARGO_CRATE_DEPENDS+=	darling-0.24.1
CARGO_CRATE_DEPENDS+=	darling_core-0.20.11
CARGO_CRATE_DEPENDS+=	darling_core-0.23.0
CARGO_CRATE_DEPENDS+=	darling_core-0.24.1
CARGO_CRATE_DEPENDS+=	darling_macro-0.20.11
CARGO_CRATE_DEPENDS+=	darling_macro-0.23.0
CARGO_CRATE_DEPENDS+=	darling_macro-0.24.1
CARGO_CRATE_DEPENDS+=	dashmap-5.5.3
CARGO_CRATE_DEPENDS+=	dashmap-6.2.1
CARGO_CRATE_DEPENDS+=	dbl-0.3.2
CARGO_CRATE_DEPENDS+=	deadpool-0.12.3
CARGO_CRATE_DEPENDS+=	deadpool-runtime-0.1.4
CARGO_CRATE_DEPENDS+=	decmpfs-0.1.0
CARGO_CRATE_DEPENDS+=	deflate64-0.1.12
CARGO_CRATE_DEPENDS+=	defmt-1.1.1
CARGO_CRATE_DEPENDS+=	defmt-macros-1.1.1
CARGO_CRATE_DEPENDS+=	defmt-parser-1.0.0
CARGO_CRATE_DEPENDS+=	demand-2.1.0
CARGO_CRATE_DEPENDS+=	der-0.7.10
CARGO_CRATE_DEPENDS+=	der-0.8.2
CARGO_CRATE_DEPENDS+=	der_derive-0.7.3
CARGO_CRATE_DEPENDS+=	der_derive-0.8.0
CARGO_CRATE_DEPENDS+=	deranged-0.5.8
CARGO_CRATE_DEPENDS+=	derive_builder-0.20.2
CARGO_CRATE_DEPENDS+=	derive_builder_core-0.20.2
CARGO_CRATE_DEPENDS+=	derive_builder_macro-0.20.2
CARGO_CRATE_DEPENDS+=	derive_more-2.1.1
CARGO_CRATE_DEPENDS+=	derive_more-impl-2.1.1
CARGO_CRATE_DEPENDS+=	des-0.8.1
CARGO_CRATE_DEPENDS+=	deunicode-1.6.2
CARGO_CRATE_DEPENDS+=	diff-0.1.13
CARGO_CRATE_DEPENDS+=	diffy-0.5.2
CARGO_CRATE_DEPENDS+=	digest-0.10.7
CARGO_CRATE_DEPENDS+=	digest-0.11.3
CARGO_CRATE_DEPENDS+=	directories-6.0.0
CARGO_CRATE_DEPENDS+=	dirs-6.0.0
CARGO_CRATE_DEPENDS+=	dirs-sys-0.5.0
CARGO_CRATE_DEPENDS+=	displaydoc-0.2.7
CARGO_CRATE_DEPENDS+=	document-features-0.2.12
CARGO_CRATE_DEPENDS+=	dotenvy-0.15.7
CARGO_CRATE_DEPENDS+=	dsa-0.6.3
CARGO_CRATE_DEPENDS+=	duct-1.1.2
CARGO_CRATE_DEPENDS+=	dunce-1.0.5
CARGO_CRATE_DEPENDS+=	dyn-clone-1.0.20
CARGO_CRATE_DEPENDS+=	eax-0.5.0
CARGO_CRATE_DEPENDS+=	ecdsa-0.16.9
CARGO_CRATE_DEPENDS+=	ed25519-2.2.3
CARGO_CRATE_DEPENDS+=	ed25519-3.0.0
CARGO_CRATE_DEPENDS+=	ed25519-dalek-2.2.0
CARGO_CRATE_DEPENDS+=	ed25519-dalek-3.0.0
CARGO_CRATE_DEPENDS+=	either-1.18.0
CARGO_CRATE_DEPENDS+=	elliptic-curve-0.13.8
CARGO_CRATE_DEPENDS+=	elsa-1.11.2
CARGO_CRATE_DEPENDS+=	encode_unicode-1.0.0
CARGO_CRATE_DEPENDS+=	encoding_rs-0.8.35
CARGO_CRATE_DEPENDS+=	enum-as-inner-0.6.1
CARGO_CRATE_DEPENDS+=	enum_dispatch-0.3.13
CARGO_CRATE_DEPENDS+=	enumflags2-0.7.12
CARGO_CRATE_DEPENDS+=	enumflags2_derive-0.7.12
CARGO_CRATE_DEPENDS+=	env_filter-2.0.0
CARGO_CRATE_DEPENDS+=	env_logger-0.11.11
CARGO_CRATE_DEPENDS+=	envmnt-0.10.4
CARGO_CRATE_DEPENDS+=	equivalent-1.0.2
CARGO_CRATE_DEPENDS+=	erased-serde-0.4.10
CARGO_CRATE_DEPENDS+=	errno-0.2.8
CARGO_CRATE_DEPENDS+=	errno-0.3.14
CARGO_CRATE_DEPENDS+=	errno-dragonfly-0.1.2
CARGO_CRATE_DEPENDS+=	event-listener-5.4.2
CARGO_CRATE_DEPENDS+=	exec-0.3.1
CARGO_CRATE_DEPENDS+=	expr-lang-2.1.0
CARGO_CRATE_DEPENDS+=	eyre-0.6.14
CARGO_CRATE_DEPENDS+=	fancy-regex-0.18.0
CARGO_CRATE_DEPENDS+=	faster-hex-0.10.0
CARGO_CRATE_DEPENDS+=	fastrand-2.5.0
CARGO_CRATE_DEPENDS+=	faststr-0.2.34
CARGO_CRATE_DEPENDS+=	ff-0.13.1
CARGO_CRATE_DEPENDS+=	fiat-crypto-0.2.9
CARGO_CRATE_DEPENDS+=	fiat-crypto-0.3.0
CARGO_CRATE_DEPENDS+=	file-id-0.2.3
CARGO_CRATE_DEPENDS+=	file_url-0.3.2
CARGO_CRATE_DEPENDS+=	filetime-0.2.29
CARGO_CRATE_DEPENDS+=	find-crate-0.6.3
CARGO_CRATE_DEPENDS+=	find-msvc-tools-0.1.13
CARGO_CRATE_DEPENDS+=	fixed_decimal-0.7.2
CARGO_CRATE_DEPENDS+=	fixedbitset-0.5.7
CARGO_CRATE_DEPENDS+=	flagset-0.4.7
CARGO_CRATE_DEPENDS+=	flate2-1.1.10
CARGO_CRATE_DEPENDS+=	float-cmp-0.10.0
CARGO_CRATE_DEPENDS+=	fluent-0.16.1
CARGO_CRATE_DEPENDS+=	fluent-0.17.0
CARGO_CRATE_DEPENDS+=	fluent-bundle-0.15.3
CARGO_CRATE_DEPENDS+=	fluent-bundle-0.16.0
CARGO_CRATE_DEPENDS+=	fluent-langneg-0.13.1
CARGO_CRATE_DEPENDS+=	fluent-syntax-0.11.1
CARGO_CRATE_DEPENDS+=	fluent-syntax-0.12.0
CARGO_CRATE_DEPENDS+=	fnv-1.0.7
CARGO_CRATE_DEPENDS+=	foldhash-0.1.5
CARGO_CRATE_DEPENDS+=	foldhash-0.2.0
CARGO_CRATE_DEPENDS+=	foreign-types-0.3.2
CARGO_CRATE_DEPENDS+=	foreign-types-shared-0.1.1
CARGO_CRATE_DEPENDS+=	form_urlencoded-1.2.2
CARGO_CRATE_DEPENDS+=	formatx-0.4.2
CARGO_CRATE_DEPENDS+=	fs-err-3.3.1
CARGO_CRATE_DEPENDS+=	fs4-1.1.0
CARGO_CRATE_DEPENDS+=	fs_extra-1.3.0
CARGO_CRATE_DEPENDS+=	fsevent-sys-4.1.0
CARGO_CRATE_DEPENDS+=	fsio-0.4.1
CARGO_CRATE_DEPENDS+=	fslock-0.2.1
CARGO_CRATE_DEPENDS+=	funty-2.0.0
CARGO_CRATE_DEPENDS+=	futures-0.3.34
CARGO_CRATE_DEPENDS+=	futures-channel-0.3.34
CARGO_CRATE_DEPENDS+=	futures-core-0.3.34
CARGO_CRATE_DEPENDS+=	futures-executor-0.3.34
CARGO_CRATE_DEPENDS+=	futures-io-0.3.34
CARGO_CRATE_DEPENDS+=	futures-lite-2.6.1
CARGO_CRATE_DEPENDS+=	futures-macro-0.3.34
CARGO_CRATE_DEPENDS+=	futures-sink-0.3.34
CARGO_CRATE_DEPENDS+=	futures-task-0.3.34
CARGO_CRATE_DEPENDS+=	futures-util-0.3.34
CARGO_CRATE_DEPENDS+=	fuzzy-matcher-0.3.7
CARGO_CRATE_DEPENDS+=	generator-0.7.5
CARGO_CRATE_DEPENDS+=	generic-array-0.14.7
CARGO_CRATE_DEPENDS+=	getrandom-0.2.17
CARGO_CRATE_DEPENDS+=	getrandom-0.3.4
CARGO_CRATE_DEPENDS+=	getrandom-0.4.3
CARGO_CRATE_DEPENDS+=	ghash-0.5.1
CARGO_CRATE_DEPENDS+=	gimli-0.32.3
CARGO_CRATE_DEPENDS+=	gix-0.87.1
CARGO_CRATE_DEPENDS+=	gix-actor-0.42.0
CARGO_CRATE_DEPENDS+=	gix-attributes-0.35.0
CARGO_CRATE_DEPENDS+=	gix-bitmap-0.4.0
CARGO_CRATE_DEPENDS+=	gix-chunk-0.8.0
CARGO_CRATE_DEPENDS+=	gix-command-0.10.1
CARGO_CRATE_DEPENDS+=	gix-commitgraph-0.39.0
CARGO_CRATE_DEPENDS+=	gix-config-0.60.0
CARGO_CRATE_DEPENDS+=	gix-config-value-0.19.1
CARGO_CRATE_DEPENDS+=	gix-credentials-0.40.0
CARGO_CRATE_DEPENDS+=	gix-date-0.16.0
CARGO_CRATE_DEPENDS+=	gix-diff-0.67.1
CARGO_CRATE_DEPENDS+=	gix-discover-0.55.0
CARGO_CRATE_DEPENDS+=	gix-error-0.3.2
CARGO_CRATE_DEPENDS+=	gix-features-0.49.1
CARGO_CRATE_DEPENDS+=	gix-filter-0.34.0
CARGO_CRATE_DEPENDS+=	gix-fs-0.22.1
CARGO_CRATE_DEPENDS+=	gix-glob-0.27.1
CARGO_CRATE_DEPENDS+=	gix-hash-0.26.2
CARGO_CRATE_DEPENDS+=	gix-hashtable-0.16.0
CARGO_CRATE_DEPENDS+=	gix-ignore-0.22.1
CARGO_CRATE_DEPENDS+=	gix-index-0.55.0
CARGO_CRATE_DEPENDS+=	gix-lock-24.0.0
CARGO_CRATE_DEPENDS+=	gix-macros-0.1.6
CARGO_CRATE_DEPENDS+=	gix-negotiate-0.35.1
CARGO_CRATE_DEPENDS+=	gix-note-0.1.1
CARGO_CRATE_DEPENDS+=	gix-object-0.64.1
CARGO_CRATE_DEPENDS+=	gix-odb-0.84.0
CARGO_CRATE_DEPENDS+=	gix-pack-0.74.2
CARGO_CRATE_DEPENDS+=	gix-packetline-0.22.2
CARGO_CRATE_DEPENDS+=	gix-path-0.12.6
CARGO_CRATE_DEPENDS+=	gix-pathspec-0.20.0
CARGO_CRATE_DEPENDS+=	gix-prompt-0.17.0
CARGO_CRATE_DEPENDS+=	gix-protocol-0.65.1
CARGO_CRATE_DEPENDS+=	gix-quote-0.8.0
CARGO_CRATE_DEPENDS+=	gix-ref-0.67.1
CARGO_CRATE_DEPENDS+=	gix-refspec-0.45.1
CARGO_CRATE_DEPENDS+=	gix-revision-0.49.1
CARGO_CRATE_DEPENDS+=	gix-revwalk-0.35.0
CARGO_CRATE_DEPENDS+=	gix-sec-0.14.2
CARGO_CRATE_DEPENDS+=	gix-shallow-0.13.0
CARGO_CRATE_DEPENDS+=	gix-submodule-0.34.0
CARGO_CRATE_DEPENDS+=	gix-tempfile-24.0.0
CARGO_CRATE_DEPENDS+=	gix-trace-0.1.21
CARGO_CRATE_DEPENDS+=	gix-transport-0.59.2
CARGO_CRATE_DEPENDS+=	gix-traverse-0.61.0
CARGO_CRATE_DEPENDS+=	gix-url-0.38.0
CARGO_CRATE_DEPENDS+=	gix-utils-0.3.6
CARGO_CRATE_DEPENDS+=	gix-validate-0.11.4
CARGO_CRATE_DEPENDS+=	gix-worktree-0.56.0
CARGO_CRATE_DEPENDS+=	gix-worktree-state-0.34.1
CARGO_CRATE_DEPENDS+=	gix-worktree-stream-0.36.1
CARGO_CRATE_DEPENDS+=	gix-zlib-0.1.0
CARGO_CRATE_DEPENDS+=	glob-0.3.4
CARGO_CRATE_DEPENDS+=	globset-0.4.20
CARGO_CRATE_DEPENDS+=	globwalk-0.9.1
CARGO_CRATE_DEPENDS+=	group-0.13.0
CARGO_CRATE_DEPENDS+=	h2-0.4.19
CARGO_CRATE_DEPENDS+=	halfbrown-0.4.0
CARGO_CRATE_DEPENDS+=	hash32-0.3.1
CARGO_CRATE_DEPENDS+=	hashbrown-0.12.3
CARGO_CRATE_DEPENDS+=	hashbrown-0.14.5
CARGO_CRATE_DEPENDS+=	hashbrown-0.15.5
CARGO_CRATE_DEPENDS+=	hashbrown-0.16.1
CARGO_CRATE_DEPENDS+=	hashbrown-0.17.1
CARGO_CRATE_DEPENDS+=	heapless-0.8.0
CARGO_CRATE_DEPENDS+=	heck-0.5.0
CARGO_CRATE_DEPENDS+=	hermit-abi-0.5.3
CARGO_CRATE_DEPENDS+=	hex-0.4.3
CARGO_CRATE_DEPENDS+=	hkdf-0.12.4
CARGO_CRATE_DEPENDS+=	hmac-0.12.1
CARGO_CRATE_DEPENDS+=	hmac-0.13.0
CARGO_CRATE_DEPENDS+=	homedir-0.3.6
CARGO_CRATE_DEPENDS+=	hpke-0.12.0
CARGO_CRATE_DEPENDS+=	http-0.2.12
CARGO_CRATE_DEPENDS+=	http-1.5.0
CARGO_CRATE_DEPENDS+=	http-auth-0.1.10
CARGO_CRATE_DEPENDS+=	http-body-0.4.6
CARGO_CRATE_DEPENDS+=	http-body-1.1.0
CARGO_CRATE_DEPENDS+=	http-body-util-0.1.5
CARGO_CRATE_DEPENDS+=	http-cache-semantics-3.0.0
CARGO_CRATE_DEPENDS+=	http-content-range-0.2.5
CARGO_CRATE_DEPENDS+=	http-serde-2.1.1
CARGO_CRATE_DEPENDS+=	httparse-1.10.1
CARGO_CRATE_DEPENDS+=	httpdate-1.0.3
CARGO_CRATE_DEPENDS+=	humansize-2.1.3
CARGO_CRATE_DEPENDS+=	humantime-2.4.0
CARGO_CRATE_DEPENDS+=	hybrid-array-0.2.3
CARGO_CRATE_DEPENDS+=	hybrid-array-0.4.15
CARGO_CRATE_DEPENDS+=	hyper-1.11.1
CARGO_CRATE_DEPENDS+=	hyper-rustls-0.27.10
CARGO_CRATE_DEPENDS+=	hyper-tls-0.6.0
CARGO_CRATE_DEPENDS+=	hyper-util-0.1.20
CARGO_CRATE_DEPENDS+=	i18n-config-0.4.8
CARGO_CRATE_DEPENDS+=	i18n-embed-0.15.4
CARGO_CRATE_DEPENDS+=	i18n-embed-0.16.0
CARGO_CRATE_DEPENDS+=	i18n-embed-fl-0.9.4
CARGO_CRATE_DEPENDS+=	i18n-embed-fl-0.10.1
CARGO_CRATE_DEPENDS+=	i18n-embed-impl-0.8.4
CARGO_CRATE_DEPENDS+=	iana-time-zone-0.1.65
CARGO_CRATE_DEPENDS+=	iana-time-zone-haiku-0.1.2
CARGO_CRATE_DEPENDS+=	icu_calendar-2.3.0
CARGO_CRATE_DEPENDS+=	icu_calendar_data-2.3.0
CARGO_CRATE_DEPENDS+=	icu_collections-2.3.0
CARGO_CRATE_DEPENDS+=	icu_datetime-2.3.0
CARGO_CRATE_DEPENDS+=	icu_datetime_data-2.3.0
CARGO_CRATE_DEPENDS+=	icu_decimal-2.3.0
CARGO_CRATE_DEPENDS+=	icu_decimal_data-2.3.0
CARGO_CRATE_DEPENDS+=	icu_locale-2.3.1
CARGO_CRATE_DEPENDS+=	icu_locale_core-2.3.0
CARGO_CRATE_DEPENDS+=	icu_locale_data-2.3.0
CARGO_CRATE_DEPENDS+=	icu_locale_fallback-2.3.0
CARGO_CRATE_DEPENDS+=	icu_locale_fallback_data-2.3.0
CARGO_CRATE_DEPENDS+=	icu_normalizer-2.3.0
CARGO_CRATE_DEPENDS+=	icu_normalizer_data-2.3.0
CARGO_CRATE_DEPENDS+=	icu_pattern-0.5.0
CARGO_CRATE_DEPENDS+=	icu_plurals-2.3.0
CARGO_CRATE_DEPENDS+=	icu_plurals_data-2.3.0
CARGO_CRATE_DEPENDS+=	icu_properties-2.3.0
CARGO_CRATE_DEPENDS+=	icu_properties_data-2.3.0
CARGO_CRATE_DEPENDS+=	icu_provider-2.3.1
CARGO_CRATE_DEPENDS+=	icu_time-2.3.0
CARGO_CRATE_DEPENDS+=	icu_time_data-2.3.0
CARGO_CRATE_DEPENDS+=	idea-0.5.1
CARGO_CRATE_DEPENDS+=	ident_case-1.0.1
CARGO_CRATE_DEPENDS+=	idna-1.1.0
CARGO_CRATE_DEPENDS+=	idna_adapter-1.2.2
CARGO_CRATE_DEPENDS+=	ignore-0.4.33
CARGO_CRATE_DEPENDS+=	impl-tools-0.11.4
CARGO_CRATE_DEPENDS+=	impl-tools-lib-0.11.4
CARGO_CRATE_DEPENDS+=	indenter-0.3.4
CARGO_CRATE_DEPENDS+=	indexmap-1.9.3
CARGO_CRATE_DEPENDS+=	indexmap-2.14.2
CARGO_CRATE_DEPENDS+=	indicatif-0.18.6
CARGO_CRATE_DEPENDS+=	indoc-2.0.7
CARGO_CRATE_DEPENDS+=	inotify-0.11.5
CARGO_CRATE_DEPENDS+=	inotify-sys-0.1.8
CARGO_CRATE_DEPENDS+=	inout-0.1.4
CARGO_CRATE_DEPENDS+=	inout-0.2.2
CARGO_CRATE_DEPENDS+=	insta-1.48.0
CARGO_CRATE_DEPENDS+=	intl-memoizer-0.5.3
CARGO_CRATE_DEPENDS+=	intl_pluralrules-7.0.2
CARGO_CRATE_DEPENDS+=	inventory-0.3.24
CARGO_CRATE_DEPENDS+=	io-close-0.3.7
CARGO_CRATE_DEPENDS+=	io_tee-0.1.1
CARGO_CRATE_DEPENDS+=	ipnet-2.12.2
CARGO_CRATE_DEPENDS+=	is_ci-1.2.0
CARGO_CRATE_DEPENDS+=	is_terminal_polyfill-1.70.2
CARGO_CRATE_DEPENDS+=	itertools-0.10.5
CARGO_CRATE_DEPENDS+=	itertools-0.13.0
CARGO_CRATE_DEPENDS+=	itertools-0.14.0
CARGO_CRATE_DEPENDS+=	itertools-0.15.0
CARGO_CRATE_DEPENDS+=	itoa-1.0.18
CARGO_CRATE_DEPENDS+=	ixdtf-0.6.6
CARGO_CRATE_DEPENDS+=	jdx-tar-1.1.1
CARGO_CRATE_DEPENDS+=	jiff-0.2.37
CARGO_CRATE_DEPENDS+=	jiff-core-0.1.1
CARGO_CRATE_DEPENDS+=	jiff-icu-0.2.2
CARGO_CRATE_DEPENDS+=	jiff-static-0.2.37
CARGO_CRATE_DEPENDS+=	jiff-tzdb-0.1.8
CARGO_CRATE_DEPENDS+=	jiff-tzdb-platform-0.1.3
CARGO_CRATE_DEPENDS+=	jni-0.22.4
CARGO_CRATE_DEPENDS+=	jni-macros-0.22.4
CARGO_CRATE_DEPENDS+=	jni-sys-0.4.1
CARGO_CRATE_DEPENDS+=	jni-sys-macros-0.4.1
CARGO_CRATE_DEPENDS+=	jobserver-0.1.35
CARGO_CRATE_DEPENDS+=	js-sys-0.3.105
CARGO_CRATE_DEPENDS+=	json-patch-4.2.0
CARGO_CRATE_DEPENDS+=	jsonptr-0.7.1
CARGO_CRATE_DEPENDS+=	junction-2.0.0
CARGO_CRATE_DEPENDS+=	k256-0.13.4
CARGO_CRATE_DEPENDS+=	keccak-0.1.6
CARGO_CRATE_DEPENDS+=	kem-0.3.0-pre.0
CARGO_CRATE_DEPENDS+=	known-folders-1.4.2
CARGO_CRATE_DEPENDS+=	kqueue-1.2.1
CARGO_CRATE_DEPENDS+=	kqueue-sys-1.1.2
CARGO_CRATE_DEPENDS+=	landlock-0.4.7
CARGO_CRATE_DEPENDS+=	lazy-regex-3.6.1
CARGO_CRATE_DEPENDS+=	lazy-regex-proc_macros-3.6.1
CARGO_CRATE_DEPENDS+=	lazy_static-1.5.0
CARGO_CRATE_DEPENDS+=	libbz2-rs-sys-0.2.5
CARGO_CRATE_DEPENDS+=	libc-0.2.186
CARGO_CRATE_DEPENDS+=	libloading-0.8.9
CARGO_CRATE_DEPENDS+=	libloading-0.9.0
CARGO_CRATE_DEPENDS+=	libm-0.2.16
CARGO_CRATE_DEPENDS+=	libredox-0.1.25
CARGO_CRATE_DEPENDS+=	libyaml-rs-0.3.0
CARGO_CRATE_DEPENDS+=	libz-ng-sys-1.1.29
CARGO_CRATE_DEPENDS+=	link-section-0.19.3
CARGO_CRATE_DEPENDS+=	linktime-proc-macro-0.2.3
CARGO_CRATE_DEPENDS+=	linux-raw-sys-0.4.15
CARGO_CRATE_DEPENDS+=	linux-raw-sys-0.12.1
CARGO_CRATE_DEPENDS+=	litemap-0.8.3
CARGO_CRATE_DEPENDS+=	litrs-1.0.0
CARGO_CRATE_DEPENDS+=	lock_api-0.4.14
CARGO_CRATE_DEPENDS+=	log-0.4.34
CARGO_CRATE_DEPENDS+=	logos-0.12.1
CARGO_CRATE_DEPENDS+=	logos-derive-0.12.1
CARGO_CRATE_DEPENDS+=	loom-0.5.6
CARGO_CRATE_DEPENDS+=	lru-0.18.4
CARGO_CRATE_DEPENDS+=	lru-slab-0.1.3
CARGO_CRATE_DEPENDS+=	lua-src-551.0.2
CARGO_CRATE_DEPENDS+=	luajit-src-210.7.3+1ee778a
CARGO_CRATE_DEPENDS+=	lzma-rust2-0.16.5
CARGO_CRATE_DEPENDS+=	lzma-rust2-0.20.1
CARGO_CRATE_DEPENDS+=	lzma-sys-0.1.20
CARGO_CRATE_DEPENDS+=	matchers-0.2.0
CARGO_CRATE_DEPENDS+=	matchit-0.8.4
CARGO_CRATE_DEPENDS+=	md-5-0.10.6
CARGO_CRATE_DEPENDS+=	md-5-0.11.0
CARGO_CRATE_DEPENDS+=	memchr-2.8.3
CARGO_CRATE_DEPENDS+=	memmap2-0.9.11
CARGO_CRATE_DEPENDS+=	memoffset-0.9.1
CARGO_CRATE_DEPENDS+=	miette-7.6.0
CARGO_CRATE_DEPENDS+=	miette-derive-7.6.0
CARGO_CRATE_DEPENDS+=	mime-0.3.17
CARGO_CRATE_DEPENDS+=	mime_guess-2.0.5
CARGO_CRATE_DEPENDS+=	minimal-lexical-0.2.1
CARGO_CRATE_DEPENDS+=	minisign-verify-0.2.5
CARGO_CRATE_DEPENDS+=	miniz_oxide-0.8.9
CARGO_CRATE_DEPENDS+=	miniz_oxide-0.9.1
CARGO_CRATE_DEPENDS+=	mio-1.2.3
CARGO_CRATE_DEPENDS+=	ml-kem-0.2.3
CARGO_CRATE_DEPENDS+=	mlua-0.12.1
CARGO_CRATE_DEPENDS+=	mlua-sys-0.12.0
CARGO_CRATE_DEPENDS+=	mlua_derive-0.12.1
CARGO_CRATE_DEPENDS+=	mockito-1.7.2
CARGO_CRATE_DEPENDS+=	munge-0.4.7
CARGO_CRATE_DEPENDS+=	munge_macro-0.4.7
CARGO_CRATE_DEPENDS+=	native-tls-0.2.18
CARGO_CRATE_DEPENDS+=	netrc-rs-0.1.2
CARGO_CRATE_DEPENDS+=	nix-0.30.1
CARGO_CRATE_DEPENDS+=	nix-0.31.3
CARGO_CRATE_DEPENDS+=	node-semver-2.2.0
CARGO_CRATE_DEPENDS+=	nodejs-semver-5.0.0
CARGO_CRATE_DEPENDS+=	nom-7.1.3
CARGO_CRATE_DEPENDS+=	nom-8.0.0
CARGO_CRATE_DEPENDS+=	nom-language-0.1.0
CARGO_CRATE_DEPENDS+=	nonempty-0.12.0
CARGO_CRATE_DEPENDS+=	notify-8.2.0
CARGO_CRATE_DEPENDS+=	notify-debouncer-full-0.7.0
CARGO_CRATE_DEPENDS+=	notify-types-2.1.0
CARGO_CRATE_DEPENDS+=	nu-ansi-term-0.50.3
CARGO_CRATE_DEPENDS+=	nucleo-matcher-0.3.1
CARGO_CRATE_DEPENDS+=	num-bigint-dig-0.8.6
CARGO_CRATE_DEPENDS+=	num-conv-0.2.2
CARGO_CRATE_DEPENDS+=	num-integer-0.1.47
CARGO_CRATE_DEPENDS+=	num-iter-0.1.46
CARGO_CRATE_DEPENDS+=	num-traits-0.2.19
CARGO_CRATE_DEPENDS+=	num_cpus-1.17.0
CARGO_CRATE_DEPENDS+=	num_enum-0.7.6
CARGO_CRATE_DEPENDS+=	num_enum_derive-0.7.6
CARGO_CRATE_DEPENDS+=	object-0.37.3
CARGO_CRATE_DEPENDS+=	ocb3-0.1.0
CARGO_CRATE_DEPENDS+=	once_cell-1.21.4
CARGO_CRATE_DEPENDS+=	once_cell_polyfill-1.70.2
CARGO_CRATE_DEPENDS+=	opaque-debug-0.3.1
CARGO_CRATE_DEPENDS+=	openssl-0.10.81
CARGO_CRATE_DEPENDS+=	openssl-macros-0.1.1
CARGO_CRATE_DEPENDS+=	openssl-probe-0.2.1
CARGO_CRATE_DEPENDS+=	openssl-sys-0.9.117
CARGO_CRATE_DEPENDS+=	opentelemetry-0.33.0
CARGO_CRATE_DEPENDS+=	opentelemetry-http-0.33.0
CARGO_CRATE_DEPENDS+=	opentelemetry-otlp-0.33.0
CARGO_CRATE_DEPENDS+=	opentelemetry-proto-0.33.0
CARGO_CRATE_DEPENDS+=	opentelemetry_sdk-0.33.0
CARGO_CRATE_DEPENDS+=	option-ext-0.2.0
CARGO_CRATE_DEPENDS+=	ordered-float-2.10.1
CARGO_CRATE_DEPENDS+=	os_pipe-1.2.3
CARGO_CRATE_DEPENDS+=	outref-0.5.2
CARGO_CRATE_DEPENDS+=	owo-colors-4.4.0
CARGO_CRATE_DEPENDS+=	p256-0.13.2
CARGO_CRATE_DEPENDS+=	p384-0.13.1
CARGO_CRATE_DEPENDS+=	p521-0.13.3
CARGO_CRATE_DEPENDS+=	packslip-1.2.0
CARGO_CRATE_DEPENDS+=	papergrid-0.18.0
CARGO_CRATE_DEPENDS+=	parking-2.2.1
CARGO_CRATE_DEPENDS+=	parking_lot-0.12.5
CARGO_CRATE_DEPENDS+=	parking_lot_core-0.9.12
CARGO_CRATE_DEPENDS+=	parse-zoneinfo-0.3.1
CARGO_CRATE_DEPENDS+=	password-hash-0.5.0
CARGO_CRATE_DEPENDS+=	pastey-0.2.3
CARGO_CRATE_DEPENDS+=	path-absolutize-4.0.1
CARGO_CRATE_DEPENDS+=	path-dedot-4.0.1
CARGO_CRATE_DEPENDS+=	path_resolver-0.2.13
CARGO_CRATE_DEPENDS+=	pathdiff-0.2.3
CARGO_CRATE_DEPENDS+=	pbkdf2-0.12.2
CARGO_CRATE_DEPENDS+=	pbkdf2-0.13.0
CARGO_CRATE_DEPENDS+=	pem-3.0.6
CARGO_CRATE_DEPENDS+=	pem-rfc7468-0.7.0
CARGO_CRATE_DEPENDS+=	pem-rfc7468-1.0.0
CARGO_CRATE_DEPENDS+=	percent-encoding-2.3.2
CARGO_CRATE_DEPENDS+=	pest-2.9.2
CARGO_CRATE_DEPENDS+=	pest_derive-2.9.2
CARGO_CRATE_DEPENDS+=	pest_generator-2.9.2
CARGO_CRATE_DEPENDS+=	pest_meta-2.9.2
CARGO_CRATE_DEPENDS+=	petgraph-0.8.3
CARGO_CRATE_DEPENDS+=	pgp-0.20.0
CARGO_CRATE_DEPENDS+=	phf-0.11.3
CARGO_CRATE_DEPENDS+=	phf-0.12.1
CARGO_CRATE_DEPENDS+=	phf-0.14.0
CARGO_CRATE_DEPENDS+=	phf_codegen-0.11.3
CARGO_CRATE_DEPENDS+=	phf_codegen-0.14.0
CARGO_CRATE_DEPENDS+=	phf_generator-0.11.3
CARGO_CRATE_DEPENDS+=	phf_generator-0.14.0
CARGO_CRATE_DEPENDS+=	phf_shared-0.11.3
CARGO_CRATE_DEPENDS+=	phf_shared-0.12.1
CARGO_CRATE_DEPENDS+=	phf_shared-0.14.0
CARGO_CRATE_DEPENDS+=	pin-project-1.1.13
CARGO_CRATE_DEPENDS+=	pin-project-internal-1.1.13
CARGO_CRATE_DEPENDS+=	pin-project-lite-0.2.17
CARGO_CRATE_DEPENDS+=	pin-utils-0.1.0
CARGO_CRATE_DEPENDS+=	pkcs1-0.7.5
CARGO_CRATE_DEPENDS+=	pkcs8-0.10.2
CARGO_CRATE_DEPENDS+=	pkg-config-0.3.34
CARGO_CRATE_DEPENDS+=	platforms-4.2.0
CARGO_CRATE_DEPENDS+=	plist-1.10.1
CARGO_CRATE_DEPENDS+=	pluralizer-0.5.0
CARGO_CRATE_DEPENDS+=	poly1305-0.8.0
CARGO_CRATE_DEPENDS+=	poly1305-0.9.1
CARGO_CRATE_DEPENDS+=	polyval-0.6.2
CARGO_CRATE_DEPENDS+=	portable-atomic-1.15.0
CARGO_CRATE_DEPENDS+=	portable-atomic-util-0.2.8
CARGO_CRATE_DEPENDS+=	potential_utf-0.1.6
CARGO_CRATE_DEPENDS+=	powerfmt-0.2.0
CARGO_CRATE_DEPENDS+=	ppmd-rust-1.5.0
CARGO_CRATE_DEPENDS+=	ppv-lite86-0.2.21
CARGO_CRATE_DEPENDS+=	pretty_assertions-1.4.1
CARGO_CRATE_DEPENDS+=	prettyplease-0.2.37
CARGO_CRATE_DEPENDS+=	primeorder-0.13.6
CARGO_CRATE_DEPENDS+=	proc-macro-error-attr2-2.0.0
CARGO_CRATE_DEPENDS+=	proc-macro-error-attr3-3.1.1
CARGO_CRATE_DEPENDS+=	proc-macro-error2-2.0.1
CARGO_CRATE_DEPENDS+=	proc-macro-error3-3.1.1
CARGO_CRATE_DEPENDS+=	proc-macro2-1.0.107
CARGO_CRATE_DEPENDS+=	prodash-31.0.0
CARGO_CRATE_DEPENDS+=	proptest-1.11.0
CARGO_CRATE_DEPENDS+=	prost-0.14.4
CARGO_CRATE_DEPENDS+=	prost-derive-0.14.4
CARGO_CRATE_DEPENDS+=	ptr_meta-0.3.2
CARGO_CRATE_DEPENDS+=	ptr_meta_derive-0.3.2
CARGO_CRATE_DEPENDS+=	pulldown-cmark-0.13.4
CARGO_CRATE_DEPENDS+=	purl-0.1.6
CARGO_CRATE_DEPENDS+=	quick-error-1.2.3
CARGO_CRATE_DEPENDS+=	quick-xml-0.41.0
CARGO_CRATE_DEPENDS+=	quick-xml-0.42.0
CARGO_CRATE_DEPENDS+=	quinn-0.11.12
CARGO_CRATE_DEPENDS+=	quinn-proto-0.11.18
CARGO_CRATE_DEPENDS+=	quinn-udp-0.5.15
CARGO_CRATE_DEPENDS+=	quote-1.0.47
CARGO_CRATE_DEPENDS+=	r-efi-5.3.0
CARGO_CRATE_DEPENDS+=	r-efi-6.0.0
CARGO_CRATE_DEPENDS+=	radium-0.7.0
CARGO_CRATE_DEPENDS+=	rancor-0.1.3
CARGO_CRATE_DEPENDS+=	rand-0.8.8
CARGO_CRATE_DEPENDS+=	rand-0.9.5
CARGO_CRATE_DEPENDS+=	rand-0.10.3
CARGO_CRATE_DEPENDS+=	rand_chacha-0.3.1
CARGO_CRATE_DEPENDS+=	rand_chacha-0.9.0
CARGO_CRATE_DEPENDS+=	rand_core-0.6.4
CARGO_CRATE_DEPENDS+=	rand_core-0.9.5
CARGO_CRATE_DEPENDS+=	rand_core-0.10.1
CARGO_CRATE_DEPENDS+=	rand_pcg-0.10.2
CARGO_CRATE_DEPENDS+=	rand_xorshift-0.4.0
CARGO_CRATE_DEPENDS+=	rattler-0.49.0
CARGO_CRATE_DEPENDS+=	rattler_cache-0.10.11
CARGO_CRATE_DEPENDS+=	rattler_conda_types-0.53.0
CARGO_CRATE_DEPENDS+=	rattler_conda_version-0.1.1
CARGO_CRATE_DEPENDS+=	rattler_digest-1.3.2
CARGO_CRATE_DEPENDS+=	rattler_macros-1.1.2
CARGO_CRATE_DEPENDS+=	rattler_menuinst-0.3.0
CARGO_CRATE_DEPENDS+=	rattler_networking-0.30.9
CARGO_CRATE_DEPENDS+=	rattler_package_streaming-0.27.6
CARGO_CRATE_DEPENDS+=	rattler_pty-0.2.16
CARGO_CRATE_DEPENDS+=	rattler_redaction-0.2.5
CARGO_CRATE_DEPENDS+=	rattler_repodata_gateway-0.33.5
CARGO_CRATE_DEPENDS+=	rattler_shell-0.28.0
CARGO_CRATE_DEPENDS+=	rattler_solve-9.0.8
CARGO_CRATE_DEPENDS+=	rattler_virtual_packages-7.0.0
CARGO_CRATE_DEPENDS+=	rayon-1.12.0
CARGO_CRATE_DEPENDS+=	rayon-core-1.13.0
CARGO_CRATE_DEPENDS+=	redox_syscall-0.5.18
CARGO_CRATE_DEPENDS+=	redox_users-0.5.3
CARGO_CRATE_DEPENDS+=	ref-cast-1.0.27
CARGO_CRATE_DEPENDS+=	ref-cast-impl-1.0.27
CARGO_CRATE_DEPENDS+=	reflink-copy-0.1.30
CARGO_CRATE_DEPENDS+=	regex-1.13.1
CARGO_CRATE_DEPENDS+=	regex-automata-0.4.18
CARGO_CRATE_DEPENDS+=	regex-lite-0.1.9
CARGO_CRATE_DEPENDS+=	regex-syntax-0.6.29
CARGO_CRATE_DEPENDS+=	regex-syntax-0.8.11
CARGO_CRATE_DEPENDS+=	rend-0.5.4
CARGO_CRATE_DEPENDS+=	replace_with-0.1.8
CARGO_CRATE_DEPENDS+=	reqwest-0.13.5
CARGO_CRATE_DEPENDS+=	resolvo-0.12.1
CARGO_CRATE_DEPENDS+=	retry-policies-0.5.2
CARGO_CRATE_DEPENDS+=	rfc6979-0.4.0
CARGO_CRATE_DEPENDS+=	ring-0.17.14
CARGO_CRATE_DEPENDS+=	ripemd-0.1.3
CARGO_CRATE_DEPENDS+=	rkyv-0.8.18
CARGO_CRATE_DEPENDS+=	rkyv_derive-0.8.18
CARGO_CRATE_DEPENDS+=	rmcp-3.4.0
CARGO_CRATE_DEPENDS+=	rmcp-macros-3.4.0
CARGO_CRATE_DEPENDS+=	rmp-0.8.15
CARGO_CRATE_DEPENDS+=	rmp-serde-1.3.1
CARGO_CRATE_DEPENDS+=	roff-1.1.1
CARGO_CRATE_DEPENDS+=	rops-0.1.7
CARGO_CRATE_DEPENDS+=	rowan-0.15.19
CARGO_CRATE_DEPENDS+=	rsa-0.9.10
CARGO_CRATE_DEPENDS+=	rust-embed-8.12.0
CARGO_CRATE_DEPENDS+=	rust-embed-impl-8.12.0
CARGO_CRATE_DEPENDS+=	rust-embed-utils-8.12.0
CARGO_CRATE_DEPENDS+=	rustc-demangle-0.1.28
CARGO_CRATE_DEPENDS+=	rustc-hash-1.1.0
CARGO_CRATE_DEPENDS+=	rustc-hash-2.1.3
CARGO_CRATE_DEPENDS+=	rustc_version-0.4.1
CARGO_CRATE_DEPENDS+=	rustix-0.38.44
CARGO_CRATE_DEPENDS+=	rustix-1.1.5
CARGO_CRATE_DEPENDS+=	rustls-0.23.45
CARGO_CRATE_DEPENDS+=	rustls-native-certs-0.8.4
CARGO_CRATE_DEPENDS+=	rustls-pki-types-1.15.1
CARGO_CRATE_DEPENDS+=	rustls-platform-verifier-0.7.0
CARGO_CRATE_DEPENDS+=	rustls-platform-verifier-android-0.1.1
CARGO_CRATE_DEPENDS+=	rustls-webpki-0.103.15
CARGO_CRATE_DEPENDS+=	rustversion-1.0.23
CARGO_CRATE_DEPENDS+=	rusty-fork-0.3.1
CARGO_CRATE_DEPENDS+=	ryu-1.0.23
CARGO_CRATE_DEPENDS+=	ryu-js-1.0.3
CARGO_CRATE_DEPENDS+=	salsa20-0.10.2
CARGO_CRATE_DEPENDS+=	same-file-1.0.6
CARGO_CRATE_DEPENDS+=	schannel-0.1.29
CARGO_CRATE_DEPENDS+=	schemars-0.9.0
CARGO_CRATE_DEPENDS+=	schemars-1.2.2
CARGO_CRATE_DEPENDS+=	schemars_derive-1.2.2
CARGO_CRATE_DEPENDS+=	scoped-tls-1.0.1
CARGO_CRATE_DEPENDS+=	scopeguard-1.2.0
CARGO_CRATE_DEPENDS+=	scrypt-0.11.0
CARGO_CRATE_DEPENDS+=	sec1-0.7.3
CARGO_CRATE_DEPENDS+=	seccompiler-0.5.0
CARGO_CRATE_DEPENDS+=	secrecy-0.10.3
CARGO_CRATE_DEPENDS+=	security-framework-3.7.0
CARGO_CRATE_DEPENDS+=	security-framework-sys-2.17.0
CARGO_CRATE_DEPENDS+=	self-replace-1.5.0
CARGO_CRATE_DEPENDS+=	self_cell-0.10.3
CARGO_CRATE_DEPENDS+=	self_cell-1.3.0
CARGO_CRATE_DEPENDS+=	self_update-1.3.0
CARGO_CRATE_DEPENDS+=	semver-1.0.28
CARGO_CRATE_DEPENDS+=	send_wrapper-0.6.0
CARGO_CRATE_DEPENDS+=	serde-1.0.229
CARGO_CRATE_DEPENDS+=	serde-untagged-0.1.9
CARGO_CRATE_DEPENDS+=	serde-value-0.7.0
CARGO_CRATE_DEPENDS+=	serde_bytes-0.11.19
CARGO_CRATE_DEPENDS+=	serde_core-1.0.229
CARGO_CRATE_DEPENDS+=	serde_derive-1.0.229
CARGO_CRATE_DEPENDS+=	serde_derive_internals-0.30.0
CARGO_CRATE_DEPENDS+=	serde_ignored-0.1.14
CARGO_CRATE_DEPENDS+=	serde_json-1.0.151
CARGO_CRATE_DEPENDS+=	serde_json_canonicalizer-0.3.2
CARGO_CRATE_DEPENDS+=	serde_regex-1.2.0
CARGO_CRATE_DEPENDS+=	serde_repr-0.1.21
CARGO_CRATE_DEPENDS+=	serde_spanned-0.6.9
CARGO_CRATE_DEPENDS+=	serde_spanned-1.1.1
CARGO_CRATE_DEPENDS+=	serde_urlencoded-0.7.1
CARGO_CRATE_DEPENDS+=	serde_with-3.22.0
CARGO_CRATE_DEPENDS+=	serde_with_macros-3.22.0
CARGO_CRATE_DEPENDS+=	serde_yaml-0.9.34+deprecated
CARGO_CRATE_DEPENDS+=	serdect-0.2.0
CARGO_CRATE_DEPENDS+=	serdect-0.3.0
CARGO_CRATE_DEPENDS+=	serial_test-3.5.0
CARGO_CRATE_DEPENDS+=	serial_test_derive-3.5.0
CARGO_CRATE_DEPENDS+=	sevenz-rust2-0.20.2
CARGO_CRATE_DEPENDS+=	sevenz-rust2-0.22.2
CARGO_CRATE_DEPENDS+=	sha1-0.10.7
CARGO_CRATE_DEPENDS+=	sha1-0.11.0
CARGO_CRATE_DEPENDS+=	sha1-checked-0.10.0
CARGO_CRATE_DEPENDS+=	sha2-0.10.9
CARGO_CRATE_DEPENDS+=	sha2-0.11.0
CARGO_CRATE_DEPENDS+=	sha3-0.10.9
CARGO_CRATE_DEPENDS+=	sharded-slab-0.1.7
CARGO_CRATE_DEPENDS+=	shared_child-1.1.2
CARGO_CRATE_DEPENDS+=	shared_thread-0.2.1
CARGO_CRATE_DEPENDS+=	shell-escape-0.1.5
CARGO_CRATE_DEPENDS+=	shell-words-1.1.1
CARGO_CRATE_DEPENDS+=	shlex-1.3.0
CARGO_CRATE_DEPENDS+=	shlex-2.0.1
CARGO_CRATE_DEPENDS+=	sigchld-0.2.5
CARGO_CRATE_DEPENDS+=	signal-hook-0.4.4
CARGO_CRATE_DEPENDS+=	signal-hook-registry-1.4.8
CARGO_CRATE_DEPENDS+=	signature-2.2.0
CARGO_CRATE_DEPENDS+=	signature-3.0.0
CARGO_CRATE_DEPENDS+=	sigstore-bundle-0.11.0
CARGO_CRATE_DEPENDS+=	sigstore-crypto-0.11.0
CARGO_CRATE_DEPENDS+=	sigstore-merkle-0.11.0
CARGO_CRATE_DEPENDS+=	sigstore-rekor-0.11.0
CARGO_CRATE_DEPENDS+=	sigstore-trust-root-0.11.0
CARGO_CRATE_DEPENDS+=	sigstore-tsa-0.11.0
CARGO_CRATE_DEPENDS+=	sigstore-tuf-0.11.0
CARGO_CRATE_DEPENDS+=	sigstore-types-0.11.0
CARGO_CRATE_DEPENDS+=	sigstore-verify-0.11.0
CARGO_CRATE_DEPENDS+=	simd-adler32-0.3.10
CARGO_CRATE_DEPENDS+=	simd-json-0.17.3
CARGO_CRATE_DEPENDS+=	simd_cesu8-1.2.0
CARGO_CRATE_DEPENDS+=	simdutf8-0.1.5
CARGO_CRATE_DEPENDS+=	similar-2.7.0
CARGO_CRATE_DEPENDS+=	simple_spawn_blocking-1.1.1
CARGO_CRATE_DEPENDS+=	siphasher-1.0.3
CARGO_CRATE_DEPENDS+=	slab-0.4.12
CARGO_CRATE_DEPENDS+=	slug-0.1.6
CARGO_CRATE_DEPENDS+=	smallvec-1.16.1
CARGO_CRATE_DEPENDS+=	snafu-0.9.2
CARGO_CRATE_DEPENDS+=	snafu-derive-0.9.2
CARGO_CRATE_DEPENDS+=	snap-1.1.2
CARGO_CRATE_DEPENDS+=	socket2-0.6.5
CARGO_CRATE_DEPENDS+=	socks-0.3.4
CARGO_CRATE_DEPENDS+=	sonic-number-0.1.3
CARGO_CRATE_DEPENDS+=	sonic-rs-0.5.10
CARGO_CRATE_DEPENDS+=	sonic-simd-0.1.4
CARGO_CRATE_DEPENDS+=	spdx-0.13.5
CARGO_CRATE_DEPENDS+=	spin-0.9.9
CARGO_CRATE_DEPENDS+=	spin-0.10.1
CARGO_CRATE_DEPENDS+=	spki-0.7.3
CARGO_CRATE_DEPENDS+=	spki-0.8.0
CARGO_CRATE_DEPENDS+=	stable_deref_trait-1.2.1
CARGO_CRATE_DEPENDS+=	static_assertions-1.1.0
CARGO_CRATE_DEPENDS+=	strip-ansi-escapes-0.2.1
CARGO_CRATE_DEPENDS+=	strsim-0.11.1
CARGO_CRATE_DEPENDS+=	strum-0.27.2
CARGO_CRATE_DEPENDS+=	strum-0.28.0
CARGO_CRATE_DEPENDS+=	strum_macros-0.27.2
CARGO_CRATE_DEPENDS+=	strum_macros-0.28.0
CARGO_CRATE_DEPENDS+=	subtle-2.6.1
CARGO_CRATE_DEPENDS+=	superslice-1.0.0
CARGO_CRATE_DEPENDS+=	supports-color-3.0.2
CARGO_CRATE_DEPENDS+=	supports-hyperlinks-3.2.0
CARGO_CRATE_DEPENDS+=	supports-unicode-3.0.0
CARGO_CRATE_DEPENDS+=	syn-1.0.109
CARGO_CRATE_DEPENDS+=	syn-2.0.119
CARGO_CRATE_DEPENDS+=	syn-3.0.6
CARGO_CRATE_DEPENDS+=	sync_wrapper-1.0.2
CARGO_CRATE_DEPENDS+=	synstructure-0.14.0
CARGO_CRATE_DEPENDS+=	sysctl-0.5.5
CARGO_CRATE_DEPENDS+=	system-configuration-0.7.0
CARGO_CRATE_DEPENDS+=	system-configuration-sys-0.6.0
CARGO_CRATE_DEPENDS+=	tabled-0.22.0
CARGO_CRATE_DEPENDS+=	tabled_derive-0.12.0
CARGO_CRATE_DEPENDS+=	tap-1.0.1
CARGO_CRATE_DEPENDS+=	taplo-0.14.0
CARGO_CRATE_DEPENDS+=	tar-0.4.46
CARGO_CRATE_DEPENDS+=	tempfile-3.27.0
CARGO_CRATE_DEPENDS+=	tera-1.20.1
CARGO_CRATE_DEPENDS+=	tera-2.4.0
CARGO_CRATE_DEPENDS+=	tera-contrib-0.3.0
CARGO_CRATE_DEPENDS+=	termcolor-1.4.1
CARGO_CRATE_DEPENDS+=	terminal_size-0.4.4
CARGO_CRATE_DEPENDS+=	test-log-0.2.21
CARGO_CRATE_DEPENDS+=	test-log-core-0.2.21
CARGO_CRATE_DEPENDS+=	test-log-macros-0.2.21
CARGO_CRATE_DEPENDS+=	testing_table-0.3.0
CARGO_CRATE_DEPENDS+=	text-size-1.1.1
CARGO_CRATE_DEPENDS+=	textwrap-0.16.2
CARGO_CRATE_DEPENDS+=	thiserror-1.0.69
CARGO_CRATE_DEPENDS+=	thiserror-2.0.20
CARGO_CRATE_DEPENDS+=	thiserror-impl-1.0.69
CARGO_CRATE_DEPENDS+=	thiserror-impl-2.0.20
CARGO_CRATE_DEPENDS+=	thread_local-1.1.10
CARGO_CRATE_DEPENDS+=	time-0.3.55
CARGO_CRATE_DEPENDS+=	time-core-0.1.9
CARGO_CRATE_DEPENDS+=	time-macros-0.2.32
CARGO_CRATE_DEPENDS+=	tinystr-0.8.4
CARGO_CRATE_DEPENDS+=	tinyvec-1.13.3
CARGO_CRATE_DEPENDS+=	tls_codec-0.4.2
CARGO_CRATE_DEPENDS+=	tls_codec_derive-0.4.2
CARGO_CRATE_DEPENDS+=	tokio-1.53.1
CARGO_CRATE_DEPENDS+=	tokio-macros-2.7.2
CARGO_CRATE_DEPENDS+=	tokio-native-tls-0.3.1
CARGO_CRATE_DEPENDS+=	tokio-rustls-0.26.5
CARGO_CRATE_DEPENDS+=	tokio-stream-0.1.19
CARGO_CRATE_DEPENDS+=	tokio-util-0.7.19
CARGO_CRATE_DEPENDS+=	toml-0.5.11
CARGO_CRATE_DEPENDS+=	toml-0.8.23
CARGO_CRATE_DEPENDS+=	toml-1.1.6+spec-1.1.0
CARGO_CRATE_DEPENDS+=	toml_datetime-0.6.11
CARGO_CRATE_DEPENDS+=	toml_datetime-1.1.1+spec-1.1.0
CARGO_CRATE_DEPENDS+=	toml_edit-0.22.27
CARGO_CRATE_DEPENDS+=	toml_edit-0.25.15+spec-1.1.0
CARGO_CRATE_DEPENDS+=	toml_parser-1.1.3+spec-1.1.0
CARGO_CRATE_DEPENDS+=	toml_write-0.1.2
CARGO_CRATE_DEPENDS+=	toml_writer-1.1.2+spec-1.1.0
CARGO_CRATE_DEPENDS+=	tower-0.5.3
CARGO_CRATE_DEPENDS+=	tower-http-0.6.11
CARGO_CRATE_DEPENDS+=	tower-layer-0.3.3
CARGO_CRATE_DEPENDS+=	tower-service-0.3.3
CARGO_CRATE_DEPENDS+=	tracing-0.1.44
CARGO_CRATE_DEPENDS+=	tracing-attributes-0.1.31
CARGO_CRATE_DEPENDS+=	tracing-core-0.1.36
CARGO_CRATE_DEPENDS+=	tracing-log-0.2.0
CARGO_CRATE_DEPENDS+=	tracing-serde-0.2.0
CARGO_CRATE_DEPENDS+=	tracing-subscriber-0.3.23
CARGO_CRATE_DEPENDS+=	try-lock-0.2.5
CARGO_CRATE_DEPENDS+=	twofish-0.7.1
CARGO_CRATE_DEPENDS+=	type-map-0.5.1
CARGO_CRATE_DEPENDS+=	typed-path-0.12.3
CARGO_CRATE_DEPENDS+=	typeid-1.0.3
CARGO_CRATE_DEPENDS+=	typenum-1.20.1
CARGO_CRATE_DEPENDS+=	ubi-0.12.0
CARGO_CRATE_DEPENDS+=	ucd-trie-0.1.7
CARGO_CRATE_DEPENDS+=	uluru-3.1.0
CARGO_CRATE_DEPENDS+=	unarray-0.1.4
CARGO_CRATE_DEPENDS+=	unic-langid-0.9.6
CARGO_CRATE_DEPENDS+=	unic-langid-impl-0.9.6
CARGO_CRATE_DEPENDS+=	unicase-2.9.0
CARGO_CRATE_DEPENDS+=	unicode-bom-2.0.3
CARGO_CRATE_DEPENDS+=	unicode-ident-1.0.26
CARGO_CRATE_DEPENDS+=	unicode-linebreak-0.1.5
CARGO_CRATE_DEPENDS+=	unicode-normalization-0.1.25
CARGO_CRATE_DEPENDS+=	unicode-segmentation-1.13.3
CARGO_CRATE_DEPENDS+=	unicode-width-0.1.14
CARGO_CRATE_DEPENDS+=	unicode-width-0.2.2
CARGO_CRATE_DEPENDS+=	unicode-xid-0.2.6
CARGO_CRATE_DEPENDS+=	unit-prefix-0.5.2
CARGO_CRATE_DEPENDS+=	universal-hash-0.5.1
CARGO_CRATE_DEPENDS+=	universal-hash-0.6.1
CARGO_CRATE_DEPENDS+=	unsafe-libyaml-0.2.11
CARGO_CRATE_DEPENDS+=	untrusted-0.7.1
CARGO_CRATE_DEPENDS+=	untrusted-0.9.0
CARGO_CRATE_DEPENDS+=	ureq-3.4.2
CARGO_CRATE_DEPENDS+=	ureq-proto-0.6.4
CARGO_CRATE_DEPENDS+=	url-2.5.8
CARGO_CRATE_DEPENDS+=	urlencoding-2.1.3
CARGO_CRATE_DEPENDS+=	usage-argv-6.11.1
CARGO_CRATE_DEPENDS+=	usage-cli-6.11.1
CARGO_CRATE_DEPENDS+=	usage-config-6.11.1
CARGO_CRATE_DEPENDS+=	usage-derive-6.11.1
CARGO_CRATE_DEPENDS+=	usage-lib-6.11.1
CARGO_CRATE_DEPENDS+=	usage-rs-6.11.1
CARGO_CRATE_DEPENDS+=	usage-test-6.11.1
CARGO_CRATE_DEPENDS+=	usage-validation-6.11.1
CARGO_CRATE_DEPENDS+=	utf8-zero-0.8.1
CARGO_CRATE_DEPENDS+=	utf8_iter-1.0.4
CARGO_CRATE_DEPENDS+=	utf8parse-0.2.2
CARGO_CRATE_DEPENDS+=	uuid-1.26.1
CARGO_CRATE_DEPENDS+=	valuable-0.1.1
CARGO_CRATE_DEPENDS+=	value-trait-0.12.2
CARGO_CRATE_DEPENDS+=	vcpkg-0.2.15
CARGO_CRATE_DEPENDS+=	version_check-0.9.5
CARGO_CRATE_DEPENDS+=	versions-8.0.1
CARGO_CRATE_DEPENDS+=	vsimd-0.8.0
CARGO_CRATE_DEPENDS+=	vte-0.14.1
CARGO_CRATE_DEPENDS+=	wait-timeout-0.2.1
CARGO_CRATE_DEPENDS+=	walkdir-2.5.0
CARGO_CRATE_DEPENDS+=	want-0.3.1
CARGO_CRATE_DEPENDS+=	wasi-0.11.1+wasi-snapshot-preview1
CARGO_CRATE_DEPENDS+=	wasip2-1.0.4+wasi-0.2.12
CARGO_CRATE_DEPENDS+=	wasm-bindgen-0.2.128
CARGO_CRATE_DEPENDS+=	wasm-bindgen-futures-0.4.78
CARGO_CRATE_DEPENDS+=	wasm-bindgen-macro-0.2.128
CARGO_CRATE_DEPENDS+=	wasm-bindgen-macro-support-0.2.128
CARGO_CRATE_DEPENDS+=	wasm-bindgen-shared-0.2.128
CARGO_CRATE_DEPENDS+=	wasm-streams-0.5.0
CARGO_CRATE_DEPENDS+=	wasmtimer-0.4.3
CARGO_CRATE_DEPENDS+=	web-sys-0.3.105
CARGO_CRATE_DEPENDS+=	web-time-1.1.0
CARGO_CRATE_DEPENDS+=	webpki-root-certs-1.0.9
CARGO_CRATE_DEPENDS+=	webpki-roots-1.0.9
CARGO_CRATE_DEPENDS+=	which-8.0.6
CARGO_CRATE_DEPENDS+=	widestring-1.2.1
CARGO_CRATE_DEPENDS+=	winapi-0.3.9
CARGO_CRATE_DEPENDS+=	winapi-i686-pc-windows-gnu-0.4.0
CARGO_CRATE_DEPENDS+=	winapi-util-0.1.11
CARGO_CRATE_DEPENDS+=	winapi-x86_64-pc-windows-gnu-0.4.0
CARGO_CRATE_DEPENDS+=	windows-0.48.0
CARGO_CRATE_DEPENDS+=	windows-0.61.3
CARGO_CRATE_DEPENDS+=	windows-0.62.2
CARGO_CRATE_DEPENDS+=	windows-collections-0.2.0
CARGO_CRATE_DEPENDS+=	windows-collections-0.3.2
CARGO_CRATE_DEPENDS+=	windows-core-0.61.2
CARGO_CRATE_DEPENDS+=	windows-core-0.62.2
CARGO_CRATE_DEPENDS+=	windows-future-0.2.1
CARGO_CRATE_DEPENDS+=	windows-future-0.3.2
CARGO_CRATE_DEPENDS+=	windows-implement-0.60.2
CARGO_CRATE_DEPENDS+=	windows-interface-0.59.3
CARGO_CRATE_DEPENDS+=	windows-link-0.1.3
CARGO_CRATE_DEPENDS+=	windows-link-0.2.1
CARGO_CRATE_DEPENDS+=	windows-numerics-0.2.0
CARGO_CRATE_DEPENDS+=	windows-numerics-0.3.1
CARGO_CRATE_DEPENDS+=	windows-registry-0.6.1
CARGO_CRATE_DEPENDS+=	windows-result-0.3.4
CARGO_CRATE_DEPENDS+=	windows-result-0.4.1
CARGO_CRATE_DEPENDS+=	windows-strings-0.4.2
CARGO_CRATE_DEPENDS+=	windows-strings-0.5.1
CARGO_CRATE_DEPENDS+=	windows-sys-0.52.0
CARGO_CRATE_DEPENDS+=	windows-sys-0.59.0
CARGO_CRATE_DEPENDS+=	windows-sys-0.60.2
CARGO_CRATE_DEPENDS+=	windows-sys-0.61.2
CARGO_CRATE_DEPENDS+=	windows-targets-0.48.5
CARGO_CRATE_DEPENDS+=	windows-targets-0.52.6
CARGO_CRATE_DEPENDS+=	windows-targets-0.53.5
CARGO_CRATE_DEPENDS+=	windows-threading-0.1.0
CARGO_CRATE_DEPENDS+=	windows-threading-0.2.1
CARGO_CRATE_DEPENDS+=	windows_aarch64_gnullvm-0.48.5
CARGO_CRATE_DEPENDS+=	windows_aarch64_gnullvm-0.52.6
CARGO_CRATE_DEPENDS+=	windows_aarch64_gnullvm-0.53.1
CARGO_CRATE_DEPENDS+=	windows_aarch64_msvc-0.48.5
CARGO_CRATE_DEPENDS+=	windows_aarch64_msvc-0.52.6
CARGO_CRATE_DEPENDS+=	windows_aarch64_msvc-0.53.1
CARGO_CRATE_DEPENDS+=	windows_i686_gnu-0.48.5
CARGO_CRATE_DEPENDS+=	windows_i686_gnu-0.52.6
CARGO_CRATE_DEPENDS+=	windows_i686_gnu-0.53.1
CARGO_CRATE_DEPENDS+=	windows_i686_gnullvm-0.52.6
CARGO_CRATE_DEPENDS+=	windows_i686_gnullvm-0.53.1
CARGO_CRATE_DEPENDS+=	windows_i686_msvc-0.48.5
CARGO_CRATE_DEPENDS+=	windows_i686_msvc-0.52.6
CARGO_CRATE_DEPENDS+=	windows_i686_msvc-0.53.1
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnu-0.48.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnu-0.52.6
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnu-0.53.1
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnullvm-0.48.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnullvm-0.52.6
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnullvm-0.53.1
CARGO_CRATE_DEPENDS+=	windows_x86_64_msvc-0.48.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_msvc-0.52.6
CARGO_CRATE_DEPENDS+=	windows_x86_64_msvc-0.53.1
CARGO_CRATE_DEPENDS+=	winnow-0.7.15
CARGO_CRATE_DEPENDS+=	winnow-1.0.4
CARGO_CRATE_DEPENDS+=	winver-1.0.0
CARGO_CRATE_DEPENDS+=	wiremock-0.6.5
CARGO_CRATE_DEPENDS+=	wit-bindgen-0.57.1
CARGO_CRATE_DEPENDS+=	writeable-0.6.4
CARGO_CRATE_DEPENDS+=	wsl-0.1.0
CARGO_CRATE_DEPENDS+=	wyz-0.5.1
CARGO_CRATE_DEPENDS+=	x25519-dalek-2.0.1
CARGO_CRATE_DEPENDS+=	x509-cert-0.2.5
CARGO_CRATE_DEPENDS+=	x509-cert-0.3.0
CARGO_CRATE_DEPENDS+=	x509-tsp-0.1.0
CARGO_CRATE_DEPENDS+=	xattr-1.6.1
CARGO_CRATE_DEPENDS+=	xmlparser-0.13.6
CARGO_CRATE_DEPENDS+=	xx-2.6.2
CARGO_CRATE_DEPENDS+=	xz2-0.1.7
CARGO_CRATE_DEPENDS+=	yaml_serde-0.10.7
CARGO_CRATE_DEPENDS+=	yansi-1.0.1
CARGO_CRATE_DEPENDS+=	yoke-0.8.3
CARGO_CRATE_DEPENDS+=	yoke-derive-0.8.3
CARGO_CRATE_DEPENDS+=	zerocopy-0.8.57
CARGO_CRATE_DEPENDS+=	zerocopy-derive-0.8.57
CARGO_CRATE_DEPENDS+=	zerofrom-0.1.8
CARGO_CRATE_DEPENDS+=	zerofrom-derive-0.1.8
CARGO_CRATE_DEPENDS+=	zeroize-1.9.0
CARGO_CRATE_DEPENDS+=	zeroize_derive-1.5.0
CARGO_CRATE_DEPENDS+=	zerotrie-0.2.5
CARGO_CRATE_DEPENDS+=	zerovec-0.11.8
CARGO_CRATE_DEPENDS+=	zerovec-derive-0.11.6
CARGO_CRATE_DEPENDS+=	zip-7.2.0
CARGO_CRATE_DEPENDS+=	zip-8.6.0
CARGO_CRATE_DEPENDS+=	zipsign-api-0.2.1
CARGO_CRATE_DEPENDS+=	zlib-rs-0.6.8
CARGO_CRATE_DEPENDS+=	zmij-1.0.23
CARGO_CRATE_DEPENDS+=	zopfli-0.8.3
CARGO_CRATE_DEPENDS+=	zstd-0.13.3
CARGO_CRATE_DEPENDS+=	zstd-0.14.0
CARGO_CRATE_DEPENDS+=	zstd-safe-7.3.0
CARGO_CRATE_DEPENDS+=	zstd-safe-8.0.0
CARGO_CRATE_DEPENDS+=	zstd-sys-2.1.0+zstd.1.5.7
