# $NetBSD: cargo-depends.mk,v 1.8 2026/09/21 18:45:20 adam Exp $

CARGO_CRATE_DEPENDS+=	addr2line-0.25.1
CARGO_CRATE_DEPENDS+=	adler2-2.0.1
CARGO_CRATE_DEPENDS+=	aes-0.8.4
CARGO_CRATE_DEPENDS+=	aho-corasick-1.1.5
CARGO_CRATE_DEPENDS+=	android_system_properties-0.1.5
CARGO_CRATE_DEPENDS+=	anstream-0.6.11
CARGO_CRATE_DEPENDS+=	anstyle-1.0.13
CARGO_CRATE_DEPENDS+=	anstyle-parse-0.2.1
CARGO_CRATE_DEPENDS+=	anstyle-query-1.0.0
CARGO_CRATE_DEPENDS+=	anstyle-wincon-3.0.2
CARGO_CRATE_DEPENDS+=	anyhow-1.0.100
CARGO_CRATE_DEPENDS+=	approx-0.5.1
CARGO_CRATE_DEPENDS+=	ar-0.9.0
CARGO_CRATE_DEPENDS+=	arc-swap-1.9.2
CARGO_CRATE_DEPENDS+=	arcstr-1.2.0
CARGO_CRATE_DEPENDS+=	arrayref-0.3.7
CARGO_CRATE_DEPENDS+=	arrayvec-0.7.4
CARGO_CRATE_DEPENDS+=	ascii-1.1.0
CARGO_CRATE_DEPENDS+=	assert_cmd-2.0.13
CARGO_CRATE_DEPENDS+=	async-lock-3.4.2
CARGO_CRATE_DEPENDS+=	async-trait-0.1.83
CARGO_CRATE_DEPENDS+=	atomic-waker-1.1.2
CARGO_CRATE_DEPENDS+=	autocfg-1.1.0
CARGO_CRATE_DEPENDS+=	aws-lc-rs-1.17.0
CARGO_CRATE_DEPENDS+=	aws-lc-sys-0.41.0
CARGO_CRATE_DEPENDS+=	backon-1.6.0
CARGO_CRATE_DEPENDS+=	backtrace-0.3.76
CARGO_CRATE_DEPENDS+=	base64-0.13.1
CARGO_CRATE_DEPENDS+=	base64-0.21.7
CARGO_CRATE_DEPENDS+=	base64-0.22.1
CARGO_CRATE_DEPENDS+=	base64-0.23.1
CARGO_CRATE_DEPENDS+=	base64ct-1.6.0
CARGO_CRATE_DEPENDS+=	bincode-1.3.3
CARGO_CRATE_DEPENDS+=	bitflags-1.3.2
CARGO_CRATE_DEPENDS+=	bitflags-2.13.1
CARGO_CRATE_DEPENDS+=	blake3-1.8.2
CARGO_CRATE_DEPENDS+=	block-buffer-0.10.4
CARGO_CRATE_DEPENDS+=	block-buffer-0.12.1
CARGO_CRATE_DEPENDS+=	block-padding-0.3.3
CARGO_CRATE_DEPENDS+=	bstr-1.9.0
CARGO_CRATE_DEPENDS+=	buf_redux-0.8.4
CARGO_CRATE_DEPENDS+=	bumpalo-3.14.0
CARGO_CRATE_DEPENDS+=	byteorder-1.5.0
CARGO_CRATE_DEPENDS+=	bytes-1.10.1
CARGO_CRATE_DEPENDS+=	cbc-0.1.2
CARGO_CRATE_DEPENDS+=	cc-1.4.1
CARGO_CRATE_DEPENDS+=	cfg-if-0.1.10
CARGO_CRATE_DEPENDS+=	cfg-if-1.0.4
CARGO_CRATE_DEPENDS+=	cfg_aliases-0.2.1
CARGO_CRATE_DEPENDS+=	chacha20-0.10.1
CARGO_CRATE_DEPENDS+=	chrono-0.4.42
CARGO_CRATE_DEPENDS+=	chunked_transfer-1.5.0
CARGO_CRATE_DEPENDS+=	cipher-0.4.4
CARGO_CRATE_DEPENDS+=	clap-4.5.13
CARGO_CRATE_DEPENDS+=	clap_builder-4.5.13
CARGO_CRATE_DEPENDS+=	clap_derive-4.5.13
CARGO_CRATE_DEPENDS+=	clap_lex-0.7.4
CARGO_CRATE_DEPENDS+=	cmake-0.1.58
CARGO_CRATE_DEPENDS+=	cmov-0.5.4
CARGO_CRATE_DEPENDS+=	codspeed-4.2.0
CARGO_CRATE_DEPENDS+=	codspeed-divan-compat-4.2.0
CARGO_CRATE_DEPENDS+=	codspeed-divan-compat-macros-4.2.0
CARGO_CRATE_DEPENDS+=	codspeed-divan-compat-walltime-4.2.0
CARGO_CRATE_DEPENDS+=	colorchoice-1.0.0
CARGO_CRATE_DEPENDS+=	colored-2.2.0
CARGO_CRATE_DEPENDS+=	combine-4.6.6
CARGO_CRATE_DEPENDS+=	condtype-1.3.0
CARGO_CRATE_DEPENDS+=	const-oid-0.9.6
CARGO_CRATE_DEPENDS+=	const-oid-0.10.2
CARGO_CRATE_DEPENDS+=	const-random-0.1.17
CARGO_CRATE_DEPENDS+=	const-random-macro-0.1.16
CARGO_CRATE_DEPENDS+=	const_format-0.2.35
CARGO_CRATE_DEPENDS+=	const_format_proc_macros-0.2.34
CARGO_CRATE_DEPENDS+=	constant_time_eq-0.3.1
CARGO_CRATE_DEPENDS+=	core-foundation-0.9.4
CARGO_CRATE_DEPENDS+=	core-foundation-0.10.1
CARGO_CRATE_DEPENDS+=	core-foundation-sys-0.8.6
CARGO_CRATE_DEPENDS+=	core_affinity-0.8.3
CARGO_CRATE_DEPENDS+=	cpufeatures-0.2.12
CARGO_CRATE_DEPENDS+=	cpufeatures-0.3.0
CARGO_CRATE_DEPENDS+=	crc-fast-1.10.0
CARGO_CRATE_DEPENDS+=	crc16-0.4.0
CARGO_CRATE_DEPENDS+=	crc32fast-1.3.2
CARGO_CRATE_DEPENDS+=	critical-section-1.2.0
CARGO_CRATE_DEPENDS+=	crossbeam-channel-0.5.16
CARGO_CRATE_DEPENDS+=	crossbeam-epoch-0.9.20
CARGO_CRATE_DEPENDS+=	crossbeam-utils-0.8.18
CARGO_CRATE_DEPENDS+=	crunchy-0.2.2
CARGO_CRATE_DEPENDS+=	crypto-common-0.1.6
CARGO_CRATE_DEPENDS+=	crypto-common-0.2.2
CARGO_CRATE_DEPENDS+=	ctor-0.2.9
CARGO_CRATE_DEPENDS+=	ctutils-0.4.2
CARGO_CRATE_DEPENDS+=	daemonix-0.1.0
CARGO_CRATE_DEPENDS+=	data-encoding-2.11.1
CARGO_CRATE_DEPENDS+=	defmt-1.1.1
CARGO_CRATE_DEPENDS+=	defmt-macros-1.1.1
CARGO_CRATE_DEPENDS+=	defmt-parser-1.0.0
CARGO_CRATE_DEPENDS+=	der-0.7.8
CARGO_CRATE_DEPENDS+=	deranged-0.3.11
CARGO_CRATE_DEPENDS+=	difflib-0.4.0
CARGO_CRATE_DEPENDS+=	digest-0.10.7
CARGO_CRATE_DEPENDS+=	digest-0.11.3
CARGO_CRATE_DEPENDS+=	directories-6.0.0
CARGO_CRATE_DEPENDS+=	dirs-sys-0.5.0
CARGO_CRATE_DEPENDS+=	displaydoc-0.2.5
CARGO_CRATE_DEPENDS+=	divan-macros-0.1.17
CARGO_CRATE_DEPENDS+=	dlv-list-0.5.2
CARGO_CRATE_DEPENDS+=	doc-comment-0.3.3
CARGO_CRATE_DEPENDS+=	dunce-1.0.5
CARGO_CRATE_DEPENDS+=	either-1.17.0
CARGO_CRATE_DEPENDS+=	encoding_rs-0.8.34
CARGO_CRATE_DEPENDS+=	env_filter-0.1.4
CARGO_CRATE_DEPENDS+=	env_logger-0.11.8
CARGO_CRATE_DEPENDS+=	equivalent-1.0.1
CARGO_CRATE_DEPENDS+=	errno-0.3.14
CARGO_CRATE_DEPENDS+=	event-listener-5.4.2
CARGO_CRATE_DEPENDS+=	event-listener-strategy-0.5.4
CARGO_CRATE_DEPENDS+=	fastpool-1.1.0
CARGO_CRATE_DEPENDS+=	fastrand-2.0.1
CARGO_CRATE_DEPENDS+=	filetime-0.2.23
CARGO_CRATE_DEPENDS+=	find-msvc-tools-0.1.10
CARGO_CRATE_DEPENDS+=	flate2-1.1.5
CARGO_CRATE_DEPENDS+=	float-cmp-0.9.0
CARGO_CRATE_DEPENDS+=	flume-0.11.1
CARGO_CRATE_DEPENDS+=	fnv-1.0.7
CARGO_CRATE_DEPENDS+=	foreign-types-0.3.2
CARGO_CRATE_DEPENDS+=	foreign-types-shared-0.1.1
CARGO_CRATE_DEPENDS+=	form_urlencoded-1.2.1
CARGO_CRATE_DEPENDS+=	fs-err-3.2.0
CARGO_CRATE_DEPENDS+=	fs_extra-1.3.0
CARGO_CRATE_DEPENDS+=	futures-0.3.30
CARGO_CRATE_DEPENDS+=	futures-channel-0.3.31
CARGO_CRATE_DEPENDS+=	futures-core-0.3.31
CARGO_CRATE_DEPENDS+=	futures-executor-0.3.30
CARGO_CRATE_DEPENDS+=	futures-io-0.3.31
CARGO_CRATE_DEPENDS+=	futures-macro-0.3.31
CARGO_CRATE_DEPENDS+=	futures-sink-0.3.31
CARGO_CRATE_DEPENDS+=	futures-task-0.3.31
CARGO_CRATE_DEPENDS+=	futures-util-0.3.31
CARGO_CRATE_DEPENDS+=	generator-0.8.9
CARGO_CRATE_DEPENDS+=	generic-array-0.14.7
CARGO_CRATE_DEPENDS+=	getrandom-0.2.11
CARGO_CRATE_DEPENDS+=	getrandom-0.3.3
CARGO_CRATE_DEPENDS+=	getrandom-0.4.3
CARGO_CRATE_DEPENDS+=	ghac-0.3.0
CARGO_CRATE_DEPENDS+=	gimli-0.32.3
CARGO_CRATE_DEPENDS+=	glob-0.3.4
CARGO_CRATE_DEPENDS+=	gzp-2.0.1
CARGO_CRATE_DEPENDS+=	h2-0.4.5
CARGO_CRATE_DEPENDS+=	hashbrown-0.14.3
CARGO_CRATE_DEPENDS+=	hashbrown-0.16.1
CARGO_CRATE_DEPENDS+=	heck-0.5.0
CARGO_CRATE_DEPENDS+=	hermit-abi-0.5.2
CARGO_CRATE_DEPENDS+=	hex-0.4.3
CARGO_CRATE_DEPENDS+=	hickory-net-0.26.1
CARGO_CRATE_DEPENDS+=	hickory-proto-0.26.1
CARGO_CRATE_DEPENDS+=	hickory-resolver-0.26.1
CARGO_CRATE_DEPENDS+=	hmac-0.12.1
CARGO_CRATE_DEPENDS+=	hmac-0.13.0
CARGO_CRATE_DEPENDS+=	home-0.5.12
CARGO_CRATE_DEPENDS+=	hostname-0.4.2
CARGO_CRATE_DEPENDS+=	http-1.5.0
CARGO_CRATE_DEPENDS+=	http-body-1.0.0
CARGO_CRATE_DEPENDS+=	http-body-util-0.1.3
CARGO_CRATE_DEPENDS+=	httparse-1.10.1
CARGO_CRATE_DEPENDS+=	httpdate-1.0.3
CARGO_CRATE_DEPENDS+=	hybrid-array-0.4.14
CARGO_CRATE_DEPENDS+=	hyper-1.8.0
CARGO_CRATE_DEPENDS+=	hyper-rustls-0.27.2
CARGO_CRATE_DEPENDS+=	hyper-tls-0.6.0
CARGO_CRATE_DEPENDS+=	hyper-util-0.1.17
CARGO_CRATE_DEPENDS+=	iana-time-zone-0.1.59
CARGO_CRATE_DEPENDS+=	iana-time-zone-haiku-0.1.2
CARGO_CRATE_DEPENDS+=	icu_collections-2.1.1
CARGO_CRATE_DEPENDS+=	icu_locale_core-2.1.1
CARGO_CRATE_DEPENDS+=	icu_normalizer-2.1.1
CARGO_CRATE_DEPENDS+=	icu_normalizer_data-2.1.1
CARGO_CRATE_DEPENDS+=	icu_properties-2.1.2
CARGO_CRATE_DEPENDS+=	icu_properties_data-2.1.2
CARGO_CRATE_DEPENDS+=	icu_provider-2.1.1
CARGO_CRATE_DEPENDS+=	idna-1.1.0
CARGO_CRATE_DEPENDS+=	idna_adapter-1.2.1
CARGO_CRATE_DEPENDS+=	indexmap-2.12.1
CARGO_CRATE_DEPENDS+=	inout-0.1.3
CARGO_CRATE_DEPENDS+=	ipconfig-0.3.4
CARGO_CRATE_DEPENDS+=	ipnet-2.9.0
CARGO_CRATE_DEPENDS+=	iri-string-0.7.10
CARGO_CRATE_DEPENDS+=	itertools-0.14.0
CARGO_CRATE_DEPENDS+=	itoa-1.0.10
CARGO_CRATE_DEPENDS+=	jiff-0.2.35
CARGO_CRATE_DEPENDS+=	jiff-core-0.1.0
CARGO_CRATE_DEPENDS+=	jiff-static-0.2.35
CARGO_CRATE_DEPENDS+=	jiff-tzdb-0.1.4
CARGO_CRATE_DEPENDS+=	jiff-tzdb-platform-0.1.3
CARGO_CRATE_DEPENDS+=	jni-0.22.4
CARGO_CRATE_DEPENDS+=	jni-macros-0.22.4
CARGO_CRATE_DEPENDS+=	jni-sys-0.4.1
CARGO_CRATE_DEPENDS+=	jni-sys-macros-0.4.1
CARGO_CRATE_DEPENDS+=	jobserver-0.1.35
CARGO_CRATE_DEPENDS+=	js-sys-0.3.103
CARGO_CRATE_DEPENDS+=	jsonwebtoken-9.2.0
CARGO_CRATE_DEPENDS+=	lazy_static-1.5.0
CARGO_CRATE_DEPENDS+=	libc-0.2.189
CARGO_CRATE_DEPENDS+=	libm-0.2.8
CARGO_CRATE_DEPENDS+=	libmount-0.1.15
CARGO_CRATE_DEPENDS+=	libredox-0.1.10
CARGO_CRATE_DEPENDS+=	linked-hash-map-0.5.6
CARGO_CRATE_DEPENDS+=	linux-raw-sys-0.4.15
CARGO_CRATE_DEPENDS+=	litemap-0.8.1
CARGO_CRATE_DEPENDS+=	lock_api-0.4.11
CARGO_CRATE_DEPENDS+=	log-0.4.33
CARGO_CRATE_DEPENDS+=	loom-0.7.2
CARGO_CRATE_DEPENDS+=	lru-slab-0.1.2
CARGO_CRATE_DEPENDS+=	matchers-0.1.0
CARGO_CRATE_DEPENDS+=	md-5-0.11.0
CARGO_CRATE_DEPENDS+=	mea-0.6.5
CARGO_CRATE_DEPENDS+=	memchr-2.8.3
CARGO_CRATE_DEPENDS+=	memmap2-0.9.4
CARGO_CRATE_DEPENDS+=	mime-0.3.17
CARGO_CRATE_DEPENDS+=	mime_guess-2.0.4
CARGO_CRATE_DEPENDS+=	miniz_oxide-0.8.9
CARGO_CRATE_DEPENDS+=	mio-1.2.2
CARGO_CRATE_DEPENDS+=	moka-0.12.10
CARGO_CRATE_DEPENDS+=	multipart-0.18.0
CARGO_CRATE_DEPENDS+=	nanorand-0.7.0
CARGO_CRATE_DEPENDS+=	native-tls-0.2.18
CARGO_CRATE_DEPENDS+=	ndk-context-0.1.1
CARGO_CRATE_DEPENDS+=	nix-0.14.1
CARGO_CRATE_DEPENDS+=	nix-0.30.1
CARGO_CRATE_DEPENDS+=	normalize-line-endings-0.3.0
CARGO_CRATE_DEPENDS+=	nu-ansi-term-0.46.0
CARGO_CRATE_DEPENDS+=	num-bigint-0.4.6
CARGO_CRATE_DEPENDS+=	num-bigint-dig-0.8.6
CARGO_CRATE_DEPENDS+=	num-conv-0.1.0
CARGO_CRATE_DEPENDS+=	num-integer-0.1.46
CARGO_CRATE_DEPENDS+=	num-iter-0.1.43
CARGO_CRATE_DEPENDS+=	num-traits-0.2.19
CARGO_CRATE_DEPENDS+=	num_cpus-1.17.0
CARGO_CRATE_DEPENDS+=	num_threads-0.1.6
CARGO_CRATE_DEPENDS+=	number_prefix-0.4.0
CARGO_CRATE_DEPENDS+=	object-0.37.1
CARGO_CRATE_DEPENDS+=	once_cell-1.21.4
CARGO_CRATE_DEPENDS+=	opendal-0.58.1
CARGO_CRATE_DEPENDS+=	opendal-core-0.58.1
CARGO_CRATE_DEPENDS+=	opendal-http-transport-reqwest-0.58.1
CARGO_CRATE_DEPENDS+=	opendal-layer-logging-0.58.1
CARGO_CRATE_DEPENDS+=	opendal-service-azblob-0.58.1
CARGO_CRATE_DEPENDS+=	opendal-service-azure-common-0.58.1
CARGO_CRATE_DEPENDS+=	opendal-service-cos-0.58.1
CARGO_CRATE_DEPENDS+=	opendal-service-gcs-0.58.1
CARGO_CRATE_DEPENDS+=	opendal-service-ghac-0.58.1
CARGO_CRATE_DEPENDS+=	opendal-service-memcached-0.58.1
CARGO_CRATE_DEPENDS+=	opendal-service-oss-0.58.1
CARGO_CRATE_DEPENDS+=	opendal-service-redis-0.58.1
CARGO_CRATE_DEPENDS+=	opendal-service-s3-0.58.1
CARGO_CRATE_DEPENDS+=	opendal-service-webdav-0.58.1
CARGO_CRATE_DEPENDS+=	openssl-0.10.75
CARGO_CRATE_DEPENDS+=	openssl-macros-0.1.1
CARGO_CRATE_DEPENDS+=	openssl-probe-0.1.5
CARGO_CRATE_DEPENDS+=	openssl-probe-0.2.1
CARGO_CRATE_DEPENDS+=	openssl-src-300.5.4+3.5.4
CARGO_CRATE_DEPENDS+=	openssl-sys-0.9.111
CARGO_CRATE_DEPENDS+=	option-ext-0.2.0
CARGO_CRATE_DEPENDS+=	ordered-multimap-0.7.0
CARGO_CRATE_DEPENDS+=	overload-0.1.1
CARGO_CRATE_DEPENDS+=	parking-2.2.1
CARGO_CRATE_DEPENDS+=	parking_lot-0.12.1
CARGO_CRATE_DEPENDS+=	parking_lot_core-0.9.9
CARGO_CRATE_DEPENDS+=	paste-1.0.15
CARGO_CRATE_DEPENDS+=	pbkdf2-0.12.2
CARGO_CRATE_DEPENDS+=	pem-3.0.3
CARGO_CRATE_DEPENDS+=	pem-4.0.0
CARGO_CRATE_DEPENDS+=	pem-rfc7468-0.7.0
CARGO_CRATE_DEPENDS+=	percent-encoding-2.3.1
CARGO_CRATE_DEPENDS+=	pin-project-1.1.7
CARGO_CRATE_DEPENDS+=	pin-project-internal-1.1.7
CARGO_CRATE_DEPENDS+=	pin-project-lite-0.2.13
CARGO_CRATE_DEPENDS+=	pin-utils-0.1.0
CARGO_CRATE_DEPENDS+=	pkcs1-0.7.5
CARGO_CRATE_DEPENDS+=	pkcs5-0.7.1
CARGO_CRATE_DEPENDS+=	pkcs8-0.10.2
CARGO_CRATE_DEPENDS+=	pkg-config-0.3.28
CARGO_CRATE_DEPENDS+=	portable-atomic-1.14.0
CARGO_CRATE_DEPENDS+=	portable-atomic-util-0.2.4
CARGO_CRATE_DEPENDS+=	potential_utf-0.1.4
CARGO_CRATE_DEPENDS+=	powerfmt-0.2.0
CARGO_CRATE_DEPENDS+=	ppv-lite86-0.2.17
CARGO_CRATE_DEPENDS+=	predicates-3.1.0
CARGO_CRATE_DEPENDS+=	predicates-core-1.0.6
CARGO_CRATE_DEPENDS+=	predicates-tree-1.0.9
CARGO_CRATE_DEPENDS+=	prefix-trie-0.8.4
CARGO_CRATE_DEPENDS+=	proc-macro-crate-3.4.0
CARGO_CRATE_DEPENDS+=	proc-macro2-1.0.107
CARGO_CRATE_DEPENDS+=	prost-0.14.4
CARGO_CRATE_DEPENDS+=	prost-derive-0.14.4
CARGO_CRATE_DEPENDS+=	quick-error-1.2.3
CARGO_CRATE_DEPENDS+=	quick-xml-0.41.0
CARGO_CRATE_DEPENDS+=	quinn-0.11.11
CARGO_CRATE_DEPENDS+=	quinn-proto-0.11.16
CARGO_CRATE_DEPENDS+=	quinn-udp-0.5.15
CARGO_CRATE_DEPENDS+=	quote-1.0.47
CARGO_CRATE_DEPENDS+=	r-efi-5.3.0
CARGO_CRATE_DEPENDS+=	r-efi-6.0.0
CARGO_CRATE_DEPENDS+=	rand-0.8.5
CARGO_CRATE_DEPENDS+=	rand-0.10.2
CARGO_CRATE_DEPENDS+=	rand_chacha-0.3.1
CARGO_CRATE_DEPENDS+=	rand_core-0.6.4
CARGO_CRATE_DEPENDS+=	rand_core-0.10.1
CARGO_CRATE_DEPENDS+=	rand_pcg-0.10.2
CARGO_CRATE_DEPENDS+=	redis-1.5.0
CARGO_CRATE_DEPENDS+=	redox_syscall-0.4.1
CARGO_CRATE_DEPENDS+=	redox_users-0.5.2
CARGO_CRATE_DEPENDS+=	regex-1.13.1
CARGO_CRATE_DEPENDS+=	regex-automata-0.1.10
CARGO_CRATE_DEPENDS+=	regex-automata-0.4.18
CARGO_CRATE_DEPENDS+=	regex-lite-0.1.8
CARGO_CRATE_DEPENDS+=	regex-syntax-0.6.29
CARGO_CRATE_DEPENDS+=	regex-syntax-0.8.11
CARGO_CRATE_DEPENDS+=	reqsign-aliyun-oss-3.1.4
CARGO_CRATE_DEPENDS+=	reqsign-aws-core-3.1.0
CARGO_CRATE_DEPENDS+=	reqsign-aws-v4-3.2.0
CARGO_CRATE_DEPENDS+=	reqsign-azure-storage-3.2.0
CARGO_CRATE_DEPENDS+=	reqsign-core-3.3.0
CARGO_CRATE_DEPENDS+=	reqsign-file-read-tokio-3.0.5
CARGO_CRATE_DEPENDS+=	reqsign-google-3.1.0
CARGO_CRATE_DEPENDS+=	reqsign-tencent-cos-3.0.5
CARGO_CRATE_DEPENDS+=	reqwest-0.12.28
CARGO_CRATE_DEPENDS+=	reqwest-0.13.4
CARGO_CRATE_DEPENDS+=	resolv-conf-0.7.6
CARGO_CRATE_DEPENDS+=	ring-0.17.13
CARGO_CRATE_DEPENDS+=	rouille-3.6.2
CARGO_CRATE_DEPENDS+=	rsa-0.9.10
CARGO_CRATE_DEPENDS+=	rust-ini-0.21.0
CARGO_CRATE_DEPENDS+=	rustc-demangle-0.1.26
CARGO_CRATE_DEPENDS+=	rustc-hash-2.0.0
CARGO_CRATE_DEPENDS+=	rustc_version-0.4.0
CARGO_CRATE_DEPENDS+=	rustix-0.38.44
CARGO_CRATE_DEPENDS+=	rustls-0.23.31
CARGO_CRATE_DEPENDS+=	rustls-native-certs-0.8.1
CARGO_CRATE_DEPENDS+=	rustls-pki-types-1.12.0
CARGO_CRATE_DEPENDS+=	rustls-platform-verifier-0.7.0
CARGO_CRATE_DEPENDS+=	rustls-platform-verifier-android-0.1.1
CARGO_CRATE_DEPENDS+=	rustls-webpki-0.103.4
CARGO_CRATE_DEPENDS+=	rustversion-1.0.22
CARGO_CRATE_DEPENDS+=	ruzstd-0.8.1
CARGO_CRATE_DEPENDS+=	ryu-1.0.16
CARGO_CRATE_DEPENDS+=	safemem-0.3.3
CARGO_CRATE_DEPENDS+=	salsa20-0.10.2
CARGO_CRATE_DEPENDS+=	same-file-1.0.6
CARGO_CRATE_DEPENDS+=	scc-2.1.0
CARGO_CRATE_DEPENDS+=	schannel-0.1.29
CARGO_CRATE_DEPENDS+=	scoped-tls-1.0.1
CARGO_CRATE_DEPENDS+=	scopeguard-1.2.0
CARGO_CRATE_DEPENDS+=	scrypt-0.11.0
CARGO_CRATE_DEPENDS+=	sdd-0.2.0
CARGO_CRATE_DEPENDS+=	security-framework-3.5.1
CARGO_CRATE_DEPENDS+=	security-framework-sys-2.15.0
CARGO_CRATE_DEPENDS+=	semver-1.0.23
CARGO_CRATE_DEPENDS+=	serde-1.0.228
CARGO_CRATE_DEPENDS+=	serde_core-1.0.228
CARGO_CRATE_DEPENDS+=	serde_derive-1.0.228
CARGO_CRATE_DEPENDS+=	serde_json-1.0.146
CARGO_CRATE_DEPENDS+=	serde_repr-0.1.20
CARGO_CRATE_DEPENDS+=	serde_spanned-1.0.4
CARGO_CRATE_DEPENDS+=	serde_urlencoded-0.7.1
CARGO_CRATE_DEPENDS+=	serial_test-3.1.0
CARGO_CRATE_DEPENDS+=	serial_test_derive-3.1.0
CARGO_CRATE_DEPENDS+=	sha1-0.11.0
CARGO_CRATE_DEPENDS+=	sha1_smol-1.0.0
CARGO_CRATE_DEPENDS+=	sha2-0.10.8
CARGO_CRATE_DEPENDS+=	sha2-0.11.0
CARGO_CRATE_DEPENDS+=	sharded-slab-0.1.7
CARGO_CRATE_DEPENDS+=	shlex-2.0.1
CARGO_CRATE_DEPENDS+=	signal-hook-registry-1.4.1
CARGO_CRATE_DEPENDS+=	signature-2.2.0
CARGO_CRATE_DEPENDS+=	simd-adler32-0.3.7
CARGO_CRATE_DEPENDS+=	simd_cesu8-1.2.0
CARGO_CRATE_DEPENDS+=	simdutf8-0.1.5
CARGO_CRATE_DEPENDS+=	simple_asn1-0.6.2
CARGO_CRATE_DEPENDS+=	simplelog-0.12.2
CARGO_CRATE_DEPENDS+=	slab-0.4.12
CARGO_CRATE_DEPENDS+=	smallvec-1.15.1
CARGO_CRATE_DEPENDS+=	socket2-0.5.5
CARGO_CRATE_DEPENDS+=	socket2-0.6.5
CARGO_CRATE_DEPENDS+=	spin-0.9.8
CARGO_CRATE_DEPENDS+=	spin-0.10.1
CARGO_CRATE_DEPENDS+=	spki-0.7.3
CARGO_CRATE_DEPENDS+=	stable_deref_trait-1.2.1
CARGO_CRATE_DEPENDS+=	statrs-0.18.0
CARGO_CRATE_DEPENDS+=	stringmatch-0.4.0
CARGO_CRATE_DEPENDS+=	strip-ansi-escapes-0.2.0
CARGO_CRATE_DEPENDS+=	strsim-0.11.1
CARGO_CRATE_DEPENDS+=	subtle-2.5.0
CARGO_CRATE_DEPENDS+=	syn-2.0.119
CARGO_CRATE_DEPENDS+=	syn-3.0.3
CARGO_CRATE_DEPENDS+=	sync_wrapper-1.0.1
CARGO_CRATE_DEPENDS+=	synstructure-0.13.2
CARGO_CRATE_DEPENDS+=	syslog-7.0.0
CARGO_CRATE_DEPENDS+=	system-configuration-0.6.1
CARGO_CRATE_DEPENDS+=	system-configuration-0.7.0
CARGO_CRATE_DEPENDS+=	system-configuration-sys-0.6.0
CARGO_CRATE_DEPENDS+=	tagptr-0.2.0
CARGO_CRATE_DEPENDS+=	tar-0.4.40
CARGO_CRATE_DEPENDS+=	temp-env-0.3.6
CARGO_CRATE_DEPENDS+=	tempfile-3.10.1
CARGO_CRATE_DEPENDS+=	termcolor-1.4.0
CARGO_CRATE_DEPENDS+=	terminal_size-0.3.0
CARGO_CRATE_DEPENDS+=	termtree-0.4.1
CARGO_CRATE_DEPENDS+=	test-case-3.3.1
CARGO_CRATE_DEPENDS+=	test-case-core-3.3.1
CARGO_CRATE_DEPENDS+=	test-case-macros-3.3.1
CARGO_CRATE_DEPENDS+=	thirtyfour-0.36.1
CARGO_CRATE_DEPENDS+=	thirtyfour-macros-0.2.0
CARGO_CRATE_DEPENDS+=	thiserror-1.0.56
CARGO_CRATE_DEPENDS+=	thiserror-2.0.17
CARGO_CRATE_DEPENDS+=	thiserror-impl-1.0.56
CARGO_CRATE_DEPENDS+=	thiserror-impl-2.0.17
CARGO_CRATE_DEPENDS+=	thread_local-1.1.10
CARGO_CRATE_DEPENDS+=	threadpool-1.8.1
CARGO_CRATE_DEPENDS+=	time-0.3.36
CARGO_CRATE_DEPENDS+=	time-core-0.1.2
CARGO_CRATE_DEPENDS+=	time-macros-0.2.18
CARGO_CRATE_DEPENDS+=	tiny-keccak-2.0.2
CARGO_CRATE_DEPENDS+=	tiny_http-0.12.0
CARGO_CRATE_DEPENDS+=	tinystr-0.8.2
CARGO_CRATE_DEPENDS+=	tinyvec-1.6.0
CARGO_CRATE_DEPENDS+=	tinyvec_macros-0.1.1
CARGO_CRATE_DEPENDS+=	tokio-1.53.1
CARGO_CRATE_DEPENDS+=	tokio-macros-2.7.2
CARGO_CRATE_DEPENDS+=	tokio-native-tls-0.3.1
CARGO_CRATE_DEPENDS+=	tokio-rustls-0.26.0
CARGO_CRATE_DEPENDS+=	tokio-serde-0.9.0
CARGO_CRATE_DEPENDS+=	tokio-util-0.7.18
CARGO_CRATE_DEPENDS+=	toml-0.9.11+spec-1.1.0
CARGO_CRATE_DEPENDS+=	toml_datetime-0.7.5+spec-1.1.0
CARGO_CRATE_DEPENDS+=	toml_edit-0.23.10+spec-1.0.0
CARGO_CRATE_DEPENDS+=	toml_parser-1.0.6+spec-1.1.0
CARGO_CRATE_DEPENDS+=	toml_writer-1.0.6+spec-1.1.0
CARGO_CRATE_DEPENDS+=	tower-0.5.3
CARGO_CRATE_DEPENDS+=	tower-http-0.6.8
CARGO_CRATE_DEPENDS+=	tower-layer-0.3.3
CARGO_CRATE_DEPENDS+=	tower-service-0.3.3
CARGO_CRATE_DEPENDS+=	tracing-0.1.40
CARGO_CRATE_DEPENDS+=	tracing-attributes-0.1.27
CARGO_CRATE_DEPENDS+=	tracing-core-0.1.32
CARGO_CRATE_DEPENDS+=	tracing-log-0.2.0
CARGO_CRATE_DEPENDS+=	tracing-subscriber-0.3.18
CARGO_CRATE_DEPENDS+=	trim-in-place-0.1.7
CARGO_CRATE_DEPENDS+=	try-lock-0.2.5
CARGO_CRATE_DEPENDS+=	twoway-0.1.8
CARGO_CRATE_DEPENDS+=	twox-hash-2.1.1
CARGO_CRATE_DEPENDS+=	typed-path-0.12.0
CARGO_CRATE_DEPENDS+=	typenum-1.20.1
CARGO_CRATE_DEPENDS+=	unicase-2.7.0
CARGO_CRATE_DEPENDS+=	unicode-ident-1.0.24
CARGO_CRATE_DEPENDS+=	unicode-xid-0.2.6
CARGO_CRATE_DEPENDS+=	untrusted-0.9.0
CARGO_CRATE_DEPENDS+=	url-2.5.4
CARGO_CRATE_DEPENDS+=	utf8_iter-1.0.4
CARGO_CRATE_DEPENDS+=	utf8parse-0.2.1
CARGO_CRATE_DEPENDS+=	uuid-1.18.1
CARGO_CRATE_DEPENDS+=	valuable-0.1.1
CARGO_CRATE_DEPENDS+=	vcpkg-0.2.15
CARGO_CRATE_DEPENDS+=	version-compare-0.1.1
CARGO_CRATE_DEPENDS+=	version_check-0.9.4
CARGO_CRATE_DEPENDS+=	void-1.0.2
CARGO_CRATE_DEPENDS+=	vte-0.11.1
CARGO_CRATE_DEPENDS+=	vte_generate_state_changes-0.1.1
CARGO_CRATE_DEPENDS+=	wait-timeout-0.2.0
CARGO_CRATE_DEPENDS+=	walkdir-2.5.0
CARGO_CRATE_DEPENDS+=	want-0.3.1
CARGO_CRATE_DEPENDS+=	wasi-0.11.0+wasi-snapshot-preview1
CARGO_CRATE_DEPENDS+=	wasi-0.14.7+wasi-0.2.4
CARGO_CRATE_DEPENDS+=	wasip2-1.0.1+wasi-0.2.4
CARGO_CRATE_DEPENDS+=	wasm-bindgen-0.2.126
CARGO_CRATE_DEPENDS+=	wasm-bindgen-futures-0.4.76
CARGO_CRATE_DEPENDS+=	wasm-bindgen-macro-0.2.126
CARGO_CRATE_DEPENDS+=	wasm-bindgen-macro-support-0.2.126
CARGO_CRATE_DEPENDS+=	wasm-bindgen-shared-0.2.126
CARGO_CRATE_DEPENDS+=	wasm-streams-0.5.0
CARGO_CRATE_DEPENDS+=	web-sys-0.3.103
CARGO_CRATE_DEPENDS+=	web-time-1.1.0
CARGO_CRATE_DEPENDS+=	webpki-root-certs-1.0.9
CARGO_CRATE_DEPENDS+=	webpki-roots-0.26.2
CARGO_CRATE_DEPENDS+=	webpki-roots-1.0.5
CARGO_CRATE_DEPENDS+=	which-6.0.3
CARGO_CRATE_DEPENDS+=	widestring-1.2.1
CARGO_CRATE_DEPENDS+=	winapi-0.3.9
CARGO_CRATE_DEPENDS+=	winapi-i686-pc-windows-gnu-0.4.0
CARGO_CRATE_DEPENDS+=	winapi-util-0.1.8
CARGO_CRATE_DEPENDS+=	winapi-x86_64-pc-windows-gnu-0.4.0
CARGO_CRATE_DEPENDS+=	windows-core-0.52.0
CARGO_CRATE_DEPENDS+=	windows-link-0.1.3
CARGO_CRATE_DEPENDS+=	windows-link-0.2.1
CARGO_CRATE_DEPENDS+=	windows-registry-0.5.3
CARGO_CRATE_DEPENDS+=	windows-registry-0.6.1
CARGO_CRATE_DEPENDS+=	windows-result-0.3.4
CARGO_CRATE_DEPENDS+=	windows-result-0.4.1
CARGO_CRATE_DEPENDS+=	windows-strings-0.4.2
CARGO_CRATE_DEPENDS+=	windows-strings-0.5.1
CARGO_CRATE_DEPENDS+=	windows-sys-0.48.0
CARGO_CRATE_DEPENDS+=	windows-sys-0.52.0
CARGO_CRATE_DEPENDS+=	windows-sys-0.59.0
CARGO_CRATE_DEPENDS+=	windows-sys-0.61.1
CARGO_CRATE_DEPENDS+=	windows-targets-0.48.5
CARGO_CRATE_DEPENDS+=	windows-targets-0.52.6
CARGO_CRATE_DEPENDS+=	windows_aarch64_gnullvm-0.48.5
CARGO_CRATE_DEPENDS+=	windows_aarch64_gnullvm-0.52.6
CARGO_CRATE_DEPENDS+=	windows_aarch64_msvc-0.48.5
CARGO_CRATE_DEPENDS+=	windows_aarch64_msvc-0.52.6
CARGO_CRATE_DEPENDS+=	windows_i686_gnu-0.48.5
CARGO_CRATE_DEPENDS+=	windows_i686_gnu-0.52.6
CARGO_CRATE_DEPENDS+=	windows_i686_gnullvm-0.52.6
CARGO_CRATE_DEPENDS+=	windows_i686_msvc-0.48.5
CARGO_CRATE_DEPENDS+=	windows_i686_msvc-0.52.6
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnu-0.48.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnu-0.52.6
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnullvm-0.48.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnullvm-0.52.6
CARGO_CRATE_DEPENDS+=	windows_x86_64_msvc-0.48.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_msvc-0.52.6
CARGO_CRATE_DEPENDS+=	winnow-0.7.14
CARGO_CRATE_DEPENDS+=	winsafe-0.0.19
CARGO_CRATE_DEPENDS+=	wit-bindgen-0.46.0
CARGO_CRATE_DEPENDS+=	writeable-0.6.2
CARGO_CRATE_DEPENDS+=	xattr-1.2.0
CARGO_CRATE_DEPENDS+=	xxhash-rust-0.8.18
CARGO_CRATE_DEPENDS+=	yoke-0.8.1
CARGO_CRATE_DEPENDS+=	yoke-derive-0.8.1
CARGO_CRATE_DEPENDS+=	zerofrom-0.1.6
CARGO_CRATE_DEPENDS+=	zerofrom-derive-0.1.6
CARGO_CRATE_DEPENDS+=	zeroize-1.9.0
CARGO_CRATE_DEPENDS+=	zerotrie-0.2.3
CARGO_CRATE_DEPENDS+=	zerovec-0.11.5
CARGO_CRATE_DEPENDS+=	zerovec-derive-0.11.2
CARGO_CRATE_DEPENDS+=	zip-0.6.6
CARGO_CRATE_DEPENDS+=	zstd-0.13.1
CARGO_CRATE_DEPENDS+=	zstd-safe-7.1.0
CARGO_CRATE_DEPENDS+=	zstd-sys-2.0.10+zstd.1.5.6
