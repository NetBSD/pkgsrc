# $NetBSD: cargo-depends.mk,v 1.2 2024/02/10 21:43:54 pin Exp $

CARGO_CRATE_DEPENDS+=	addr2line-0.21.0
CARGO_CRATE_DEPENDS+=	adler-1.0.2
CARGO_CRATE_DEPENDS+=	ahash-0.8.7
CARGO_CRATE_DEPENDS+=	aho-corasick-1.1.2
CARGO_CRATE_DEPENDS+=	android-tzdata-0.1.1
CARGO_CRATE_DEPENDS+=	android_system_properties-0.1.5
CARGO_CRATE_DEPENDS+=	anstream-0.6.11
CARGO_CRATE_DEPENDS+=	anstyle-1.0.6
CARGO_CRATE_DEPENDS+=	anstyle-parse-0.2.3
CARGO_CRATE_DEPENDS+=	anstyle-query-1.0.2
CARGO_CRATE_DEPENDS+=	anstyle-wincon-3.0.2
CARGO_CRATE_DEPENDS+=	anyhow-1.0.79
CARGO_CRATE_DEPENDS+=	arc-swap-1.6.0
CARGO_CRATE_DEPENDS+=	arrayvec-0.5.2
CARGO_CRATE_DEPENDS+=	arrayvec-0.7.4
CARGO_CRATE_DEPENDS+=	autocfg-1.1.0
CARGO_CRATE_DEPENDS+=	backtrace-0.3.69
CARGO_CRATE_DEPENDS+=	bitflags-1.3.2
CARGO_CRATE_DEPENDS+=	bitflags-2.4.2
CARGO_CRATE_DEPENDS+=	bstr-1.9.0
CARGO_CRATE_DEPENDS+=	btoi-0.4.3
CARGO_CRATE_DEPENDS+=	bumpalo-3.14.0
CARGO_CRATE_DEPENDS+=	bytes-1.5.0
CARGO_CRATE_DEPENDS+=	bytesize-1.3.0
CARGO_CRATE_DEPENDS+=	camino-1.1.6
CARGO_CRATE_DEPENDS+=	cargo-platform-0.1.7
CARGO_CRATE_DEPENDS+=	cargo_metadata-0.18.1
CARGO_CRATE_DEPENDS+=	cc-1.0.83
CARGO_CRATE_DEPENDS+=	cfg-if-1.0.0
CARGO_CRATE_DEPENDS+=	chrono-0.4.33
CARGO_CRATE_DEPENDS+=	clap-4.5.0
CARGO_CRATE_DEPENDS+=	clap-help-1.1.1
CARGO_CRATE_DEPENDS+=	clap_builder-4.5.0
CARGO_CRATE_DEPENDS+=	clap_derive-4.5.0
CARGO_CRATE_DEPENDS+=	clap_lex-0.7.0
CARGO_CRATE_DEPENDS+=	cli-log-2.0.0
CARGO_CRATE_DEPENDS+=	clru-0.6.1
CARGO_CRATE_DEPENDS+=	colorchoice-1.0.0
CARGO_CRATE_DEPENDS+=	coolor-0.9.0
CARGO_CRATE_DEPENDS+=	core-foundation-sys-0.8.6
CARGO_CRATE_DEPENDS+=	crc32fast-1.3.2
CARGO_CRATE_DEPENDS+=	crokey-0.6.3
CARGO_CRATE_DEPENDS+=	crokey-proc_macros-0.6.3
CARGO_CRATE_DEPENDS+=	crossbeam-0.8.4
CARGO_CRATE_DEPENDS+=	crossbeam-channel-0.5.11
CARGO_CRATE_DEPENDS+=	crossbeam-deque-0.8.5
CARGO_CRATE_DEPENDS+=	crossbeam-epoch-0.9.18
CARGO_CRATE_DEPENDS+=	crossbeam-queue-0.3.11
CARGO_CRATE_DEPENDS+=	crossbeam-utils-0.8.19
CARGO_CRATE_DEPENDS+=	crossterm-0.27.0
CARGO_CRATE_DEPENDS+=	crossterm_winapi-0.9.1
CARGO_CRATE_DEPENDS+=	dashmap-5.5.3
CARGO_CRATE_DEPENDS+=	deranged-0.3.11
CARGO_CRATE_DEPENDS+=	directories-next-2.0.0
CARGO_CRATE_DEPENDS+=	dirs-sys-next-0.1.2
CARGO_CRATE_DEPENDS+=	dunce-1.0.4
CARGO_CRATE_DEPENDS+=	either-1.9.0
CARGO_CRATE_DEPENDS+=	encoding_rs-0.8.33
CARGO_CRATE_DEPENDS+=	equivalent-1.0.1
CARGO_CRATE_DEPENDS+=	errno-0.3.8
CARGO_CRATE_DEPENDS+=	faster-hex-0.9.0
CARGO_CRATE_DEPENDS+=	fastrand-2.0.1
CARGO_CRATE_DEPENDS+=	file-size-1.0.3
CARGO_CRATE_DEPENDS+=	filetime-0.2.23
CARGO_CRATE_DEPENDS+=	flate2-1.0.28
CARGO_CRATE_DEPENDS+=	form_urlencoded-1.2.1
CARGO_CRATE_DEPENDS+=	fsevent-sys-4.1.0
CARGO_CRATE_DEPENDS+=	getrandom-0.2.12
CARGO_CRATE_DEPENDS+=	gimli-0.28.1
CARGO_CRATE_DEPENDS+=	gix-0.58.0
CARGO_CRATE_DEPENDS+=	gix-actor-0.30.0
CARGO_CRATE_DEPENDS+=	gix-archive-0.9.0
CARGO_CRATE_DEPENDS+=	gix-attributes-0.22.0
CARGO_CRATE_DEPENDS+=	gix-bitmap-0.2.10
CARGO_CRATE_DEPENDS+=	gix-chunk-0.4.7
CARGO_CRATE_DEPENDS+=	gix-command-0.3.4
CARGO_CRATE_DEPENDS+=	gix-commitgraph-0.24.0
CARGO_CRATE_DEPENDS+=	gix-config-0.34.0
CARGO_CRATE_DEPENDS+=	gix-config-value-0.14.4
CARGO_CRATE_DEPENDS+=	gix-credentials-0.24.0
CARGO_CRATE_DEPENDS+=	gix-date-0.8.3
CARGO_CRATE_DEPENDS+=	gix-diff-0.40.0
CARGO_CRATE_DEPENDS+=	gix-discover-0.29.0
CARGO_CRATE_DEPENDS+=	gix-features-0.38.0
CARGO_CRATE_DEPENDS+=	gix-filter-0.9.0
CARGO_CRATE_DEPENDS+=	gix-fs-0.10.0
CARGO_CRATE_DEPENDS+=	gix-glob-0.16.0
CARGO_CRATE_DEPENDS+=	gix-hash-0.14.1
CARGO_CRATE_DEPENDS+=	gix-hashtable-0.5.1
CARGO_CRATE_DEPENDS+=	gix-ignore-0.11.0
CARGO_CRATE_DEPENDS+=	gix-index-0.29.0
CARGO_CRATE_DEPENDS+=	gix-lock-13.1.0
CARGO_CRATE_DEPENDS+=	gix-macros-0.1.3
CARGO_CRATE_DEPENDS+=	gix-mailmap-0.22.0
CARGO_CRATE_DEPENDS+=	gix-negotiate-0.12.0
CARGO_CRATE_DEPENDS+=	gix-object-0.41.0
CARGO_CRATE_DEPENDS+=	gix-odb-0.57.0
CARGO_CRATE_DEPENDS+=	gix-pack-0.47.0
CARGO_CRATE_DEPENDS+=	gix-packetline-blocking-0.17.3
CARGO_CRATE_DEPENDS+=	gix-path-0.10.5
CARGO_CRATE_DEPENDS+=	gix-pathspec-0.6.0
CARGO_CRATE_DEPENDS+=	gix-prompt-0.8.2
CARGO_CRATE_DEPENDS+=	gix-quote-0.4.10
CARGO_CRATE_DEPENDS+=	gix-ref-0.41.0
CARGO_CRATE_DEPENDS+=	gix-refspec-0.22.0
CARGO_CRATE_DEPENDS+=	gix-revision-0.26.0
CARGO_CRATE_DEPENDS+=	gix-revwalk-0.12.0
CARGO_CRATE_DEPENDS+=	gix-sec-0.10.4
CARGO_CRATE_DEPENDS+=	gix-status-0.5.0
CARGO_CRATE_DEPENDS+=	gix-submodule-0.8.0
CARGO_CRATE_DEPENDS+=	gix-tempfile-13.1.0
CARGO_CRATE_DEPENDS+=	gix-trace-0.1.7
CARGO_CRATE_DEPENDS+=	gix-traverse-0.37.0
CARGO_CRATE_DEPENDS+=	gix-url-0.27.0
CARGO_CRATE_DEPENDS+=	gix-utils-0.1.9
CARGO_CRATE_DEPENDS+=	gix-validate-0.8.3
CARGO_CRATE_DEPENDS+=	gix-worktree-0.30.0
CARGO_CRATE_DEPENDS+=	gix-worktree-state-0.7.0
CARGO_CRATE_DEPENDS+=	gix-worktree-stream-0.9.0
CARGO_CRATE_DEPENDS+=	hashbrown-0.12.3
CARGO_CRATE_DEPENDS+=	hashbrown-0.14.3
CARGO_CRATE_DEPENDS+=	heck-0.4.1
CARGO_CRATE_DEPENDS+=	home-0.5.9
CARGO_CRATE_DEPENDS+=	human_format-1.0.3
CARGO_CRATE_DEPENDS+=	iana-time-zone-0.1.60
CARGO_CRATE_DEPENDS+=	iana-time-zone-haiku-0.1.2
CARGO_CRATE_DEPENDS+=	idna-0.5.0
CARGO_CRATE_DEPENDS+=	imara-diff-0.1.5
CARGO_CRATE_DEPENDS+=	indexmap-2.2.2
CARGO_CRATE_DEPENDS+=	inotify-0.9.6
CARGO_CRATE_DEPENDS+=	inotify-sys-0.1.5
CARGO_CRATE_DEPENDS+=	io-close-0.3.7
CARGO_CRATE_DEPENDS+=	itoa-1.0.10
CARGO_CRATE_DEPENDS+=	js-sys-0.3.68
CARGO_CRATE_DEPENDS+=	jwalk-0.8.1
CARGO_CRATE_DEPENDS+=	kqueue-1.0.8
CARGO_CRATE_DEPENDS+=	kqueue-sys-1.0.4
CARGO_CRATE_DEPENDS+=	kstring-2.0.0
CARGO_CRATE_DEPENDS+=	lazy-regex-3.1.0
CARGO_CRATE_DEPENDS+=	lazy-regex-proc_macros-3.1.0
CARGO_CRATE_DEPENDS+=	libc-0.2.153
CARGO_CRATE_DEPENDS+=	libredox-0.0.1
CARGO_CRATE_DEPENDS+=	linux-raw-sys-0.4.13
CARGO_CRATE_DEPENDS+=	lock_api-0.4.11
CARGO_CRATE_DEPENDS+=	log-0.4.20
CARGO_CRATE_DEPENDS+=	memchr-2.7.1
CARGO_CRATE_DEPENDS+=	memmap2-0.9.4
CARGO_CRATE_DEPENDS+=	memoffset-0.6.5
CARGO_CRATE_DEPENDS+=	minimad-0.13.0
CARGO_CRATE_DEPENDS+=	miniz_oxide-0.7.2
CARGO_CRATE_DEPENDS+=	mio-0.8.10
CARGO_CRATE_DEPENDS+=	nix-0.22.3
CARGO_CRATE_DEPENDS+=	notify-5.2.0
CARGO_CRATE_DEPENDS+=	num-conv-0.1.0
CARGO_CRATE_DEPENDS+=	num-traits-0.2.18
CARGO_CRATE_DEPENDS+=	num_threads-0.1.6
CARGO_CRATE_DEPENDS+=	object-0.32.2
CARGO_CRATE_DEPENDS+=	once_cell-1.19.0
CARGO_CRATE_DEPENDS+=	parking_lot-0.12.1
CARGO_CRATE_DEPENDS+=	parking_lot_core-0.9.9
CARGO_CRATE_DEPENDS+=	percent-encoding-2.3.1
CARGO_CRATE_DEPENDS+=	pin-project-lite-0.2.13
CARGO_CRATE_DEPENDS+=	powerfmt-0.2.0
CARGO_CRATE_DEPENDS+=	proc-macro2-1.0.78
CARGO_CRATE_DEPENDS+=	proc-status-0.1.1
CARGO_CRATE_DEPENDS+=	prodash-28.0.0
CARGO_CRATE_DEPENDS+=	quote-1.0.35
CARGO_CRATE_DEPENDS+=	rayon-1.8.1
CARGO_CRATE_DEPENDS+=	rayon-core-1.12.1
CARGO_CRATE_DEPENDS+=	redox_syscall-0.4.1
CARGO_CRATE_DEPENDS+=	redox_users-0.4.4
CARGO_CRATE_DEPENDS+=	regex-1.10.3
CARGO_CRATE_DEPENDS+=	regex-automata-0.4.5
CARGO_CRATE_DEPENDS+=	regex-syntax-0.8.2
CARGO_CRATE_DEPENDS+=	rustc-demangle-0.1.23
CARGO_CRATE_DEPENDS+=	rustix-0.38.31
CARGO_CRATE_DEPENDS+=	ryu-1.0.16
CARGO_CRATE_DEPENDS+=	same-file-1.0.6
CARGO_CRATE_DEPENDS+=	scopeguard-1.2.0
CARGO_CRATE_DEPENDS+=	semver-1.0.21
CARGO_CRATE_DEPENDS+=	serde-1.0.196
CARGO_CRATE_DEPENDS+=	serde_derive-1.0.196
CARGO_CRATE_DEPENDS+=	serde_json-1.0.113
CARGO_CRATE_DEPENDS+=	serde_spanned-0.6.5
CARGO_CRATE_DEPENDS+=	sha1_smol-1.0.0
CARGO_CRATE_DEPENDS+=	shell-words-1.1.0
CARGO_CRATE_DEPENDS+=	signal-hook-0.3.17
CARGO_CRATE_DEPENDS+=	signal-hook-mio-0.2.3
CARGO_CRATE_DEPENDS+=	signal-hook-registry-1.4.1
CARGO_CRATE_DEPENDS+=	smallvec-1.13.1
CARGO_CRATE_DEPENDS+=	socket2-0.5.5
CARGO_CRATE_DEPENDS+=	static_assertions-1.1.0
CARGO_CRATE_DEPENDS+=	strict-0.2.0
CARGO_CRATE_DEPENDS+=	strsim-0.11.0
CARGO_CRATE_DEPENDS+=	syn-1.0.109
CARGO_CRATE_DEPENDS+=	syn-2.0.48
CARGO_CRATE_DEPENDS+=	tempfile-3.10.0
CARGO_CRATE_DEPENDS+=	termimad-0.29.0
CARGO_CRATE_DEPENDS+=	terminal-light-1.2.0
CARGO_CRATE_DEPENDS+=	thiserror-1.0.56
CARGO_CRATE_DEPENDS+=	thiserror-impl-1.0.56
CARGO_CRATE_DEPENDS+=	time-0.3.34
CARGO_CRATE_DEPENDS+=	time-core-0.1.2
CARGO_CRATE_DEPENDS+=	time-macros-0.2.17
CARGO_CRATE_DEPENDS+=	tinyvec-1.6.0
CARGO_CRATE_DEPENDS+=	tinyvec_macros-0.1.1
CARGO_CRATE_DEPENDS+=	tokio-1.36.0
CARGO_CRATE_DEPENDS+=	tokio-macros-2.2.0
CARGO_CRATE_DEPENDS+=	toml-0.7.8
CARGO_CRATE_DEPENDS+=	toml_datetime-0.6.5
CARGO_CRATE_DEPENDS+=	toml_edit-0.19.15
CARGO_CRATE_DEPENDS+=	uluru-3.0.0
CARGO_CRATE_DEPENDS+=	unicode-bidi-0.3.15
CARGO_CRATE_DEPENDS+=	unicode-bom-2.0.3
CARGO_CRATE_DEPENDS+=	unicode-ident-1.0.12
CARGO_CRATE_DEPENDS+=	unicode-normalization-0.1.22
CARGO_CRATE_DEPENDS+=	unicode-width-0.1.11
CARGO_CRATE_DEPENDS+=	url-2.5.0
CARGO_CRATE_DEPENDS+=	utf8parse-0.2.1
CARGO_CRATE_DEPENDS+=	version_check-0.9.4
CARGO_CRATE_DEPENDS+=	vte-0.8.0
CARGO_CRATE_DEPENDS+=	vte_generate_state_changes-0.1.1
CARGO_CRATE_DEPENDS+=	walkdir-2.4.0
CARGO_CRATE_DEPENDS+=	wasi-0.11.0+wasi-snapshot-preview1
CARGO_CRATE_DEPENDS+=	wasm-bindgen-0.2.91
CARGO_CRATE_DEPENDS+=	wasm-bindgen-backend-0.2.91
CARGO_CRATE_DEPENDS+=	wasm-bindgen-macro-0.2.91
CARGO_CRATE_DEPENDS+=	wasm-bindgen-macro-support-0.2.91
CARGO_CRATE_DEPENDS+=	wasm-bindgen-shared-0.2.91
CARGO_CRATE_DEPENDS+=	winapi-0.3.9
CARGO_CRATE_DEPENDS+=	winapi-i686-pc-windows-gnu-0.4.0
CARGO_CRATE_DEPENDS+=	winapi-util-0.1.6
CARGO_CRATE_DEPENDS+=	winapi-x86_64-pc-windows-gnu-0.4.0
CARGO_CRATE_DEPENDS+=	windows-core-0.52.0
CARGO_CRATE_DEPENDS+=	windows-sys-0.45.0
CARGO_CRATE_DEPENDS+=	windows-sys-0.48.0
CARGO_CRATE_DEPENDS+=	windows-sys-0.52.0
CARGO_CRATE_DEPENDS+=	windows-targets-0.42.2
CARGO_CRATE_DEPENDS+=	windows-targets-0.48.5
CARGO_CRATE_DEPENDS+=	windows-targets-0.52.0
CARGO_CRATE_DEPENDS+=	windows_aarch64_gnullvm-0.42.2
CARGO_CRATE_DEPENDS+=	windows_aarch64_gnullvm-0.48.5
CARGO_CRATE_DEPENDS+=	windows_aarch64_gnullvm-0.52.0
CARGO_CRATE_DEPENDS+=	windows_aarch64_msvc-0.42.2
CARGO_CRATE_DEPENDS+=	windows_aarch64_msvc-0.48.5
CARGO_CRATE_DEPENDS+=	windows_aarch64_msvc-0.52.0
CARGO_CRATE_DEPENDS+=	windows_i686_gnu-0.42.2
CARGO_CRATE_DEPENDS+=	windows_i686_gnu-0.48.5
CARGO_CRATE_DEPENDS+=	windows_i686_gnu-0.52.0
CARGO_CRATE_DEPENDS+=	windows_i686_msvc-0.42.2
CARGO_CRATE_DEPENDS+=	windows_i686_msvc-0.48.5
CARGO_CRATE_DEPENDS+=	windows_i686_msvc-0.52.0
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnu-0.42.2
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnu-0.48.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnu-0.52.0
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnullvm-0.42.2
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnullvm-0.48.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnullvm-0.52.0
CARGO_CRATE_DEPENDS+=	windows_x86_64_msvc-0.42.2
CARGO_CRATE_DEPENDS+=	windows_x86_64_msvc-0.48.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_msvc-0.52.0
CARGO_CRATE_DEPENDS+=	winnow-0.5.39
CARGO_CRATE_DEPENDS+=	xterm-query-0.2.0
CARGO_CRATE_DEPENDS+=	zerocopy-0.7.32
CARGO_CRATE_DEPENDS+=	zerocopy-derive-0.7.32
