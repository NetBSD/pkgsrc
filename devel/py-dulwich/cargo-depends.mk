# $NetBSD: cargo-depends.mk,v 1.5 2024/12/11 12:29:46 adam Exp $

CARGO_CRATE_DEPENDS+=	autocfg-1.4.0
CARGO_CRATE_DEPENDS+=	cfg-if-1.0.0
CARGO_CRATE_DEPENDS+=	heck-0.5.0
CARGO_CRATE_DEPENDS+=	indoc-2.0.5
CARGO_CRATE_DEPENDS+=	libc-0.2.162
CARGO_CRATE_DEPENDS+=	memchr-2.7.4
CARGO_CRATE_DEPENDS+=	memoffset-0.9.1
CARGO_CRATE_DEPENDS+=	once_cell-1.20.2
CARGO_CRATE_DEPENDS+=	portable-atomic-1.9.0
CARGO_CRATE_DEPENDS+=	proc-macro2-1.0.89
CARGO_CRATE_DEPENDS+=	pyo3-0.23.0
CARGO_CRATE_DEPENDS+=	pyo3-build-config-0.23.0
CARGO_CRATE_DEPENDS+=	pyo3-ffi-0.23.0
CARGO_CRATE_DEPENDS+=	pyo3-macros-0.23.0
CARGO_CRATE_DEPENDS+=	pyo3-macros-backend-0.23.0
CARGO_CRATE_DEPENDS+=	quote-1.0.37
CARGO_CRATE_DEPENDS+=	syn-2.0.87
CARGO_CRATE_DEPENDS+=	target-lexicon-0.12.16
CARGO_CRATE_DEPENDS+=	unicode-ident-1.0.13
CARGO_CRATE_DEPENDS+=	unindent-0.2.3
