# $NetBSD: cargo-depends.mk,v 1.5 2024/08/22 12:14:51 pin Exp $

CARGO_CRATE_DEPENDS+=	accessory-1.3.0
CARGO_CRATE_DEPENDS+=	addr2line-0.21.0
CARGO_CRATE_DEPENDS+=	adler-1.0.2
CARGO_CRATE_DEPENDS+=	aead-0.5.2
CARGO_CRATE_DEPENDS+=	aes-0.8.4
CARGO_CRATE_DEPENDS+=	ahash-0.8.11
CARGO_CRATE_DEPENDS+=	aho-corasick-1.1.3
CARGO_CRATE_DEPENDS+=	allocator-api2-0.2.18
CARGO_CRATE_DEPENDS+=	android-tzdata-0.1.1
CARGO_CRATE_DEPENDS+=	android_system_properties-0.1.5
CARGO_CRATE_DEPENDS+=	anstream-0.3.2
CARGO_CRATE_DEPENDS+=	anstyle-1.0.6
CARGO_CRATE_DEPENDS+=	anstyle-parse-0.2.3
CARGO_CRATE_DEPENDS+=	anstyle-query-1.0.2
CARGO_CRATE_DEPENDS+=	anstyle-wincon-1.0.2
CARGO_CRATE_DEPENDS+=	anyhow-1.0.82
CARGO_CRATE_DEPENDS+=	anymap2-0.13.0
CARGO_CRATE_DEPENDS+=	aquamarine-0.5.0
CARGO_CRATE_DEPENDS+=	arboard-3.3.2
CARGO_CRATE_DEPENDS+=	arrayref-0.3.7
CARGO_CRATE_DEPENDS+=	arrayvec-0.7.4
CARGO_CRATE_DEPENDS+=	as_variant-1.2.0
CARGO_CRATE_DEPENDS+=	assign-1.1.1
CARGO_CRATE_DEPENDS+=	async-broadcast-0.5.1
CARGO_CRATE_DEPENDS+=	async-channel-2.2.1
CARGO_CRATE_DEPENDS+=	async-executor-1.8.0
CARGO_CRATE_DEPENDS+=	async-fs-1.6.0
CARGO_CRATE_DEPENDS+=	async-io-1.13.0
CARGO_CRATE_DEPENDS+=	async-io-2.3.2
CARGO_CRATE_DEPENDS+=	async-lock-2.8.0
CARGO_CRATE_DEPENDS+=	async-lock-3.3.0
CARGO_CRATE_DEPENDS+=	async-process-1.8.1
CARGO_CRATE_DEPENDS+=	async-recursion-1.1.0
CARGO_CRATE_DEPENDS+=	async-signal-0.2.5
CARGO_CRATE_DEPENDS+=	async-stream-0.3.5
CARGO_CRATE_DEPENDS+=	async-stream-impl-0.3.5
CARGO_CRATE_DEPENDS+=	async-task-4.7.0
CARGO_CRATE_DEPENDS+=	async-trait-0.1.80
CARGO_CRATE_DEPENDS+=	atomic-waker-1.1.2
CARGO_CRATE_DEPENDS+=	autocfg-1.2.0
CARGO_CRATE_DEPENDS+=	backoff-0.4.0
CARGO_CRATE_DEPENDS+=	backtrace-0.3.71
CARGO_CRATE_DEPENDS+=	base64-0.21.7
CARGO_CRATE_DEPENDS+=	base64ct-1.6.0
CARGO_CRATE_DEPENDS+=	bit_field-0.10.2
CARGO_CRATE_DEPENDS+=	bitflags-1.3.2
CARGO_CRATE_DEPENDS+=	bitflags-2.5.0
CARGO_CRATE_DEPENDS+=	bitmaps-3.2.1
CARGO_CRATE_DEPENDS+=	blake3-1.5.1
CARGO_CRATE_DEPENDS+=	block-0.1.6
CARGO_CRATE_DEPENDS+=	block-buffer-0.10.4
CARGO_CRATE_DEPENDS+=	block-padding-0.3.3
CARGO_CRATE_DEPENDS+=	blocking-1.5.1
CARGO_CRATE_DEPENDS+=	bs58-0.5.1
CARGO_CRATE_DEPENDS+=	bumpalo-3.16.0
CARGO_CRATE_DEPENDS+=	bytemuck-1.15.0
CARGO_CRATE_DEPENDS+=	byteorder-1.5.0
CARGO_CRATE_DEPENDS+=	bytes-1.6.0
CARGO_CRATE_DEPENDS+=	bytesize-1.3.0
CARGO_CRATE_DEPENDS+=	cassowary-0.3.0
CARGO_CRATE_DEPENDS+=	castaway-0.2.2
CARGO_CRATE_DEPENDS+=	cbc-0.1.2
CARGO_CRATE_DEPENDS+=	cc-1.0.94
CARGO_CRATE_DEPENDS+=	cfg-if-1.0.0
CARGO_CRATE_DEPENDS+=	cfg-vis-0.3.0
CARGO_CRATE_DEPENDS+=	cfg_aliases-0.1.1
CARGO_CRATE_DEPENDS+=	chacha20-0.9.1
CARGO_CRATE_DEPENDS+=	chacha20poly1305-0.10.1
CARGO_CRATE_DEPENDS+=	chrono-0.4.31
CARGO_CRATE_DEPENDS+=	cipher-0.4.4
CARGO_CRATE_DEPENDS+=	clap-4.3.24
CARGO_CRATE_DEPENDS+=	clap_builder-4.3.24
CARGO_CRATE_DEPENDS+=	clap_derive-4.3.12
CARGO_CRATE_DEPENDS+=	clap_lex-0.5.1
CARGO_CRATE_DEPENDS+=	clipboard-win-5.3.1
CARGO_CRATE_DEPENDS+=	color_quant-1.1.0
CARGO_CRATE_DEPENDS+=	colorchoice-1.0.0
CARGO_CRATE_DEPENDS+=	compact_str-0.7.1
CARGO_CRATE_DEPENDS+=	comrak-0.22.0
CARGO_CRATE_DEPENDS+=	concurrent-queue-2.4.0
CARGO_CRATE_DEPENDS+=	const-oid-0.9.6
CARGO_CRATE_DEPENDS+=	const_panic-0.2.8
CARGO_CRATE_DEPENDS+=	constant_time_eq-0.3.0
CARGO_CRATE_DEPENDS+=	convert_case-0.4.0
CARGO_CRATE_DEPENDS+=	core-foundation-0.9.4
CARGO_CRATE_DEPENDS+=	core-foundation-sys-0.8.6
CARGO_CRATE_DEPENDS+=	core-graphics-0.23.2
CARGO_CRATE_DEPENDS+=	core-graphics-types-0.1.3
CARGO_CRATE_DEPENDS+=	cpufeatures-0.2.12
CARGO_CRATE_DEPENDS+=	crc32fast-1.4.0
CARGO_CRATE_DEPENDS+=	crossbeam-channel-0.5.12
CARGO_CRATE_DEPENDS+=	crossbeam-deque-0.8.5
CARGO_CRATE_DEPENDS+=	crossbeam-epoch-0.9.18
CARGO_CRATE_DEPENDS+=	crossbeam-utils-0.8.19
CARGO_CRATE_DEPENDS+=	crossterm-0.27.0
CARGO_CRATE_DEPENDS+=	crossterm_winapi-0.9.1
CARGO_CRATE_DEPENDS+=	crunchy-0.2.2
CARGO_CRATE_DEPENDS+=	crypto-common-0.1.6
CARGO_CRATE_DEPENDS+=	css-color-parser-0.1.2
CARGO_CRATE_DEPENDS+=	ctr-0.9.2
CARGO_CRATE_DEPENDS+=	curve25519-dalek-4.1.2
CARGO_CRATE_DEPENDS+=	curve25519-dalek-derive-0.1.1
CARGO_CRATE_DEPENDS+=	darling-0.14.4
CARGO_CRATE_DEPENDS+=	darling_core-0.14.4
CARGO_CRATE_DEPENDS+=	darling_macro-0.14.4
CARGO_CRATE_DEPENDS+=	deadpool-0.10.0
CARGO_CRATE_DEPENDS+=	deadpool-runtime-0.1.3
CARGO_CRATE_DEPENDS+=	deadpool-sqlite-0.7.0
CARGO_CRATE_DEPENDS+=	deadpool-sync-0.1.2
CARGO_CRATE_DEPENDS+=	delegate-display-2.1.1
CARGO_CRATE_DEPENDS+=	der-0.7.9
CARGO_CRATE_DEPENDS+=	der_derive-0.7.2
CARGO_CRATE_DEPENDS+=	deranged-0.3.11
CARGO_CRATE_DEPENDS+=	derivative-2.2.0
CARGO_CRATE_DEPENDS+=	derive-new-0.6.0
CARGO_CRATE_DEPENDS+=	derive_builder-0.12.0
CARGO_CRATE_DEPENDS+=	derive_builder_core-0.12.0
CARGO_CRATE_DEPENDS+=	derive_builder_macro-0.12.0
CARGO_CRATE_DEPENDS+=	derive_more-0.99.17
CARGO_CRATE_DEPENDS+=	deunicode-1.4.4
CARGO_CRATE_DEPENDS+=	diff-0.1.13
CARGO_CRATE_DEPENDS+=	digest-0.10.7
CARGO_CRATE_DEPENDS+=	dirs-4.0.0
CARGO_CRATE_DEPENDS+=	dirs-next-2.0.0
CARGO_CRATE_DEPENDS+=	dirs-sys-0.3.7
CARGO_CRATE_DEPENDS+=	dirs-sys-next-0.1.2
CARGO_CRATE_DEPENDS+=	displaydoc-0.2.4
CARGO_CRATE_DEPENDS+=	dlib-0.5.2
CARGO_CRATE_DEPENDS+=	downcast-rs-1.2.1
CARGO_CRATE_DEPENDS+=	dyn-clone-1.0.14
CARGO_CRATE_DEPENDS+=	ed25519-2.2.3
CARGO_CRATE_DEPENDS+=	ed25519-dalek-2.1.1
CARGO_CRATE_DEPENDS+=	edit-0.1.4
CARGO_CRATE_DEPENDS+=	either-1.11.0
CARGO_CRATE_DEPENDS+=	emojis-0.5.3
CARGO_CRATE_DEPENDS+=	encoding_rs-0.8.34
CARGO_CRATE_DEPENDS+=	endian-type-0.1.2
CARGO_CRATE_DEPENDS+=	entities-1.0.1
CARGO_CRATE_DEPENDS+=	enumflags2-0.7.9
CARGO_CRATE_DEPENDS+=	enumflags2_derive-0.7.9
CARGO_CRATE_DEPENDS+=	equivalent-1.0.1
CARGO_CRATE_DEPENDS+=	errno-0.3.8
CARGO_CRATE_DEPENDS+=	error-code-3.2.0
CARGO_CRATE_DEPENDS+=	event-listener-2.5.3
CARGO_CRATE_DEPENDS+=	event-listener-3.1.0
CARGO_CRATE_DEPENDS+=	event-listener-4.0.3
CARGO_CRATE_DEPENDS+=	event-listener-5.3.0
CARGO_CRATE_DEPENDS+=	event-listener-strategy-0.4.0
CARGO_CRATE_DEPENDS+=	event-listener-strategy-0.5.1
CARGO_CRATE_DEPENDS+=	exr-1.72.0
CARGO_CRATE_DEPENDS+=	eyeball-0.8.7
CARGO_CRATE_DEPENDS+=	eyeball-im-0.4.2
CARGO_CRATE_DEPENDS+=	fallible-iterator-0.3.0
CARGO_CRATE_DEPENDS+=	fallible-streaming-iterator-0.1.9
CARGO_CRATE_DEPENDS+=	fancy_constructor-1.2.2
CARGO_CRATE_DEPENDS+=	fastrand-1.9.0
CARGO_CRATE_DEPENDS+=	fastrand-2.0.2
CARGO_CRATE_DEPENDS+=	fdeflate-0.3.4
CARGO_CRATE_DEPENDS+=	fiat-crypto-0.2.7
CARGO_CRATE_DEPENDS+=	fixedbitset-0.4.2
CARGO_CRATE_DEPENDS+=	flagset-0.4.5
CARGO_CRATE_DEPENDS+=	flate2-1.0.28
CARGO_CRATE_DEPENDS+=	flume-0.11.0
CARGO_CRATE_DEPENDS+=	fnv-1.0.7
CARGO_CRATE_DEPENDS+=	foreign-types-0.3.2
CARGO_CRATE_DEPENDS+=	foreign-types-0.5.0
CARGO_CRATE_DEPENDS+=	foreign-types-macros-0.2.3
CARGO_CRATE_DEPENDS+=	foreign-types-shared-0.1.1
CARGO_CRATE_DEPENDS+=	foreign-types-shared-0.3.1
CARGO_CRATE_DEPENDS+=	form_urlencoded-1.2.1
CARGO_CRATE_DEPENDS+=	fs2-0.4.3
CARGO_CRATE_DEPENDS+=	futf-0.1.5
CARGO_CRATE_DEPENDS+=	futures-0.3.30
CARGO_CRATE_DEPENDS+=	futures-channel-0.3.30
CARGO_CRATE_DEPENDS+=	futures-core-0.3.30
CARGO_CRATE_DEPENDS+=	futures-executor-0.3.30
CARGO_CRATE_DEPENDS+=	futures-io-0.3.30
CARGO_CRATE_DEPENDS+=	futures-lite-1.13.0
CARGO_CRATE_DEPENDS+=	futures-lite-2.3.0
CARGO_CRATE_DEPENDS+=	futures-macro-0.3.30
CARGO_CRATE_DEPENDS+=	futures-sink-0.3.30
CARGO_CRATE_DEPENDS+=	futures-task-0.3.30
CARGO_CRATE_DEPENDS+=	futures-util-0.3.30
CARGO_CRATE_DEPENDS+=	fxhash-0.2.1
CARGO_CRATE_DEPENDS+=	generic-array-0.14.7
CARGO_CRATE_DEPENDS+=	gethostname-0.4.3
CARGO_CRATE_DEPENDS+=	getrandom-0.2.14
CARGO_CRATE_DEPENDS+=	gif-0.13.1
CARGO_CRATE_DEPENDS+=	gimli-0.28.1
CARGO_CRATE_DEPENDS+=	gloo-timers-0.3.0
CARGO_CRATE_DEPENDS+=	gloo-utils-0.2.0
CARGO_CRATE_DEPENDS+=	h2-0.3.26
CARGO_CRATE_DEPENDS+=	half-2.4.1
CARGO_CRATE_DEPENDS+=	hashbrown-0.14.3
CARGO_CRATE_DEPENDS+=	hashlink-0.8.4
CARGO_CRATE_DEPENDS+=	heck-0.4.1
CARGO_CRATE_DEPENDS+=	hermit-abi-0.3.9
CARGO_CRATE_DEPENDS+=	hex-0.4.3
CARGO_CRATE_DEPENDS+=	hkdf-0.12.4
CARGO_CRATE_DEPENDS+=	hmac-0.12.1
CARGO_CRATE_DEPENDS+=	home-0.5.5
CARGO_CRATE_DEPENDS+=	html5ever-0.26.0
CARGO_CRATE_DEPENDS+=	http-0.2.12
CARGO_CRATE_DEPENDS+=	http-body-0.4.6
CARGO_CRATE_DEPENDS+=	httparse-1.8.0
CARGO_CRATE_DEPENDS+=	httpdate-1.0.3
CARGO_CRATE_DEPENDS+=	humansize-2.1.3
CARGO_CRATE_DEPENDS+=	hyper-0.14.28
CARGO_CRATE_DEPENDS+=	hyper-rustls-0.24.2
CARGO_CRATE_DEPENDS+=	hyper-tls-0.5.0
CARGO_CRATE_DEPENDS+=	iana-time-zone-0.1.57
CARGO_CRATE_DEPENDS+=	iana-time-zone-haiku-0.1.2
CARGO_CRATE_DEPENDS+=	icy_sixel-0.1.1
CARGO_CRATE_DEPENDS+=	ident_case-1.0.1
CARGO_CRATE_DEPENDS+=	idna-0.5.0
CARGO_CRATE_DEPENDS+=	image-0.24.9
CARGO_CRATE_DEPENDS+=	imbl-2.0.3
CARGO_CRATE_DEPENDS+=	imbl-sized-chunks-0.1.2
CARGO_CRATE_DEPENDS+=	include_dir-0.7.3
CARGO_CRATE_DEPENDS+=	include_dir_macros-0.7.3
CARGO_CRATE_DEPENDS+=	indexed_db_futures-0.4.1
CARGO_CRATE_DEPENDS+=	indexmap-2.2.6
CARGO_CRATE_DEPENDS+=	indoc-2.0.4
CARGO_CRATE_DEPENDS+=	inout-0.1.3
CARGO_CRATE_DEPENDS+=	instant-0.1.12
CARGO_CRATE_DEPENDS+=	intervaltree-0.2.7
CARGO_CRATE_DEPENDS+=	io-lifetimes-1.0.11
CARGO_CRATE_DEPENDS+=	ipnet-2.9.0
CARGO_CRATE_DEPENDS+=	is-terminal-0.4.12
CARGO_CRATE_DEPENDS+=	itertools-0.10.5
CARGO_CRATE_DEPENDS+=	itertools-0.12.1
CARGO_CRATE_DEPENDS+=	itoa-1.0.11
CARGO_CRATE_DEPENDS+=	jpeg-decoder-0.3.1
CARGO_CRATE_DEPENDS+=	js-sys-0.3.69
CARGO_CRATE_DEPENDS+=	js_int-0.2.2
CARGO_CRATE_DEPENDS+=	js_option-0.1.1
CARGO_CRATE_DEPENDS+=	keybindings-0.0.1
CARGO_CRATE_DEPENDS+=	konst-0.3.8
CARGO_CRATE_DEPENDS+=	konst_kernel-0.3.8
CARGO_CRATE_DEPENDS+=	lazy_static-0.1.16
CARGO_CRATE_DEPENDS+=	lazy_static-1.4.0
CARGO_CRATE_DEPENDS+=	lebe-0.5.2
CARGO_CRATE_DEPENDS+=	libc-0.2.153
CARGO_CRATE_DEPENDS+=	libloading-0.8.3
CARGO_CRATE_DEPENDS+=	libm-0.2.8
CARGO_CRATE_DEPENDS+=	libredox-0.1.3
CARGO_CRATE_DEPENDS+=	libsqlite3-sys-0.27.0
CARGO_CRATE_DEPENDS+=	linux-raw-sys-0.3.8
CARGO_CRATE_DEPENDS+=	linux-raw-sys-0.4.13
CARGO_CRATE_DEPENDS+=	lock_api-0.4.11
CARGO_CRATE_DEPENDS+=	log-0.4.21
CARGO_CRATE_DEPENDS+=	lru-0.12.3
CARGO_CRATE_DEPENDS+=	mac-0.1.1
CARGO_CRATE_DEPENDS+=	mac-notification-sys-0.6.1
CARGO_CRATE_DEPENDS+=	macroific-1.3.1
CARGO_CRATE_DEPENDS+=	macroific_attr_parse-1.3.0
CARGO_CRATE_DEPENDS+=	macroific_core-1.0.2
CARGO_CRATE_DEPENDS+=	macroific_macro-1.1.0
CARGO_CRATE_DEPENDS+=	malloc_buf-0.0.6
CARGO_CRATE_DEPENDS+=	maplit-1.0.2
CARGO_CRATE_DEPENDS+=	markup5ever-0.11.0
CARGO_CRATE_DEPENDS+=	markup5ever_rcdom-0.2.0
CARGO_CRATE_DEPENDS+=	matrix-pickle-0.1.1
CARGO_CRATE_DEPENDS+=	matrix-pickle-derive-0.1.1
CARGO_CRATE_DEPENDS+=	matrix-sdk-0.7.1
CARGO_CRATE_DEPENDS+=	matrix-sdk-base-0.7.0
CARGO_CRATE_DEPENDS+=	matrix-sdk-common-0.7.0
CARGO_CRATE_DEPENDS+=	matrix-sdk-crypto-0.7.0
CARGO_CRATE_DEPENDS+=	matrix-sdk-indexeddb-0.7.0
CARGO_CRATE_DEPENDS+=	matrix-sdk-sqlite-0.7.0
CARGO_CRATE_DEPENDS+=	matrix-sdk-store-encryption-0.7.0
CARGO_CRATE_DEPENDS+=	memchr-2.7.2
CARGO_CRATE_DEPENDS+=	memoffset-0.7.1
CARGO_CRATE_DEPENDS+=	memoffset-0.9.1
CARGO_CRATE_DEPENDS+=	mime-0.3.17
CARGO_CRATE_DEPENDS+=	mime2ext-0.1.52
CARGO_CRATE_DEPENDS+=	mime_guess-2.0.4
CARGO_CRATE_DEPENDS+=	minimal-lexical-0.2.1
CARGO_CRATE_DEPENDS+=	miniz_oxide-0.7.2
CARGO_CRATE_DEPENDS+=	mio-0.8.11
CARGO_CRATE_DEPENDS+=	modalkit-0.0.20
CARGO_CRATE_DEPENDS+=	modalkit-ratatui-0.0.20
CARGO_CRATE_DEPENDS+=	native-tls-0.2.11
CARGO_CRATE_DEPENDS+=	new_debug_unreachable-1.0.4
CARGO_CRATE_DEPENDS+=	nibble_vec-0.1.0
CARGO_CRATE_DEPENDS+=	nix-0.26.4
CARGO_CRATE_DEPENDS+=	nix-0.28.0
CARGO_CRATE_DEPENDS+=	nom-7.1.3
CARGO_CRATE_DEPENDS+=	notify-rust-4.10.0
CARGO_CRATE_DEPENDS+=	nu-ansi-term-0.46.0
CARGO_CRATE_DEPENDS+=	num-conv-0.1.0
CARGO_CRATE_DEPENDS+=	num-traits-0.2.18
CARGO_CRATE_DEPENDS+=	num_cpus-1.16.0
CARGO_CRATE_DEPENDS+=	num_threads-0.1.7
CARGO_CRATE_DEPENDS+=	objc-0.2.7
CARGO_CRATE_DEPENDS+=	objc-foundation-0.1.1
CARGO_CRATE_DEPENDS+=	objc_id-0.1.1
CARGO_CRATE_DEPENDS+=	object-0.32.2
CARGO_CRATE_DEPENDS+=	once_cell-1.19.0
CARGO_CRATE_DEPENDS+=	opaque-debug-0.3.1
CARGO_CRATE_DEPENDS+=	open-3.2.0
CARGO_CRATE_DEPENDS+=	openssl-0.10.64
CARGO_CRATE_DEPENDS+=	openssl-macros-0.1.1
CARGO_CRATE_DEPENDS+=	openssl-probe-0.1.5
CARGO_CRATE_DEPENDS+=	openssl-sys-0.9.102
CARGO_CRATE_DEPENDS+=	ordered-stream-0.2.0
CARGO_CRATE_DEPENDS+=	os_pipe-1.1.5
CARGO_CRATE_DEPENDS+=	overload-0.1.1
CARGO_CRATE_DEPENDS+=	parking-2.2.0
CARGO_CRATE_DEPENDS+=	parking_lot-0.11.2
CARGO_CRATE_DEPENDS+=	parking_lot-0.12.1
CARGO_CRATE_DEPENDS+=	parking_lot_core-0.8.6
CARGO_CRATE_DEPENDS+=	parking_lot_core-0.9.9
CARGO_CRATE_DEPENDS+=	paste-1.0.14
CARGO_CRATE_DEPENDS+=	pathdiff-0.2.1
CARGO_CRATE_DEPENDS+=	pbkdf2-0.12.2
CARGO_CRATE_DEPENDS+=	percent-encoding-2.3.1
CARGO_CRATE_DEPENDS+=	petgraph-0.6.4
CARGO_CRATE_DEPENDS+=	phf-0.10.1
CARGO_CRATE_DEPENDS+=	phf-0.11.2
CARGO_CRATE_DEPENDS+=	phf_codegen-0.10.0
CARGO_CRATE_DEPENDS+=	phf_generator-0.10.0
CARGO_CRATE_DEPENDS+=	phf_shared-0.10.0
CARGO_CRATE_DEPENDS+=	phf_shared-0.11.2
CARGO_CRATE_DEPENDS+=	pin-project-lite-0.2.14
CARGO_CRATE_DEPENDS+=	pin-utils-0.1.0
CARGO_CRATE_DEPENDS+=	piper-0.2.1
CARGO_CRATE_DEPENDS+=	pkcs7-0.4.1
CARGO_CRATE_DEPENDS+=	pkcs8-0.10.2
CARGO_CRATE_DEPENDS+=	pkg-config-0.3.30
CARGO_CRATE_DEPENDS+=	platforms-3.4.0
CARGO_CRATE_DEPENDS+=	png-0.17.13
CARGO_CRATE_DEPENDS+=	polling-2.8.0
CARGO_CRATE_DEPENDS+=	polling-3.5.0
CARGO_CRATE_DEPENDS+=	poly1305-0.8.0
CARGO_CRATE_DEPENDS+=	powerfmt-0.2.0
CARGO_CRATE_DEPENDS+=	ppv-lite86-0.2.17
CARGO_CRATE_DEPENDS+=	precomputed-hash-0.1.1
CARGO_CRATE_DEPENDS+=	pretty_assertions-1.4.0
CARGO_CRATE_DEPENDS+=	proc-macro-crate-1.3.1
CARGO_CRATE_DEPENDS+=	proc-macro-crate-2.0.0
CARGO_CRATE_DEPENDS+=	proc-macro-error-1.0.4
CARGO_CRATE_DEPENDS+=	proc-macro-error-attr-1.0.4
CARGO_CRATE_DEPENDS+=	proc-macro2-1.0.81
CARGO_CRATE_DEPENDS+=	prost-0.12.4
CARGO_CRATE_DEPENDS+=	prost-derive-0.12.4
CARGO_CRATE_DEPENDS+=	qoi-0.4.1
CARGO_CRATE_DEPENDS+=	quick-xml-0.30.0
CARGO_CRATE_DEPENDS+=	quick-xml-0.31.0
CARGO_CRATE_DEPENDS+=	quote-1.0.36
CARGO_CRATE_DEPENDS+=	radix_trie-0.2.1
CARGO_CRATE_DEPENDS+=	rand-0.8.5
CARGO_CRATE_DEPENDS+=	rand_chacha-0.3.1
CARGO_CRATE_DEPENDS+=	rand_core-0.6.4
CARGO_CRATE_DEPENDS+=	rand_xoshiro-0.6.0
CARGO_CRATE_DEPENDS+=	ratatui-0.26.1
CARGO_CRATE_DEPENDS+=	ratatui-image-1.0.0
CARGO_CRATE_DEPENDS+=	rayon-1.10.0
CARGO_CRATE_DEPENDS+=	rayon-core-1.12.1
CARGO_CRATE_DEPENDS+=	readlock-0.1.7
CARGO_CRATE_DEPENDS+=	redox_syscall-0.2.16
CARGO_CRATE_DEPENDS+=	redox_syscall-0.4.1
CARGO_CRATE_DEPENDS+=	redox_users-0.4.5
CARGO_CRATE_DEPENDS+=	regex-1.10.4
CARGO_CRATE_DEPENDS+=	regex-automata-0.4.6
CARGO_CRATE_DEPENDS+=	regex-syntax-0.8.3
CARGO_CRATE_DEPENDS+=	reqwest-0.11.27
CARGO_CRATE_DEPENDS+=	ring-0.17.8
CARGO_CRATE_DEPENDS+=	rmp-0.8.13
CARGO_CRATE_DEPENDS+=	rmp-serde-1.2.0
CARGO_CRATE_DEPENDS+=	ropey-1.6.1
CARGO_CRATE_DEPENDS+=	rpassword-7.2.0
CARGO_CRATE_DEPENDS+=	rtoolbox-0.0.1
CARGO_CRATE_DEPENDS+=	ruma-0.9.4
CARGO_CRATE_DEPENDS+=	ruma-client-api-0.17.4
CARGO_CRATE_DEPENDS+=	ruma-common-0.12.1
CARGO_CRATE_DEPENDS+=	ruma-events-0.27.11
CARGO_CRATE_DEPENDS+=	ruma-federation-api-0.8.0
CARGO_CRATE_DEPENDS+=	ruma-identifiers-validation-0.9.3
CARGO_CRATE_DEPENDS+=	ruma-macros-0.12.0
CARGO_CRATE_DEPENDS+=	rusqlite-0.30.0
CARGO_CRATE_DEPENDS+=	rustc-demangle-0.1.23
CARGO_CRATE_DEPENDS+=	rustc_version-0.4.0
CARGO_CRATE_DEPENDS+=	rustix-0.37.27
CARGO_CRATE_DEPENDS+=	rustix-0.38.32
CARGO_CRATE_DEPENDS+=	rustls-0.21.10
CARGO_CRATE_DEPENDS+=	rustls-pemfile-1.0.4
CARGO_CRATE_DEPENDS+=	rustls-webpki-0.101.7
CARGO_CRATE_DEPENDS+=	rustversion-1.0.14
CARGO_CRATE_DEPENDS+=	ryu-1.0.17
CARGO_CRATE_DEPENDS+=	schannel-0.1.23
CARGO_CRATE_DEPENDS+=	scoped-tls-1.0.1
CARGO_CRATE_DEPENDS+=	scopeguard-1.2.0
CARGO_CRATE_DEPENDS+=	sct-0.7.1
CARGO_CRATE_DEPENDS+=	security-framework-2.10.0
CARGO_CRATE_DEPENDS+=	security-framework-sys-2.10.0
CARGO_CRATE_DEPENDS+=	semver-1.0.22
CARGO_CRATE_DEPENDS+=	serde-1.0.198
CARGO_CRATE_DEPENDS+=	serde-wasm-bindgen-0.6.5
CARGO_CRATE_DEPENDS+=	serde_bytes-0.11.14
CARGO_CRATE_DEPENDS+=	serde_derive-1.0.198
CARGO_CRATE_DEPENDS+=	serde_html_form-0.2.6
CARGO_CRATE_DEPENDS+=	serde_json-1.0.116
CARGO_CRATE_DEPENDS+=	serde_repr-0.1.18
CARGO_CRATE_DEPENDS+=	serde_spanned-0.6.5
CARGO_CRATE_DEPENDS+=	serde_urlencoded-0.7.1
CARGO_CRATE_DEPENDS+=	sha1-0.10.6
CARGO_CRATE_DEPENDS+=	sha2-0.10.8
CARGO_CRATE_DEPENDS+=	sharded-slab-0.1.7
CARGO_CRATE_DEPENDS+=	signal-hook-0.3.17
CARGO_CRATE_DEPENDS+=	signal-hook-mio-0.2.3
CARGO_CRATE_DEPENDS+=	signal-hook-registry-1.4.1
CARGO_CRATE_DEPENDS+=	signature-2.2.0
CARGO_CRATE_DEPENDS+=	simd-adler32-0.3.7
CARGO_CRATE_DEPENDS+=	siphasher-0.3.11
CARGO_CRATE_DEPENDS+=	slab-0.4.9
CARGO_CRATE_DEPENDS+=	sled-0.34.7
CARGO_CRATE_DEPENDS+=	slug-0.1.5
CARGO_CRATE_DEPENDS+=	smallvec-1.13.2
CARGO_CRATE_DEPENDS+=	smawk-0.3.2
CARGO_CRATE_DEPENDS+=	socket2-0.4.10
CARGO_CRATE_DEPENDS+=	socket2-0.5.6
CARGO_CRATE_DEPENDS+=	spin-0.9.8
CARGO_CRATE_DEPENDS+=	spki-0.7.3
CARGO_CRATE_DEPENDS+=	stability-0.1.1
CARGO_CRATE_DEPENDS+=	static_assertions-1.1.0
CARGO_CRATE_DEPENDS+=	str_indices-0.4.3
CARGO_CRATE_DEPENDS+=	string_cache-0.8.7
CARGO_CRATE_DEPENDS+=	string_cache_codegen-0.5.2
CARGO_CRATE_DEPENDS+=	strsim-0.10.0
CARGO_CRATE_DEPENDS+=	strum-0.26.2
CARGO_CRATE_DEPENDS+=	strum_macros-0.26.2
CARGO_CRATE_DEPENDS+=	subtle-2.5.0
CARGO_CRATE_DEPENDS+=	syn-1.0.109
CARGO_CRATE_DEPENDS+=	syn-2.0.59
CARGO_CRATE_DEPENDS+=	sync_wrapper-0.1.2
CARGO_CRATE_DEPENDS+=	system-configuration-0.5.1
CARGO_CRATE_DEPENDS+=	system-configuration-sys-0.5.0
CARGO_CRATE_DEPENDS+=	tauri-winrt-notification-0.1.3
CARGO_CRATE_DEPENDS+=	temp-dir-0.1.12
CARGO_CRATE_DEPENDS+=	tempfile-3.10.1
CARGO_CRATE_DEPENDS+=	tendril-0.4.3
CARGO_CRATE_DEPENDS+=	textwrap-0.16.1
CARGO_CRATE_DEPENDS+=	thiserror-1.0.58
CARGO_CRATE_DEPENDS+=	thiserror-impl-1.0.58
CARGO_CRATE_DEPENDS+=	thread_local-1.1.8
CARGO_CRATE_DEPENDS+=	tiff-0.9.1
CARGO_CRATE_DEPENDS+=	time-0.3.36
CARGO_CRATE_DEPENDS+=	time-core-0.1.2
CARGO_CRATE_DEPENDS+=	time-macros-0.2.18
CARGO_CRATE_DEPENDS+=	tinyvec-1.6.0
CARGO_CRATE_DEPENDS+=	tinyvec_macros-0.1.1
CARGO_CRATE_DEPENDS+=	tokio-1.37.0
CARGO_CRATE_DEPENDS+=	tokio-macros-2.2.0
CARGO_CRATE_DEPENDS+=	tokio-native-tls-0.3.1
CARGO_CRATE_DEPENDS+=	tokio-rustls-0.24.1
CARGO_CRATE_DEPENDS+=	tokio-stream-0.1.15
CARGO_CRATE_DEPENDS+=	tokio-util-0.7.10
CARGO_CRATE_DEPENDS+=	toml-0.8.12
CARGO_CRATE_DEPENDS+=	toml_datetime-0.6.5
CARGO_CRATE_DEPENDS+=	toml_edit-0.19.15
CARGO_CRATE_DEPENDS+=	toml_edit-0.20.7
CARGO_CRATE_DEPENDS+=	toml_edit-0.22.9
CARGO_CRATE_DEPENDS+=	tower-0.4.13
CARGO_CRATE_DEPENDS+=	tower-layer-0.3.2
CARGO_CRATE_DEPENDS+=	tower-service-0.3.2
CARGO_CRATE_DEPENDS+=	tracing-0.1.40
CARGO_CRATE_DEPENDS+=	tracing-appender-0.2.3
CARGO_CRATE_DEPENDS+=	tracing-attributes-0.1.27
CARGO_CRATE_DEPENDS+=	tracing-core-0.1.32
CARGO_CRATE_DEPENDS+=	tracing-log-0.2.0
CARGO_CRATE_DEPENDS+=	tracing-subscriber-0.3.18
CARGO_CRATE_DEPENDS+=	tree_magic_mini-3.1.4
CARGO_CRATE_DEPENDS+=	try-lock-0.2.5
CARGO_CRATE_DEPENDS+=	typed-arena-2.0.2
CARGO_CRATE_DEPENDS+=	typenum-1.17.0
CARGO_CRATE_DEPENDS+=	typewit-1.9.0
CARGO_CRATE_DEPENDS+=	typewit_proc_macros-1.8.1
CARGO_CRATE_DEPENDS+=	uds_windows-1.1.0
CARGO_CRATE_DEPENDS+=	ulid-1.1.2
CARGO_CRATE_DEPENDS+=	unicase-2.7.0
CARGO_CRATE_DEPENDS+=	unicode-bidi-0.3.15
CARGO_CRATE_DEPENDS+=	unicode-ident-1.0.12
CARGO_CRATE_DEPENDS+=	unicode-linebreak-0.1.5
CARGO_CRATE_DEPENDS+=	unicode-normalization-0.1.23
CARGO_CRATE_DEPENDS+=	unicode-segmentation-1.11.0
CARGO_CRATE_DEPENDS+=	unicode-width-0.1.11
CARGO_CRATE_DEPENDS+=	unicode_categories-0.1.1
CARGO_CRATE_DEPENDS+=	universal-hash-0.5.1
CARGO_CRATE_DEPENDS+=	untrusted-0.9.0
CARGO_CRATE_DEPENDS+=	url-2.5.0
CARGO_CRATE_DEPENDS+=	urlencoding-2.1.3
CARGO_CRATE_DEPENDS+=	utf-8-0.7.6
CARGO_CRATE_DEPENDS+=	utf8parse-0.2.1
CARGO_CRATE_DEPENDS+=	uuid-1.6.1
CARGO_CRATE_DEPENDS+=	valuable-0.1.0
CARGO_CRATE_DEPENDS+=	vcpkg-0.2.15
CARGO_CRATE_DEPENDS+=	vergen-8.2.5
CARGO_CRATE_DEPENDS+=	version_check-0.9.4
CARGO_CRATE_DEPENDS+=	vodozemac-0.5.1
CARGO_CRATE_DEPENDS+=	waker-fn-1.1.1
CARGO_CRATE_DEPENDS+=	want-0.3.1
CARGO_CRATE_DEPENDS+=	wasi-0.11.0+wasi-snapshot-preview1
CARGO_CRATE_DEPENDS+=	wasm-bindgen-0.2.92
CARGO_CRATE_DEPENDS+=	wasm-bindgen-backend-0.2.92
CARGO_CRATE_DEPENDS+=	wasm-bindgen-futures-0.4.42
CARGO_CRATE_DEPENDS+=	wasm-bindgen-macro-0.2.92
CARGO_CRATE_DEPENDS+=	wasm-bindgen-macro-support-0.2.92
CARGO_CRATE_DEPENDS+=	wasm-bindgen-shared-0.2.92
CARGO_CRATE_DEPENDS+=	wasm-streams-0.4.0
CARGO_CRATE_DEPENDS+=	wayland-backend-0.3.3
CARGO_CRATE_DEPENDS+=	wayland-client-0.31.2
CARGO_CRATE_DEPENDS+=	wayland-protocols-0.31.2
CARGO_CRATE_DEPENDS+=	wayland-protocols-wlr-0.2.0
CARGO_CRATE_DEPENDS+=	wayland-scanner-0.31.1
CARGO_CRATE_DEPENDS+=	wayland-sys-0.31.1
CARGO_CRATE_DEPENDS+=	web-sys-0.3.69
CARGO_CRATE_DEPENDS+=	web-time-1.1.0
CARGO_CRATE_DEPENDS+=	webpki-roots-0.25.4
CARGO_CRATE_DEPENDS+=	weezl-0.1.8
CARGO_CRATE_DEPENDS+=	which-4.4.2
CARGO_CRATE_DEPENDS+=	wildmatch-2.3.3
CARGO_CRATE_DEPENDS+=	winapi-0.3.9
CARGO_CRATE_DEPENDS+=	winapi-i686-pc-windows-gnu-0.4.0
CARGO_CRATE_DEPENDS+=	winapi-x86_64-pc-windows-gnu-0.4.0
CARGO_CRATE_DEPENDS+=	windows-0.48.0
CARGO_CRATE_DEPENDS+=	windows-0.51.1
CARGO_CRATE_DEPENDS+=	windows-core-0.51.1
CARGO_CRATE_DEPENDS+=	windows-sys-0.42.0
CARGO_CRATE_DEPENDS+=	windows-sys-0.48.0
CARGO_CRATE_DEPENDS+=	windows-sys-0.52.0
CARGO_CRATE_DEPENDS+=	windows-targets-0.48.5
CARGO_CRATE_DEPENDS+=	windows-targets-0.52.5
CARGO_CRATE_DEPENDS+=	windows_aarch64_gnullvm-0.42.2
CARGO_CRATE_DEPENDS+=	windows_aarch64_gnullvm-0.48.5
CARGO_CRATE_DEPENDS+=	windows_aarch64_gnullvm-0.52.5
CARGO_CRATE_DEPENDS+=	windows_aarch64_msvc-0.42.2
CARGO_CRATE_DEPENDS+=	windows_aarch64_msvc-0.48.5
CARGO_CRATE_DEPENDS+=	windows_aarch64_msvc-0.52.5
CARGO_CRATE_DEPENDS+=	windows_i686_gnu-0.42.2
CARGO_CRATE_DEPENDS+=	windows_i686_gnu-0.48.5
CARGO_CRATE_DEPENDS+=	windows_i686_gnu-0.52.5
CARGO_CRATE_DEPENDS+=	windows_i686_gnullvm-0.52.5
CARGO_CRATE_DEPENDS+=	windows_i686_msvc-0.42.2
CARGO_CRATE_DEPENDS+=	windows_i686_msvc-0.48.5
CARGO_CRATE_DEPENDS+=	windows_i686_msvc-0.52.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnu-0.42.2
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnu-0.48.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnu-0.52.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnullvm-0.42.2
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnullvm-0.48.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnullvm-0.52.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_msvc-0.42.2
CARGO_CRATE_DEPENDS+=	windows_x86_64_msvc-0.48.5
CARGO_CRATE_DEPENDS+=	windows_x86_64_msvc-0.52.5
CARGO_CRATE_DEPENDS+=	winnow-0.5.40
CARGO_CRATE_DEPENDS+=	winnow-0.6.6
CARGO_CRATE_DEPENDS+=	winreg-0.50.0
CARGO_CRATE_DEPENDS+=	wl-clipboard-rs-0.8.1
CARGO_CRATE_DEPENDS+=	x11rb-0.13.0
CARGO_CRATE_DEPENDS+=	x11rb-protocol-0.13.0
CARGO_CRATE_DEPENDS+=	x25519-dalek-2.0.1
CARGO_CRATE_DEPENDS+=	x509-cert-0.2.5
CARGO_CRATE_DEPENDS+=	xdg-home-1.1.0
CARGO_CRATE_DEPENDS+=	xml5ever-0.17.0
CARGO_CRATE_DEPENDS+=	yansi-0.5.1
CARGO_CRATE_DEPENDS+=	zbus-3.15.2
CARGO_CRATE_DEPENDS+=	zbus_macros-3.15.2
CARGO_CRATE_DEPENDS+=	zbus_names-2.6.1
CARGO_CRATE_DEPENDS+=	zerocopy-0.7.32
CARGO_CRATE_DEPENDS+=	zerocopy-derive-0.7.32
CARGO_CRATE_DEPENDS+=	zeroize-1.7.0
CARGO_CRATE_DEPENDS+=	zeroize_derive-1.4.2
CARGO_CRATE_DEPENDS+=	zune-inflate-0.2.54
CARGO_CRATE_DEPENDS+=	zvariant-3.15.2
CARGO_CRATE_DEPENDS+=	zvariant_derive-3.15.2
CARGO_CRATE_DEPENDS+=	zvariant_utils-1.0.1
