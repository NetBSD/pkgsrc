...
sub parse_makefile_pkgsrc($) {
	...
	if (defined $pkgname) {
		if (defined $vars->{PKGREVISION} and not $vars->{PKGREVISION} =~ /^\s*$/) {
	...
	while (defined($_ = shift(@data))) {
		...
		if (m#^\.\s*elif\s+(.*)# && @if_false) {
			if ($if_false[$#if_false] == 0) {
				$if_false[$#if_false] = 2;
			} elsif ($if_false[$#if_false] == 1 && !parse_eval_make_false($1, \%vars)) {
				$if_false[$#if_false] = 0;
			}
		...
		if (m#^\.\s*include\s+"([^"]+)"#) {
			...
			# fully expand
			if ($incfile =~ m#/mk/bsd# || $incfile =~ /$magic_undefined/ || (!$opt{d} && $incfile =~ m#/(buildlink[^/]*\.mk)#)) {
...
sub scan_pkgsrc_distfiles_vs_distinfo($$$$) {
	...
	if ($check_distinfo) {
		...
		foreach my $sum (keys %sumfiles) {
			...
			open(DIGEST, "digest $sum @{$sumfiles{$sum}}|") || fail("Run digest: $!");
			while (<DIGEST>) {
				if (m/^$sum ?\(([^\)]+)\) = (\S+)/) {
