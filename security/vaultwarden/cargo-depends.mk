# $NetBSD: cargo-depends.mk,v 1.1 2022/08/17 15:44:16 hauke Exp $

CARGO_CRATE_DEPENDS+=	addr2line-0.17.0
CARGO_CRATE_DEPENDS+=	adler-1.0.2
CARGO_CRATE_DEPENDS+=	aead-0.4.3
CARGO_CRATE_DEPENDS+=	aes-0.7.5
CARGO_CRATE_DEPENDS+=	aes-gcm-0.9.4
CARGO_CRATE_DEPENDS+=	aho-corasick-0.7.18
CARGO_CRATE_DEPENDS+=	alloc-no-stdlib-2.0.3
CARGO_CRATE_DEPENDS+=	alloc-stdlib-0.2.1
CARGO_CRATE_DEPENDS+=	ansi_term-0.12.1
CARGO_CRATE_DEPENDS+=	async-compression-0.3.14
CARGO_CRATE_DEPENDS+=	async-stream-0.3.3
CARGO_CRATE_DEPENDS+=	async-stream-impl-0.3.3
CARGO_CRATE_DEPENDS+=	async-trait-0.1.56
CARGO_CRATE_DEPENDS+=	async_once-0.2.6
CARGO_CRATE_DEPENDS+=	atomic-0.5.1
CARGO_CRATE_DEPENDS+=	atty-0.2.14
CARGO_CRATE_DEPENDS+=	autocfg-1.1.0
CARGO_CRATE_DEPENDS+=	backtrace-0.3.66
CARGO_CRATE_DEPENDS+=	base64-0.13.0
CARGO_CRATE_DEPENDS+=	binascii-0.1.4
CARGO_CRATE_DEPENDS+=	bitflags-1.3.2
CARGO_CRATE_DEPENDS+=	block-buffer-0.7.3
CARGO_CRATE_DEPENDS+=	block-buffer-0.10.2
CARGO_CRATE_DEPENDS+=	block-padding-0.1.5
CARGO_CRATE_DEPENDS+=	brotli-3.3.4
CARGO_CRATE_DEPENDS+=	brotli-decompressor-2.3.2
CARGO_CRATE_DEPENDS+=	bumpalo-3.10.0
CARGO_CRATE_DEPENDS+=	byte-tools-0.3.1
CARGO_CRATE_DEPENDS+=	byteorder-1.4.3
CARGO_CRATE_DEPENDS+=	bytes-1.1.0
CARGO_CRATE_DEPENDS+=	cached-0.36.0
CARGO_CRATE_DEPENDS+=	cached_proc_macro-0.13.0
CARGO_CRATE_DEPENDS+=	cached_proc_macro_types-0.1.0
CARGO_CRATE_DEPENDS+=	cc-1.0.73
CARGO_CRATE_DEPENDS+=	cfg-if-1.0.0
CARGO_CRATE_DEPENDS+=	chrono-0.4.19
CARGO_CRATE_DEPENDS+=	chrono-tz-0.6.1
CARGO_CRATE_DEPENDS+=	chrono-tz-build-0.0.2
CARGO_CRATE_DEPENDS+=	cipher-0.3.0
CARGO_CRATE_DEPENDS+=	cookie-0.16.0
CARGO_CRATE_DEPENDS+=	cookie_store-0.16.1
CARGO_CRATE_DEPENDS+=	core-foundation-0.9.3
CARGO_CRATE_DEPENDS+=	core-foundation-sys-0.8.3
CARGO_CRATE_DEPENDS+=	cpufeatures-0.2.2
CARGO_CRATE_DEPENDS+=	crc32fast-1.3.2
CARGO_CRATE_DEPENDS+=	cron-0.11.0
CARGO_CRATE_DEPENDS+=	crossbeam-utils-0.8.10
CARGO_CRATE_DEPENDS+=	crypto-common-0.1.5
CARGO_CRATE_DEPENDS+=	ctr-0.8.0
CARGO_CRATE_DEPENDS+=	ctrlc-3.2.2
CARGO_CRATE_DEPENDS+=	darling-0.13.4
CARGO_CRATE_DEPENDS+=	darling_core-0.13.4
CARGO_CRATE_DEPENDS+=	darling_macro-0.13.4
CARGO_CRATE_DEPENDS+=	dashmap-5.3.4
CARGO_CRATE_DEPENDS+=	data-encoding-2.3.2
CARGO_CRATE_DEPENDS+=	data-url-0.1.1
CARGO_CRATE_DEPENDS+=	devise-0.3.1
CARGO_CRATE_DEPENDS+=	devise_codegen-0.3.1
CARGO_CRATE_DEPENDS+=	devise_core-0.3.1
CARGO_CRATE_DEPENDS+=	diesel-1.4.8
CARGO_CRATE_DEPENDS+=	diesel_derives-1.4.1
CARGO_CRATE_DEPENDS+=	diesel_migrations-1.4.0
CARGO_CRATE_DEPENDS+=	digest-0.8.1
CARGO_CRATE_DEPENDS+=	digest-0.10.3
CARGO_CRATE_DEPENDS+=	dirs-4.0.0
CARGO_CRATE_DEPENDS+=	dirs-sys-0.3.7
CARGO_CRATE_DEPENDS+=	dotenvy-0.15.1
CARGO_CRATE_DEPENDS+=	either-1.7.0
CARGO_CRATE_DEPENDS+=	email-encoding-0.1.3
CARGO_CRATE_DEPENDS+=	email_address-0.2.1
CARGO_CRATE_DEPENDS+=	encoding_rs-0.8.31
CARGO_CRATE_DEPENDS+=	enum-as-inner-0.4.0
CARGO_CRATE_DEPENDS+=	error-chain-0.12.4
CARGO_CRATE_DEPENDS+=	fake-simd-0.1.2
CARGO_CRATE_DEPENDS+=	fastrand-1.7.0
CARGO_CRATE_DEPENDS+=	fern-0.6.1
CARGO_CRATE_DEPENDS+=	figment-0.10.6
CARGO_CRATE_DEPENDS+=	flate2-1.0.24
CARGO_CRATE_DEPENDS+=	fnv-1.0.7
CARGO_CRATE_DEPENDS+=	foreign-types-0.3.2
CARGO_CRATE_DEPENDS+=	foreign-types-shared-0.1.1
CARGO_CRATE_DEPENDS+=	form_urlencoded-1.0.1
CARGO_CRATE_DEPENDS+=	futures-0.3.21
CARGO_CRATE_DEPENDS+=	futures-channel-0.3.21
CARGO_CRATE_DEPENDS+=	futures-core-0.3.21
CARGO_CRATE_DEPENDS+=	futures-executor-0.3.21
CARGO_CRATE_DEPENDS+=	futures-io-0.3.21
CARGO_CRATE_DEPENDS+=	futures-macro-0.3.21
CARGO_CRATE_DEPENDS+=	futures-sink-0.3.21
CARGO_CRATE_DEPENDS+=	futures-task-0.3.21
CARGO_CRATE_DEPENDS+=	futures-timer-3.0.2
CARGO_CRATE_DEPENDS+=	futures-util-0.3.21
CARGO_CRATE_DEPENDS+=	generator-0.7.0
CARGO_CRATE_DEPENDS+=	generic-array-0.12.4
CARGO_CRATE_DEPENDS+=	generic-array-0.14.5
CARGO_CRATE_DEPENDS+=	getrandom-0.2.7
CARGO_CRATE_DEPENDS+=	ghash-0.4.4
CARGO_CRATE_DEPENDS+=	gimli-0.26.1
CARGO_CRATE_DEPENDS+=	glob-0.3.0
CARGO_CRATE_DEPENDS+=	governor-0.4.2
CARGO_CRATE_DEPENDS+=	h2-0.3.13
CARGO_CRATE_DEPENDS+=	half-1.8.2
CARGO_CRATE_DEPENDS+=	handlebars-4.3.2
CARGO_CRATE_DEPENDS+=	hashbrown-0.11.2
CARGO_CRATE_DEPENDS+=	hashbrown-0.12.2
CARGO_CRATE_DEPENDS+=	heck-0.4.0
CARGO_CRATE_DEPENDS+=	hermit-abi-0.1.19
CARGO_CRATE_DEPENDS+=	hkdf-0.12.3
CARGO_CRATE_DEPENDS+=	hmac-0.12.1
CARGO_CRATE_DEPENDS+=	hostname-0.3.1
CARGO_CRATE_DEPENDS+=	html5gum-0.5.2
CARGO_CRATE_DEPENDS+=	http-0.2.8
CARGO_CRATE_DEPENDS+=	http-body-0.4.5
CARGO_CRATE_DEPENDS+=	httparse-1.7.1
CARGO_CRATE_DEPENDS+=	httpdate-1.0.2
CARGO_CRATE_DEPENDS+=	hyper-0.14.20
CARGO_CRATE_DEPENDS+=	hyper-tls-0.5.0
CARGO_CRATE_DEPENDS+=	ident_case-1.0.1
CARGO_CRATE_DEPENDS+=	idna-0.1.5
CARGO_CRATE_DEPENDS+=	idna-0.2.3
CARGO_CRATE_DEPENDS+=	indexmap-1.9.1
CARGO_CRATE_DEPENDS+=	inlinable_string-0.1.15
CARGO_CRATE_DEPENDS+=	instant-0.1.12
CARGO_CRATE_DEPENDS+=	ipconfig-0.3.0
CARGO_CRATE_DEPENDS+=	ipnet-2.5.0
CARGO_CRATE_DEPENDS+=	itoa-1.0.2
CARGO_CRATE_DEPENDS+=	jetscii-0.5.3
CARGO_CRATE_DEPENDS+=	job_scheduler_ng-2.0.1
CARGO_CRATE_DEPENDS+=	js-sys-0.3.58
CARGO_CRATE_DEPENDS+=	jsonwebtoken-8.1.1
CARGO_CRATE_DEPENDS+=	lazy_static-1.4.0
CARGO_CRATE_DEPENDS+=	lettre-0.10.0
CARGO_CRATE_DEPENDS+=	libc-0.2.126
CARGO_CRATE_DEPENDS+=	libmimalloc-sys-0.1.25
CARGO_CRATE_DEPENDS+=	libsqlite3-sys-0.22.2
CARGO_CRATE_DEPENDS+=	linked-hash-map-0.5.6
CARGO_CRATE_DEPENDS+=	lock_api-0.4.7
CARGO_CRATE_DEPENDS+=	log-0.4.17
CARGO_CRATE_DEPENDS+=	loom-0.5.6
CARGO_CRATE_DEPENDS+=	lru-cache-0.1.2
CARGO_CRATE_DEPENDS+=	mach-0.3.2
CARGO_CRATE_DEPENDS+=	maplit-1.0.2
CARGO_CRATE_DEPENDS+=	match_cfg-0.1.0
CARGO_CRATE_DEPENDS+=	matchers-0.1.0
CARGO_CRATE_DEPENDS+=	matches-0.1.9
CARGO_CRATE_DEPENDS+=	memchr-2.5.0
CARGO_CRATE_DEPENDS+=	migrations_internals-1.4.1
CARGO_CRATE_DEPENDS+=	migrations_macros-1.4.2
CARGO_CRATE_DEPENDS+=	mimalloc-0.1.29
CARGO_CRATE_DEPENDS+=	mime-0.3.16
CARGO_CRATE_DEPENDS+=	minimal-lexical-0.2.1
CARGO_CRATE_DEPENDS+=	miniz_oxide-0.5.3
CARGO_CRATE_DEPENDS+=	mio-0.8.4
CARGO_CRATE_DEPENDS+=	multer-2.0.3
CARGO_CRATE_DEPENDS+=	mysqlclient-sys-0.2.5
CARGO_CRATE_DEPENDS+=	native-tls-0.2.10
CARGO_CRATE_DEPENDS+=	nix-0.24.1
CARGO_CRATE_DEPENDS+=	no-std-compat-0.4.1
CARGO_CRATE_DEPENDS+=	nom-7.1.1
CARGO_CRATE_DEPENDS+=	nonzero_ext-0.3.0
CARGO_CRATE_DEPENDS+=	num-bigint-0.4.3
CARGO_CRATE_DEPENDS+=	num-derive-0.3.3
CARGO_CRATE_DEPENDS+=	num-integer-0.1.45
CARGO_CRATE_DEPENDS+=	num-traits-0.2.15
CARGO_CRATE_DEPENDS+=	num_cpus-1.13.1
CARGO_CRATE_DEPENDS+=	num_threads-0.1.6
CARGO_CRATE_DEPENDS+=	object-0.29.0
CARGO_CRATE_DEPENDS+=	once_cell-1.13.0
CARGO_CRATE_DEPENDS+=	opaque-debug-0.2.3
CARGO_CRATE_DEPENDS+=	opaque-debug-0.3.0
CARGO_CRATE_DEPENDS+=	openssl-0.10.41
CARGO_CRATE_DEPENDS+=	openssl-macros-0.1.0
CARGO_CRATE_DEPENDS+=	openssl-probe-0.1.5
CARGO_CRATE_DEPENDS+=	openssl-src-111.22.0+1.1.1q
CARGO_CRATE_DEPENDS+=	openssl-sys-0.9.75
CARGO_CRATE_DEPENDS+=	parking_lot-0.12.1
CARGO_CRATE_DEPENDS+=	parking_lot_core-0.9.3
CARGO_CRATE_DEPENDS+=	parse-zoneinfo-0.3.0
CARGO_CRATE_DEPENDS+=	paste-1.0.7
CARGO_CRATE_DEPENDS+=	pear-0.2.3
CARGO_CRATE_DEPENDS+=	pear_codegen-0.2.3
CARGO_CRATE_DEPENDS+=	pem-1.1.0
CARGO_CRATE_DEPENDS+=	percent-encoding-1.0.1
CARGO_CRATE_DEPENDS+=	percent-encoding-2.1.0
CARGO_CRATE_DEPENDS+=	pest-2.1.3
CARGO_CRATE_DEPENDS+=	pest_derive-2.1.0
CARGO_CRATE_DEPENDS+=	pest_generator-2.1.3
CARGO_CRATE_DEPENDS+=	pest_meta-2.1.3
CARGO_CRATE_DEPENDS+=	phf-0.10.1
CARGO_CRATE_DEPENDS+=	phf_codegen-0.10.0
CARGO_CRATE_DEPENDS+=	phf_generator-0.10.0
CARGO_CRATE_DEPENDS+=	phf_shared-0.10.0
CARGO_CRATE_DEPENDS+=	pico-args-0.5.0
CARGO_CRATE_DEPENDS+=	pin-project-lite-0.2.9
CARGO_CRATE_DEPENDS+=	pin-utils-0.1.0
CARGO_CRATE_DEPENDS+=	pkg-config-0.3.25
CARGO_CRATE_DEPENDS+=	polyval-0.5.3
CARGO_CRATE_DEPENDS+=	ppv-lite86-0.2.16
CARGO_CRATE_DEPENDS+=	pq-sys-0.4.6
CARGO_CRATE_DEPENDS+=	proc-macro-hack-0.5.19
CARGO_CRATE_DEPENDS+=	proc-macro2-1.0.40
CARGO_CRATE_DEPENDS+=	proc-macro2-diagnostics-0.9.1
CARGO_CRATE_DEPENDS+=	psl-types-2.0.10
CARGO_CRATE_DEPENDS+=	publicsuffix-2.1.1
CARGO_CRATE_DEPENDS+=	quanta-0.9.3
CARGO_CRATE_DEPENDS+=	quick-error-1.2.3
CARGO_CRATE_DEPENDS+=	quote-1.0.20
CARGO_CRATE_DEPENDS+=	quoted_printable-0.4.5
CARGO_CRATE_DEPENDS+=	r2d2-0.8.10
CARGO_CRATE_DEPENDS+=	rand-0.8.5
CARGO_CRATE_DEPENDS+=	rand_chacha-0.3.1
CARGO_CRATE_DEPENDS+=	rand_core-0.6.3
CARGO_CRATE_DEPENDS+=	raw-cpuid-10.3.0
CARGO_CRATE_DEPENDS+=	redox_syscall-0.2.13
CARGO_CRATE_DEPENDS+=	redox_users-0.4.3
CARGO_CRATE_DEPENDS+=	ref-cast-1.0.7
CARGO_CRATE_DEPENDS+=	ref-cast-impl-1.0.7
CARGO_CRATE_DEPENDS+=	regex-1.6.0
CARGO_CRATE_DEPENDS+=	regex-automata-0.1.10
CARGO_CRATE_DEPENDS+=	regex-syntax-0.6.27
CARGO_CRATE_DEPENDS+=	remove_dir_all-0.5.3
CARGO_CRATE_DEPENDS+=	reqwest-0.11.11
CARGO_CRATE_DEPENDS+=	resolv-conf-0.7.0
CARGO_CRATE_DEPENDS+=	ring-0.16.20
CARGO_CRATE_DEPENDS+=	rmp-0.8.11
CARGO_CRATE_DEPENDS+=	rmpv-1.0.0
CARGO_CRATE_DEPENDS+=	rocket-0.5.0-rc.2
CARGO_CRATE_DEPENDS+=	rocket_codegen-0.5.0-rc.2
CARGO_CRATE_DEPENDS+=	rocket_http-0.5.0-rc.2
CARGO_CRATE_DEPENDS+=	rustc-demangle-0.1.21
CARGO_CRATE_DEPENDS+=	rustls-0.20.6
CARGO_CRATE_DEPENDS+=	rustls-pemfile-1.0.0
CARGO_CRATE_DEPENDS+=	rustversion-1.0.7
CARGO_CRATE_DEPENDS+=	ryu-1.0.10
CARGO_CRATE_DEPENDS+=	same-file-1.0.6
CARGO_CRATE_DEPENDS+=	schannel-0.1.20
CARGO_CRATE_DEPENDS+=	scheduled-thread-pool-0.2.6
CARGO_CRATE_DEPENDS+=	scoped-tls-1.0.0
CARGO_CRATE_DEPENDS+=	scopeguard-1.1.0
CARGO_CRATE_DEPENDS+=	sct-0.7.0
CARGO_CRATE_DEPENDS+=	security-framework-2.6.1
CARGO_CRATE_DEPENDS+=	security-framework-sys-2.6.1
CARGO_CRATE_DEPENDS+=	serde-1.0.139
CARGO_CRATE_DEPENDS+=	serde_cbor-0.11.2
CARGO_CRATE_DEPENDS+=	serde_derive-1.0.139
CARGO_CRATE_DEPENDS+=	serde_json-1.0.82
CARGO_CRATE_DEPENDS+=	serde_urlencoded-0.7.1
CARGO_CRATE_DEPENDS+=	sha-1-0.8.2
CARGO_CRATE_DEPENDS+=	sha-1-0.10.0
CARGO_CRATE_DEPENDS+=	sha1-0.10.1
CARGO_CRATE_DEPENDS+=	sha2-0.10.2
CARGO_CRATE_DEPENDS+=	sharded-slab-0.1.4
CARGO_CRATE_DEPENDS+=	signal-hook-registry-1.4.0
CARGO_CRATE_DEPENDS+=	simple_asn1-0.6.2
CARGO_CRATE_DEPENDS+=	siphasher-0.3.10
CARGO_CRATE_DEPENDS+=	slab-0.4.6
CARGO_CRATE_DEPENDS+=	smallvec-1.9.0
CARGO_CRATE_DEPENDS+=	socket2-0.4.4
CARGO_CRATE_DEPENDS+=	spin-0.5.2
CARGO_CRATE_DEPENDS+=	spin-0.9.4
CARGO_CRATE_DEPENDS+=	stable-pattern-0.1.0
CARGO_CRATE_DEPENDS+=	state-0.5.3
CARGO_CRATE_DEPENDS+=	strsim-0.10.0
CARGO_CRATE_DEPENDS+=	subtle-2.4.1
CARGO_CRATE_DEPENDS+=	syn-1.0.98
CARGO_CRATE_DEPENDS+=	syslog-6.0.1
CARGO_CRATE_DEPENDS+=	tempfile-3.3.0
CARGO_CRATE_DEPENDS+=	thiserror-1.0.31
CARGO_CRATE_DEPENDS+=	thiserror-impl-1.0.31
CARGO_CRATE_DEPENDS+=	thread_local-1.1.4
CARGO_CRATE_DEPENDS+=	threadpool-1.8.1
CARGO_CRATE_DEPENDS+=	time-0.1.43
CARGO_CRATE_DEPENDS+=	time-0.3.11
CARGO_CRATE_DEPENDS+=	time-macros-0.2.4
CARGO_CRATE_DEPENDS+=	tinyvec-1.6.0
CARGO_CRATE_DEPENDS+=	tinyvec_macros-0.1.0
CARGO_CRATE_DEPENDS+=	tokio-1.20.0
CARGO_CRATE_DEPENDS+=	tokio-macros-1.8.0
CARGO_CRATE_DEPENDS+=	tokio-native-tls-0.3.0
CARGO_CRATE_DEPENDS+=	tokio-rustls-0.23.4
CARGO_CRATE_DEPENDS+=	tokio-socks-0.5.1
CARGO_CRATE_DEPENDS+=	tokio-stream-0.1.9
CARGO_CRATE_DEPENDS+=	tokio-tungstenite-0.17.2
CARGO_CRATE_DEPENDS+=	tokio-util-0.7.3
CARGO_CRATE_DEPENDS+=	toml-0.5.9
CARGO_CRATE_DEPENDS+=	totp-lite-2.0.0
CARGO_CRATE_DEPENDS+=	tower-service-0.3.2
CARGO_CRATE_DEPENDS+=	tracing-0.1.35
CARGO_CRATE_DEPENDS+=	tracing-attributes-0.1.22
CARGO_CRATE_DEPENDS+=	tracing-core-0.1.28
CARGO_CRATE_DEPENDS+=	tracing-log-0.1.3
CARGO_CRATE_DEPENDS+=	tracing-subscriber-0.3.14
CARGO_CRATE_DEPENDS+=	trust-dns-proto-0.21.2
CARGO_CRATE_DEPENDS+=	trust-dns-resolver-0.21.2
CARGO_CRATE_DEPENDS+=	try-lock-0.2.3
CARGO_CRATE_DEPENDS+=	tungstenite-0.17.3
CARGO_CRATE_DEPENDS+=	typenum-1.15.0
CARGO_CRATE_DEPENDS+=	ubyte-0.10.2
CARGO_CRATE_DEPENDS+=	ucd-trie-0.1.4
CARGO_CRATE_DEPENDS+=	uncased-0.9.7
CARGO_CRATE_DEPENDS+=	unicode-bidi-0.3.8
CARGO_CRATE_DEPENDS+=	unicode-ident-1.0.1
CARGO_CRATE_DEPENDS+=	unicode-normalization-0.1.21
CARGO_CRATE_DEPENDS+=	unicode-xid-0.2.3
CARGO_CRATE_DEPENDS+=	universal-hash-0.4.1
CARGO_CRATE_DEPENDS+=	untrusted-0.7.1
CARGO_CRATE_DEPENDS+=	url-1.7.2
CARGO_CRATE_DEPENDS+=	url-2.2.2
CARGO_CRATE_DEPENDS+=	utf-8-0.7.6
CARGO_CRATE_DEPENDS+=	uuid-1.1.2
CARGO_CRATE_DEPENDS+=	valuable-0.1.0
CARGO_CRATE_DEPENDS+=	vcpkg-0.2.15
CARGO_CRATE_DEPENDS+=	version_check-0.9.4
CARGO_CRATE_DEPENDS+=	walkdir-2.3.2
CARGO_CRATE_DEPENDS+=	want-0.3.0
CARGO_CRATE_DEPENDS+=	wasi-0.10.2+wasi-snapshot-preview1
CARGO_CRATE_DEPENDS+=	wasi-0.11.0+wasi-snapshot-preview1
CARGO_CRATE_DEPENDS+=	wasm-bindgen-0.2.81
CARGO_CRATE_DEPENDS+=	wasm-bindgen-backend-0.2.81
CARGO_CRATE_DEPENDS+=	wasm-bindgen-futures-0.4.31
CARGO_CRATE_DEPENDS+=	wasm-bindgen-macro-0.2.81
CARGO_CRATE_DEPENDS+=	wasm-bindgen-macro-support-0.2.81
CARGO_CRATE_DEPENDS+=	wasm-bindgen-shared-0.2.81
CARGO_CRATE_DEPENDS+=	web-sys-0.3.58
CARGO_CRATE_DEPENDS+=	webauthn-rs-0.3.2
CARGO_CRATE_DEPENDS+=	webpki-0.22.0
CARGO_CRATE_DEPENDS+=	widestring-0.5.1
CARGO_CRATE_DEPENDS+=	winapi-0.3.9
CARGO_CRATE_DEPENDS+=	winapi-i686-pc-windows-gnu-0.4.0
CARGO_CRATE_DEPENDS+=	winapi-util-0.1.5
CARGO_CRATE_DEPENDS+=	winapi-x86_64-pc-windows-gnu-0.4.0
CARGO_CRATE_DEPENDS+=	windows-sys-0.36.1
CARGO_CRATE_DEPENDS+=	windows_aarch64_msvc-0.36.1
CARGO_CRATE_DEPENDS+=	windows_i686_gnu-0.36.1
CARGO_CRATE_DEPENDS+=	windows_i686_msvc-0.36.1
CARGO_CRATE_DEPENDS+=	windows_x86_64_gnu-0.36.1
CARGO_CRATE_DEPENDS+=	windows_x86_64_msvc-0.36.1
CARGO_CRATE_DEPENDS+=	winreg-0.7.0
CARGO_CRATE_DEPENDS+=	winreg-0.10.1
CARGO_CRATE_DEPENDS+=	yansi-0.5.1
CARGO_CRATE_DEPENDS+=	yubico-0.11.0
